 ...
 def pbJudgeSwitch(favorDraws = false)
  ...
  def pbMegaEvolve(index)
    ...
    # Resurgence Crystal Delta Metagross animation
      ...
      pbCommonAnimation("Freeze Shock charging",@battlers[index],nil)
    elsif @battlers[index].species == :RAYQUAZA
  ...
  def pbOnActiveOne(pkmn, onlyabilities = false)
    ...
    if !onlyabilities
      ...
      if pkmn.crested == :MINUN && (pkmn.pbPartner.species == :PLUSLE && pkmn.pbPartner.isDelta?) && pkmn.pbOwnSide.effects[:AuroraVeil] == 0
      ...
      if pkmn.crested == :PLUSLE && (pkmn.pbPartner.species == :MINUN && pkmn.pbPartner.isDelta?)
  ...
  def runtrainerskills(pkmn, delay = false)
    ...
    if trainereffect[:fieldChange] && trainereffect[:fieldChange][0] != @field.effect
      if trainereffect[:fieldChange][0] == :DIMENSIONAL
        duration = 5
