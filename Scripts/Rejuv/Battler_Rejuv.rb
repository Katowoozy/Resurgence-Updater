class PokeBattle_Battler
    ...
    def pbUseMoveSimpleBoss(moveid, index = -1, target = -1, danced = false, specialZ = false)
      choice = [:move, index, moveid, self, target]
      ...
      def crestStats
        ...
        # Resurgence - Delta Avalugg Crest calling all of its Abilities using LAWDS' P3 Code
        when :AVALUGG
          ability = [:SOLIDROCK,:BATTLEARMOR,:SANDSTREAM]
          self.ability = PokeAbility.new(ability)
        when :LUXRAY
          @speed = @attack if @form == 1
        when :INFERNAPE
      ...
      def pbFaint(showMessage=true)
        ...
        # Resurgence - Return Delta Emolga to its Normal form if it faints.
        if @species == :EMOLGA && @pokemon.form == 2
          self.form = 1
        end
