  ...
  def getMoveScore(initialscores=[],scoreindex=-1)
    ...
    if @mondata.skill >= HIGHSKILL && @opponent.index != @attacker.index
      for j in getAIMemory(@opponent)
        ...
        shieldcheck = true if j.move == :SPIKYSHIELD || j.move == :KINGSSHIELD || j.move == :BANEFULBUNKER || j.move == :OBSTRUCT || j.move == :SILKTRAP || j.move == :BURNINGBULWARK
      end
...
#All functions here return a modifier to the original score, similar to miniscore
    ...
    if (@attacker.hp.to_f)/@attacker.totalhp<0.5
      ...
      if @attacker.hp*4<@attacker.totalhp
        ...
        if @attacker.hp<@attacker.totalhp*0.13
          ...
          # Gen 9 Mod - Hail/Snow/Both consider HAIL only when it is damaging
          miniscore*=2 if (@battle.pbWeather== :HAIL && !@attacker.hasType?(:ICE) && HAILSNOWMOD != "Snow") || (@battle.pbWeather== :SANDSTORM && !@attacker.hasType?(:ROCK) && !@attacker.hasType?(:GROUND) && !@attacker.hasType?(:STEEL)) || (@battle.pbWeather== :SHADOWSKY && !@attacker.hasType?(:SHADOW) && !@attacker.hasType?(:DARK) && !@attacker.hasType?(:GHOST))
        end
      end
    else
      miniscore*=0.9
  ...
  def endurecode
    ...
    # Gen 9 Mod - Hail/Snow/Both consider HAIL only when it is damaging
    return 0 if (@battle.pbWeather== :HAIL && !@attacker.hasType?(:ICE) && HAILSNOWMOD != "Snow") || (@battle.pbWeather== :SANDSTORM && !(@attacker.hasType?(:ROCK) || @attacker.hasType?(:GROUND) || @attacker.hasType?(:STEEL))) || (@battle.pbWeather== :SHADOWSKY && !@attacker.hasType?(:SHADOW) && !@attacker.hasType?(:DARK) && !@attacker.hasType?(:GHOST))
...
######################################################
  ...
  def hpGainPerTurn(attacker=@attacker)
    ...
    if attacker.ability != :MAGICGUARD && !(attacker.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
      ...
      healing -= (Rejuv && @battle.FE == :DESERT) ? 0.125 : 0.0625 if @battle.pbWeather == :SANDSTORM && (!(attacker.hasType?(:GROUND) || attacker.hasType?(:ROCK) || attacker.hasType?(:STEEL) || [:SANDFORCE,:SANDRUSH,:SANDVEIL,:MAGICGUARD,:OVERCOAT,:TEMPEST,:SANDSTREAM].include?(attacker.ability)) || attacker.effects[:DesertsMark])
...
################################################################################
  ...
  def getSwitchInScoresParty(hard_switch, revival = false)
    ...
    for partyindex in 0...party.length
      ...
      if !i.moves.any? {|moveloop| moveloop != nil && moveloop.move != 0 && moveloop.move != :LASTRESPECTS}
        partyScores.push(-5000)
      ...
      if @mondata.skill>=HIGHSKILL
        ...
        if (i.item == :FOCUSSASH || (@battle.FE == :CHESS && i.pokemon.piece==:PAWN) || i.ability == :STURDY || (@battle.FE == :CHESS && i.ability == :STALWART)) && i.hp == i.totalhp
          # Gen 9 Mod - Hail/Snow/Both
          if   (((@battle.weather== :SANDSTORM && !(i.hasType?(:ROCK) || i.hasType?(:GROUND) || i.hasType?(:STEEL)))  || (@battle.weather== :HAIL && !(i.hasType?(:ICE) && HAILSNOWMOD != "Snow"))) && !((i.ability == :OVERCOAT)))  || @attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock] ||
  ...
  def pbRoughDamage(move = @move, attacker = @attacker, opponent = @opponent)
    ...
    if @mondata.skill >= MEDIUMSKILL
      ...
      case attacker.ability
        ...
        # only check attacker ability here, partner ability is checked seperately
        when :FLOWERGIFT then atk = (atk * 1.5).round if attacker.pbPartner.flowerGiftActive? && (move.pbIsPhysical?(attacker, type) || (move.pbIsSpecial?(attacker, type) && attackerUseSpDef))
...
#####################################################
  ...
  def notOHKO?(attacker,opponent, immediate = false)
    ...
    return false if @battle.pbWeather == :SANDSTORM && !(attacker.hasType?(:ROCK) || attacker.hasType?(:GROUND) || attacker.hasType?(:STEEL) || [:SANDFORCE,:SANDRUSH,:SANDVEIL,:MAGICGUARD,:OVERCOAT,:TEMPEST,:SANDSTREAM].include?(attacker.ability)) && !immediate # Resurgence added Sand Stream
  ...
  def seedProtection?(battler = @attacker)
    return battler.effects[:KingsShield] || battler.effects[:BanefulBunker] || battler.effects[:SpikyShield] || battler.effects[:SilkTrap] || battler.effects[:BurningBulwark] # Gen 9 Mod - Added Silk Trap and Burning Bulwark
  end
