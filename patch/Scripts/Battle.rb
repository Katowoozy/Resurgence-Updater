...
################################################################################
  ...
  def quarkdriveCheck
    priority = pbPriority
    if [:ELECTERRAIN, :SHORTCIRCUIT, :FACTORY].include?(@field.effect) || @state.effects[:ELECTERRAIN] > 0
      for i in priority
        next if i.isFainted?
  ...
  def protosynthesisCheck
    ...
    if pbWeather != :SUNNYDAY && !@state.effects[:HarshSunlight]
      for i in priority
        ...
        pbDisplay(_INTL("{1} couldn't take advantage of the sunny surroundings!", i.pbThis))
        if i.item == :BOOSTERENERGY
          i.pbDisposeItem(false)
          @battle.pbDisplay(_INTL("{1}'s Booster Energy was used up...", i.pbThis))
          aBoost = (i.attack * PBStats::StageMul[i.stages[PBStats::ATTACK]]).floor
  ...
  def protoabsolutionCheck
    priority == pbPriority
    if pbWeather == :SHADOWSKY 
      for i in priority
        next if i.isFainted?
        next if i.ability != :PROTOABSOLUTION
        next if i.effects[:Protoabsolution][0] > 0
        aBoost = (i.attack * PBStats::StageMul[i.stages[PBStats::ATTACK]]).floor
        dBoost = (i.defense * PBStats::StageMul[i.stages[PBStats::DEFENSE]]).floor
        saBoost = (i.spatk * PBStats::StageMul[i.stages[PBStats::SPATK]]).floor
        sdBoost = (i.spdef * PBStats::StageMul[i.stages[PBStats::SPDEF]]).floor
        ...
        stats = [aBoost, dBoost, saBoost, sdBoost, spdBoost]
        boostStat = stats.index(stats.max) + 1
        i.effects[:Protoabsolution] = [boostStat, false]
        @battle.pbDisplay(_INTL("{1}'s Protoabsolution heightened its {2}!", i.pbThis, i.pbGetStatName(boostStat)))
      end
    end
    if pbWeather != :SHADOWSKY
      for i in priority
        next if i.isFainted?
        next if i.effects[:Protoabsolution][0] == 0
        next if i.effects[:Protoabsolution][1]

        i.effects[:Protoabsolution] = [0, false]
        pbDisplay(_INTL("{1} couldn't take advantage of the dark surroundings!", i.pbThis))
        if i.item == :BOOSTERENERGY
          i.pbDisposeItem(false)
          @battle.pbDisplay(_INTL("{1}'s Booster Energy was used up...", i.pbThis))
          aBoost = (i.attack * PBStats::StageMul[i.stages[PBStats::ATTACK]]).floor
          ...
          stats = [aBoost, dBoost, saBoost, sdBoost, spdBoost]
          boostStat = stats.index(stats.max) + 1
          i.effects[:Protoabsolution] = [boostStat, true]
          @battle.pbDisplay(_INTL("{1}'s Protoabsolution heightened its {2}!", i.pbThis, i.pbGetStatName(boostStat)))
        end
      end
    end
  ...
  def protocryophilicCheck
    ...
    if pbWeather == :HAIL || [:ICY, :SNOWYMOUNTAIN, :FROZENDIMENSION].include?(@field.effect)
      for i in priority
        ...
        stats = [aBoost, dBoost, saBoost, sdBoost, spdBoost]
        boostStat = stats.index(stats.max) + 1
        i.effects[:Protocryophilic] = [boostStat, false]
    ...
    if pbWeather != :HAIL && ![:ICY, :SNOWYMOUNTAIN, :FROZENDIMENSION].include?(@field.effect)
      for i in priority
        next if i.isFainted?
        next if i.effects[:Protocryophilic][0] == 0
        next if i.effects[:Protocryophilic][1]

        i.effects[:Protocryophilic] = [0, false]
        pbDisplay(_INTL("{1} couldn't take advantage of the snowy surroundings!", i.pbThis))
        if i.item == :BOOSTERENERGY
          i.pbDisposeItem(false)
          @battle.pbDisplay(_INTL("{1}'s Booster Energy was used up...", i.pbThis))
          aBoost = (i.attack * PBStats::StageMul[i.stages[PBStats::ATTACK]]).floor
          ...
          stats = [aBoost, dBoost, saBoost, sdBoost, spdBoost]
          boostStat = stats.index(stats.max) + 1
          i.effects[:Protocryophilic] = [boostStat, true]
          @battle.pbDisplay(_INTL("{1}'s Protocryophilic heightened its {2}!", i.pbThis, i.pbGetStatName(boostStat)))
        end
      end
    end
  ...
  def seedCheck
    for battler in pbPriority
      ...
      case @field.effect
        ...
        when :WASTELAND
          if battler.ability == :FOUNDRY
            battler.pbOwnSide.effects[:FoundryStealthRock] = true
            battler.pbOpposingSide.effects[:FoundryStealthRock] = true
          else
  ...
  def pbIsUnlosableItem(pkmn, item)
    ...
    # Resurgence - Making the Delta Noctowl/Luxray Crests unlosable if Delta Noctowl/Luxray are Crested
    return true if ((pkmn.species == :NOCTOWL && item == :DENOCCREST) || pkmn.species == :LUXRAY && pkmn.species == :DELUXCREST) && pkmn.form == 1
    ...
    return true if (pkmn.species == :REGIGIGAS || pkmn.species == :GIRATINA || pkmn.species == :ARCEUS) && item == :CRYSTALPIECE
  ...
  def pbCanShowCommands?(idxPokemon)
    ...
    return false if thispkmn.effects[:Rollout] > 0 && thispkmn.ability!=:JUGGERNAUT # Resurgence - LAWDS Juggernaut added
  ...
  def pbPriority(ignorequickclaw = true, megacalc = false)
    ...
    for i in 0..3
      ...
      if @choices[i][0] == :move # Is a move
        ...
        pri += 1 if @battlers[i].pbOwnSide.effects[:JetStream] == 1 # Resurgence - Jet Stream
        pri += 2 if @battlers[i].pbOwnSide.effects[:JetStream] == 1 && @battle.FE == :SKY # Resurgence - Jet Stream
      end
      ...
      if !ignorequickclaw && @choices[i][0] == :move # Is a move
        ...
        # Resurgence - Partner receiving Custap effect if partnered with Crested Delta Crustle
        elsif @battlers[i].partnercustap
          priorityarray[i][1] = 1
          quickclawarray[i] = :CUSTAPBERRY
        end
      end
  ...
  def pbCanSwitch?(idxPokemon, pkmnidxTo, showMessages, ai_phase = false, running: false)
    ...
    if thispkmn.effects[:SkyDrop] || thispkmn.effects[:SpiritAway] # lía
    ...
    if thispkmn.hasWorkingItem(:SHEDSHELL) || thispkmn.hasWorkingItem(:NOCTURNEINCENSE)
      return true
    end
 ...
 def pbJudgeSwitch(favorDraws = false)
  ...
  def pbCanMegaEvolve?(index)
    ...
    return false if @megaEvolution[side][owner]!=index && side == 0 && owner== 0
  ...
  def pbMegaEvolve(index)
    ...
    # Resurgence Crystal Megagross
    elsif @battlers[index].species == :METAGROSS && @battlers[index].item==:CRYSTALFRAGMENT
      pbDisplay(_INTL("{1} began to crystallize!", @battlers[index].pbThis))
    # Resurgence - fih
    elsif @battle.opponent && !@battle.opponent.is_a?(Array) && @battle.opponent.trainertype == :FIHHORDE
      pbDisplay(_INTL("{1} embraces the power of the Fih Horde!", @battlers[index].pbThis))
    # Resurgence - tingothy
    elsif @battle.opponent && !@battle.opponent.is_a?(Array) && @battle.opponent.trainertype == :DESERTHORDE
      pbDisplay(_INTL("{1}'s Mega Power started going rogue!", @battlers[index].pbThis))
    else
    ...
    # Resurgence Crystal Delta Metagross animation
    elsif @battlers[index].species == :METAGROSS && @battlers[index].item==:CRYSTALFRAGMENT
      pbCommonAnimation("Crystallization",@battlers[index],nil)
    elsif @battlers[index].species == :RAYQUAZA
    ...
    if @battlers[index].item==:PULSEHOLD
      ...
      pbDisplay(_INTL("{1} mutated into {2}!", @battlers[index].pbThis, meganame))
    elsif @battlers[index].item==:CRYSTALFRAGMENT
      pbDisplay(_INTL("{1} crystallized into {2}!", @battlers[index].pbThis, meganame))
    else
  ...
  def pbOnActiveOne(pkmn, onlyabilities = false)
    ...
    if !onlyabilities
      ...
      # Resurgence - Foundry Stealth Rock
      if pkmn.pbOwnSide.effects[:FoundryStealthRock] && @field.effect != :WASTELAND
        if pkmn.ability != :MAGICGUARD && !(pkmn.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM) && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS)
          atype = :FIRE
          atype = @field.getRoll if @field.effect == :CRYSTALCAVERN
          atype = :ROCK if @field.effect == :ROCKY || @field.effect == :CAVE
          atype = :POISON if @field.effect == :CORRUPTED
          eff=PBTypes.twoTypeEff(atype,pkmn.type1,pkmn.type2)
          if @field.effect == :INVERSE
            switcheff = { 16 => 1, 8 => 2, 4 => 4, 2 => 8, 1 => 16, 0 => 16}
          ...
          if eff>0
            if @field.effect == :VOLCANICTOP || @field.effect == :INFERNAL || (Rejuv && @field.effect == :DRAGONSDEN)
              eff = eff*2
            end
            @scene.pbDamageAnimation(pkmn, 0)
            pkmn.pbReduceHP([(pkmn.totalhp * eff / 32).floor, 1].max)
            if @field.effect == :CRYSTALCAVERN
              pbDisplay(_INTL("{1} was hurt by the crystalized stealth rocks!", pkmn.pbThis))
            elsif @field.effect == :ROCKY || @field.effect == :CAVE
              pbDisplay(_INTL("{1} was hurt by Stealth Rocks!", pkmn.pbThis))
            elsif @field.effect == :CORRUPTED
              pbDisplay(_INTL("{1} was hurt by the corrupted stealth rocks!", pkmn.pbThis))
            else
      ...
      # Resurgence - Delta Minun Crest activation on entry if partnered with Delta Plusle
      if pkmn.crested == :MINUN && (pkmn.pbPartner.species == :PLUSLE && pkmn.pbPartner.isDelta?) && pkmn.pbOwnSide.effects[:AuroraVeil] == 0 && pkmn.pbOwnSide.effects[:DeMinCrest] == false
        pbAnimation(:AURORAVEIL, pkmn, nil)
        pkmn.pbOwnSide.effects[:AuroraVeil] = 5
        pkmn.pbOwnSide.effects[:DeMinCrest] = true
        @battle.pbDisplay(_INTL("{1} set up an Aurora Veil to help its partner!", pkmn.pbThis))
      end
      # Resurgence - Delta Plusle Crest activation on entry if partnered with Delta Minun
      if pkmn.crested == :PLUSLE && (pkmn.pbPartner.species == :MINUN && pkmn.pbPartner.isDelta?) && pkmn.pbOwnSide.effects[:DePlusCrest] == false
        pbAnimation(:BURNINGBULWARK, pkmn, nil)
        pbAnimation(:BURNINGBULWARK, pkmn.pbPartner, nil)
        pkmn.effects[:Protect] = :BurningBulwark
        pkmn.effects[:ProtectRate] += 1
        pkmn.pbPartner.effects[:Protect] = :BurningBulwark
        pkmn.pbPartner.effects[:ProtectRate] += 1
        pkmn.pbOwnSide.effects[:DePlusCrest] = true
        @battle.pbDisplay(_INTL("{1} set up a Burning Bulwark for its partner and itself!", pkmn.pbThis))
      end
      if pkmn.isFainted?
        pkmn.pbFaint
        pkmn.pbOwnSide.effects[:Retaliate] = true
  ...
  def runtrainerskills(pkmn, delay = false)
    ...
    if trainereffect[:fieldChange] && trainereffect[:fieldChange][0] != @field.effect
      if trainereffect[:fieldChange][0] == :DIMENSIONAL && !@opponent.is_a?(Array) && @opponent.trainertype == :INTERCEPTOR
        duration = 5
        pbCommonAnimation("Lunar Cannon charging",pkmn,nil)
        setField(trainereffect[:fieldChange][0],duration)
      elsif trainereffect[:fieldChange][0] == :FAIRYTALE && !@opponent.is_a?(Array) && @opponent.trainertype == :REBORNCHAMPION
        duration = 5
        pbAnimation(:GEOMANCY,pkmn,nil)
        setField(trainereffect[:fieldChange][0],duration)
      elsif trainereffect[:fieldChange][0] == :COLOSSEUM
        pbAnimation(:FISSURE,pkmn,nil)
 ...
 def pbCommandPhase(delay = true)
      ...
      if pbCanShowCommands?(i)
        loop do
          ...
          if cmd == 0 # Fight
            ...
            elsif @battlers[i].effects[:SpiritAway]
              pbDisplay(_INTL("{1}'s spirit isn't here!", @battlers[i].name))
            else
              item = pbItemMenu(i)
              if item[0]
  ...
  def pbAttackPhase
    ...
    @switching = false
    for i in priority
      if switched.include?(i.pbPartner.index)
        # Resurgence - Delta Minun Crest activation upon Delta Plusle switching in
        if i.crested == :MINUN && (i.pbPartner.species == :PLUSLE && i.pbPartner.isDelta?) && i.pbOwnSide.effects[:AuroraVeil] == 0
          pbAnimation(:AURORAVEIL, i, nil)
          i.pbOwnSide.effects[:AuroraVeil] = 5
          @battle.pbDisplay(_INTL("{1} set up an Aurora Veil to help its partner!", i.pbThis))
        end
        # Resurgence - Delta Plusle Crest activation upon Delta Minun switching in
        if i.crested == :PLUSLE && (i.pbPartner.species == :MINUN && i.pbPartner.isDelta?)
          pbAnimation(:BURNINGBULWARK, i, nil)
          pbAnimation(:BURNINGBULWARK, i.pbPartner, nil)
          i.effects[:Protect] = :BurningBulwark
          i.effects[:ProtectRate] += 1
          i.pbPartner.effects[:Protect] = :BurningBulwark
          i.pbPartner.effects[:ProtectRate] += 1
          @battle.pbDisplay(_INTL("{1} set up a Burning Bulwark for its partner and itself!", i.pbThis))
        end
      end
    end
    # Gen 9 Mod - Checks and updates for Opportunist
    ...
    for i in 0...4
      ...
      for j in 0...4
        if !switched.include?(j)
          @battlers[j].pbPartnerCustapBerry
        end
      end
    end
    # Use items
    ...
    for i in priority
      if i.species == :EEVEE && i.form >= 4
        change = true
        protype = i.moves[@choices[i.index][1]].type
        if protype == :NORMAL && i.form != 4
          i.form = 4
        elsif protype == :WATER && i.form != 5
          i.form = 5
        elsif protype == :ELECTRIC && i.form != 6
          i.form = 6
        elsif protype == :FIRE && i.form != 7
          i.form = 7
        elsif protype == :PSYCHIC && i.form != 8
          i.form = 8
        elsif protype == :DARK && i.form != 9
          i.form = 9
        elsif protype == :GRASS && i.form != 10
          i.form = 10
        elsif protype == :ICE && i.form != 11
          i.form = 11
        elsif protype == :FAIRY && i.form != 12
          i.form = 12
        else
          change = false
        end
        if change
          transformed = true
          i.pokemon.initAbility()
          pbCommonAnimation("TypeRoll",i,nil)
          i.pbUpdate(true)
  ...
  def pbPursuitInterrupt(pursuiter, switcher)
    newpoke = nil
    if pursuiter.status != :SLEEP && !pursuiter.effects[:Truant]
      @switching = true
  ...
  def pbEndOfRoundPhase
    ...
    for i in 0...4
      ...
      @battlers[i].effects[:Chlorofury] -= 1  if @battlers[i].effects[:Chlorofury] > 0
      @battlers[i].effects[:Unleafed] -= 1    if @battlers[i].effects[:Unleafed] > 0
    end
    ...
    for i in priority
      ...
      case @field.effect
        ...
        when :VOLCANICTOP
          ...
          if @eruption
            if i.hasType?(:FIRE) ||
               ...
               # Resurgence
               i.ability == :BLAZEBOOST || i.ability == :PRISMGUARD || 
        ...
        when :HOLY # Resurgence - Blessed Field damages Heliophobia
          next if i.hp <= 0

          if i.ability == :HELIOPHOBIA  
            @scene.pbDamageAnimation(i, 0)
            i.pbReduceHP((i.totalhp / 8).floor)
            pbDisplay(_INTL("The holy energy damaged {1}!", i.pbThis))
            if i.hp <= 0
              return if !i.pbFaint
            end
          end
    ...
    if @field.effect == :VOLCANICTOP
      if @eruption
        ...
        for i in priority
          ...
          if i.pbOwnSide.effects[:StealthRock] || i.pbOwnSide.effects[:FoundryStealthRock]
            i.pbOwnSide.effects[:StealthRock] = false
            i.pbOwnSide.effects[:FoundryStealthRock] = false
            hazardsOnSide = true
          end
    ...
    case @weather
      ...
      when :RAINDANCE
        ...
        if @weatherduration == 0
          ...
          if pbWeather == :RAINDANCE
            for i in priority
              next if i.isFainted?
              if i.ability == :SUPERCELL
                pbDisplay(_INTL("{1} was hurt by the surging electricity!", i.pbThis))
                @scene.pbDamageAnimation(i, 0)
                i.pbReduceHP((i.totalhp / 8.0).floor)
                if i.isFainted?
                  return if !i.pbFaint
        ...
        if @weatherduration == 0
          ...
          if pbWeather == :SANDSTORM
            ...
            for i in priority
              ...
              if (!i.hasType?(:GROUND) && !i.hasType?(:ROCK) && !i.hasType?(:STEEL) && !i.hasType?(:CRYSTAL) && !(i.ability == :SANDVEIL || i.ability == :SANDRUSH ||
                i.ability == :SANDFORCE || i.ability == :MAGICGUARD || i.ability == :TEMPEST || i.ability == :SANDSTREAM || (i.ability == :WONDERGUARD && @field.effect == :COLOSSEUM) || i.ability == :OVERCOAT) &&
            ...
            for i in priority
              ...
              if HAILSNOWMOD != "Snow"
                if !i.hasType?(:ICE) && i.item != :SAFETYGOGGLES &&
                   ![:TEMPEST, :ICEBODY, :SNOWCLOAK, :SLUSHRUSH, :LUNARIDOL, :MAGICGUARD, :OVERCOAT, :ICECLEATS, :SNOWWARNING, :SLEET].include?(i.ability) && # Resurgence added Ice Cleats, Snow Warning and Sleet
                  ...
                  if @battle.pbCheckGlobalAbility(:SLEET)
                    pbDisplay(_INTL("The Pokémon were bombarded by the heavy Sleet!", i.pbThis)) if !endmessage
                  else
                    pbDisplay(_INTL("The Pokémon were buffeted by the hail!", i.pbThis)) if !endmessage
                  end
                  endmessage = true
                  @scene.pbDamageAnimation(i, 0, quick: true)
                  if @field.effect == :FROZENDIMENSION && @battle.pbCheckGlobalAbility(:SLEET)
                    reductions.push [i, (i.totalhp / 6.0).floor]
                  elsif @field.effect == :FROZENDIMENSION || @battle.pbCheckGlobalAbility(:SLEET)
                    reductions.push [i, (i.totalhp / 8.0).floor]
                  else
                    reductions.push [i, (i.totalhp / 16.0).floor]
                  end
      ...
      when :SHADOWSKY
        ...
        if @weatherduration == 0
          ...
          if @field.effect == :CRYSTALCAVERN # Crystal Cavern
            duration = @weatherduration + 1
            setField(:DARKCRYSTALCAVERN, duration)
            @field.duration_condition = proc { |battle| battle.weather == :SHADOWSKY }
            @field.permanent_condition = proc { |battle| battle.FE != :DARKCRYSTALCAVERN }
            pbDisplay(_INTL("The crystals lost their light!"))
          end
          if @field.effect == :HOLY # Blessed
            duration = @weatherduration + 1
            setField(:HAUNTED, duration)
            @field.duration_condition = proc { |battle| battle.weather == :SHADOWSKY }
            @field.permanent_condition = proc { |battle| battle.FE != :HAUNTED }
            pbDisplay(_INTL("Evil spirits gathered under the Shadow Sky!"))
          end
          if @weather == :SHADOWSKY
            ...
            for i in priority
              ...
              if i.ability == :ABSOLUTION
                pbDisplay(_INTL("{1} was hurt by the darkness!", i.pbThis))
                @scene.pbDamageAnimation(i, 0)
                i.pbReduceHP((i.totalhp / 8.0).floor)
                if i.isFainted?
                  return if !i.pbFaint
              ...
              if !i.isShadow? && !i.hasType?(:DARK) && !i.hasType?(:GHOST) && i.item != :SAFETYGOGGLES &&
                 ![:TEMPEST, :MAGICGUARD, :OVERCOAT, :SHADOWDANCE, :ABSOLUTION, :HELIOPHOBIA, :NOCTEM].include?(i.ability) && !(i.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM) &&
                 ($cache.moves[i.effects[:TwoTurnAttack]].nil? || ![0xCA, 0xCB].include?($cache.moves[i.effects[:TwoTurnAttack]].function)) # Dig, Dive
    ...
    # Resurgence - Supercell activated by field
    if pbWeather != :RAINDANCE
      for i in priority
        next if i.isFainted?
        if i.ability == :SUPERCELL && (@field.effect == :FACTORY || @field.effect == :SHORTCIRCUIT || @field.effect == :ELECTRICTERRAIN)
          pbDisplay(_INTL("{1} was hurt by the surging electricity!",i.pbThis))
          @scene.pbDamageAnimation(i,0)
          i.pbReduceHP((i.totalhp/8.0).floor)
          if i.isFainted?
    ...
    # Resurgence - Absolution activated by field
    if pbWeather != :SHADOWSKY
      for i in priority
        next if i.isFainted?
        if i.ability == :ABSOLUTION && [:DARKCRYSTALCAVERN, :DARKNESS1, :DARKNESS2, :DARKNESS3, :SHORTCIRCUIT, :DIMENSIONAL].include?(@battle.FE)
          pbDisplay(_INTL("{1} was hurt by the darkness!",i.pbThis))
          @scene.pbDamageAnimation(i,0)
          i.pbReduceHP((i.totalhp/8.0).floor)
          if i.isFainted?
    ...
    for i in battlers # not priority
      ...
      i.effects[:FutureSightMove] = 0 if i.effects[:FutureSight2]==0
      i.effects[:FutureSightUser] = -1 if i.effects[:FutureSight2]==0
      i.effects[:FutureSightPokemonIndex] = -1 if i.effects[:FutureSight2]==0
    ...
    # Periodic Orbit - Future Sight/Doom Desire
    for i in battlers # not priority
      next if i.effects[:FutureSight2] <= 0

      i.effects[:FutureSight2] -= 1
      next if i.isFainted? || i.effects[:FutureSight2] != 0
    ...
    for i in priority
      ...
      # Resurgence - Heliophobia
      if (i.ability == :HELIOPHOBIA && ((pbWeather == :SHADOWSKY && !i.hasWorkingItem(:SAFETYGOGGLES)) || @field.effect == :DARKCRYSTALCAVERN || @field.effect == :DARKNESS1 || @field.effect == :DARKNESS2 || @field.effect == :DARKNESS3 || @field.effect == :SHORTCIRCUIT || @field.effect == :DIMENSIONAL)) && i.effects[:HealBlock] == 0
        hpgain = i.pbRecoverHP((i.totalhp / 16.0).floor, true)
        pbDisplay(_INTL("{1}'s Heliophobia restored its HP a little!", i.pbThis)) if hpgain > 0
      end
      ...
      if i.ability == :PHOTOTROPH && i.effects[:HealBlock] == 0
        return false if (pbWeather == :RAINDANCE || pbWeather == :SHADOWSKY)
        hpgain = (i.totalhp / 16.0).floor
        hpgain = (i.totalhp / 8.0).floor if pbWeather == :SUNNYDAY
        hpgain = i.pbRecoverHP(hpgain, true)
        pbDisplay(_INTL("{1}'s Phototroph restored its HP a little!", i.pbThis)) if hpgain > 0
      end
      ...
      # Resurgence - Sublimating Edge Type Switch
      if i.ability == :SUBLIMATINGEDGE
        if i.hasType?(:ELECTRIC)
          @battle.pbAnimation(:MIST, i, nil)
          i.type1 = :ICE
        elsif i.hasType?(:ICE)
          @battle.pbCommonAnimation("Charge", i, nil)
          i.type1 = :ELECTRIC
        end
        typename = getTypeName(i.type1)
        @battle.pbDisplay(_INTL("{1} transformed into an {2}-type!", i.pbThis, typename))
      end
    end
    # Wish
    for i in priority
    ...
    for i in priority
      ...
      if i.effects[:Nightmare] && i.ability != :MAGICGUARD && !(i.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM) && @field.effect != :RAINBOW
        if ((i.status == :SLEEP || (i.ability == :COMATOSE && @battle.FE != :ELECTERRAIN)) || @battle.FE == :INFERNAL || i.pbOpposing1.ability == :WORLDOFNIGHTMARES || i.pbOpposing2.ability == :WORLDOFNIGHTMARES)
          ...
          hploss = (i.totalhp / 2.0).floor if pbWeather == :SHADOWSKY
          i.pbReduceHP(hploss, true)
        else
    ...
    for i in 0...2
      next if sides[i].effects[:JetStream] == 0

      sides[i].effects[:JetStream] -= 1
      pbDisplay(_INTL("#{texts[i]} team's supercharged air faded!")) if sides[i].effects[:JetStream] == 0
    end
    # Lucky Chant
    for i in 0...2
    ...
    # Resurgence - Amplifier Buff to wake up all Pokemon
    for i in priority
      next if i.isFainted?

      if i.effects[:Amplifier] > 0
        i.effects[:Amplifier] -= 1
        if i.effects[:Amplifier] == 0
          pbDisplay(_INTL("{1}'s amplified noise calmed down.", i.pbThis))
        end
      end
    end
    ...
    for i in priority
      ...
      # Resurgence
      if i.ability == :CHLOROFURY && i.effects[:Chlorofury]==0
        calmed = i.effects[:FaintedBoost]
        for stat in [PBStats::SPATK, PBStats::SPEED]
          if !i.pbTooLow?(stat)
            i.pbReduceStatBasic(stat, calmed)
          end
        end
        @battle.pbCommonAnimation("StatDown", i)
        pbDisplay(_INTL("{1} calmed Down!", i.pbThis))
      end
      if i.ability == :UNLEAFED && i.effects[:Unleafed]==0
        calmed = i.effects[:FaintedBoost]
        ...
        if i.species == :SUNFLORA && i.form==3
          i.form=2
    ...
    if @field.effect == :WASTELAND
      for i in priority
        ...
        elsif i.pbOwnSide.effects[:FoundryStealthRock]
    ...
    for i in priority
      ...
      if (i.status == :SLEEP || (i.ability == :COMATOSE && @battle.FE != :ELECTERRAIN)) && i.ability != :MAGICGUARD && !(i.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM) && @field.effect != :RAINBOW
        if i.pbOpposing1.ability == :BADDREAMS || i.pbOpposing2.ability == :BADDREAMS
          ...
          hpdrain = (i.totalhp / 4.0).floor if @battle.FE == :INFERNAL || @battle.FE == :DARKNESS3 || pbWeather == :SHADOWSKY
    ...
    for i in priority
      ...
      # Resurgence - Frost Orb
      if i.item == :FROSTORB && i.status.nil? && i.pbCanFrostbite?(false, true)
        i.status = :FROSTBITE
        i.statusCount = 0
        pbCommonAnimation("Frozen", i, nil)
        pbDisplay(_INTL("{1} was frostbitten by its {2}!", i.pbThis, getItemName(i.item)))
      end
      # Sticky Barb
  ...
  def pbEndOfBattle(canlose = false)
    ...
    for i in @party1
      ...
      i.form = 0 if i.species == :MIMIKYU || i.species == :EISCUE || i.species == :PALAFIN || i.species == :TERAPAGOS
      # Resurgence - Temporary forms
      i.form = 2 if (i.species == :TYPHLOSION && i.form == 5)
      i.form = 1 if (i.species == :EMOLGA && i.form == 2)
      i.form = 0 if (i.species == :SUNFLORA && i.form == 3)
      i.form = 0 if (i.species == :HYDREIGON && i.form > 1)
      i.form = 0 if (i.species == :EEVEE && i.form > 4)
      if Rejuv
    ...
    # Resurgence - Revert Battle Bond Forms and reset flags
    PokeBattle_Battler.revert_all_battle_bond_pokemon
