...
def pbLoadPokemonBitmapSpecies(pokemon, species, back=false)
  ...
  if (pokemon.form !=0 && (ShitList.include?(species))) || toobig
    if !(species == :STEELIX && [1,2].include?(pokemon.form))  #mega steelix is excused.
      ...
      if (species == :VENUSAUR && (pokemon.form == 4))
        bitmapfilename2 = sprintf("Graphics/Battlers/%03d%s%s%s%s",dexnum,gendermod,shinytag,backtag,formnumber)
        bitmapfilename2 = sprintf("Graphics/Battlers/%s%s%s%s%s",dexname,gendermod,shinytag,backtag,formnumber) if !pbResolveBitmap(bitmapfilename2)
      end
...
def LevelLimitExpGain(pokemon, exp) # For exp candies
  ...
  if pokemon.level >= LEVELCAPS[leadersDefeated] || pokemon.level >= 100 + $game_variables[:Extended_Max_Level] || noExp?
    return -1
