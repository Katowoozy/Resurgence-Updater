...
class FightMenuButtons < BitmapSprite
  ...
  def pbFieldNotesBattle(move)
    ...
    when :UNDERWATER
      ...
      moveBoost = 0 if [:RAINDANCE, :SUNNYDAY, :HAIL, :SANDSTORM, :SHADOWPRAYER].include?(move.move)
    when :NEWWORLD
      moveBoost = 0 if [:ELECTRICTERRAIN, :GRASSYTERRAIN, :MISTYTERRAIN, :PSYCHICTERRAIN, :MIST].include?(move.move)
      moveBoost = 0 if [:RAINDANCE, :SUNNYDAY, :HAIL, :SANDSTORM, :SHADOWPRAYER].include?(move.move)
    when :ELECTERRAIN
...
class PokeballSendOutAnimation
  ...
  def update
    ...
    if @frame==4
      ...
      if @illusionpoke != nil
        pbPlayCry(@illusionpoke.pokemon)
      else
...
# Tests whether or not the battle item will have any effect
  def pbCanUseBattleItem(pkmnid, item, pkmnscreen)
    ...
    # Resurgence - Battle item won't have effect when Spirited Away
    if battler && (battler.effects[:SkyDrop] || battler.effects[:Commander] || battler.effects[:SpiritAway]) # Gen 9 Mod - Battle item won't have effect on Tatsugiri in Dondozo's mouth.
      return false
    end
...
# for certain moves in double battles.
  ...
  def pbChooseTargetOneSide(index, target) # Gen 9 Mod - Ported from Reborn 19.5
    pbShowWindow(FIGHTBOX)
