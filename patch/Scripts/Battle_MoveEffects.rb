...
class PokeBattle_Move_000 < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && [:HYPERVOICE,:BOOMBURST,:OVERDRIVE].include?(@move)
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
  end
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if id == :DRACOJET
      @battle.pbAnimation(:DRAGONHAMMER,attacker,opponent,hitnum)
    elsif id == :CORRODE
      @battle.pbAnimation(:POISONGAS,attacker,opponent,hitnum)
    elsif id == :ANCIENTROAR
      @battle.pbAnimation(:NOBLEROAR,attacker,opponent,hitnum)
    elsif id == :DRAKONVOICE
      @battle.pbAnimation(:CLANGINGSCALES,attacker,opponent,hitnum)
    elsif id == :CRYSTALRUSH
...
# Relic Song / Lovely Kiss / Sing / Hypnosis / Wicked Torque)
################################################################################
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if [:VOLCANICTOP,:BACKALLEY,:CITY].include?(@battle.FE) && @move == :POISONGAS
      ...
      if attacker.ability == :POISONPUPPETEER && opponent.pbCanConfuse?(true)
      	...
      	@battle.pbDisplay(_INTL("{1} is badly poisoned and confused!",opponent.pbThis))
      else
      	@battle.pbDisplay(_INTL("{1} is badly poisoned!",opponent.pbThis))
      end
    elsif attacker.ability == :VENOMOUS
        opponent.pbPoison(attacker,true)
        @battle.pbDisplay(_INTL("{1} is badly poisoned!",opponent.pbThis))
    else
      ...
      if attacker.ability == :POISONPUPPETEER && opponent.pbCanConfuse?(true)
      	opponent.effects[:Confusion]=2+@battle.pbRandom(4)
      	@battle.pbCommonAnimation("Confusion",opponent,nil)
      	@battle.pbDisplay(_INTL("{1} is poisoned and confused!",opponent.pbThis))
      else
    ...
    if @battle.FE == :WASTELAND && ((@move == :GUNKSHOT) || (@move == :SLUDGEBOMB) ||
     ...
     (!(opponent.ability == :IMMUNITY) && !(opponent.moldbroken))
      ...
      if [:BACKALLEY,:CITY].include?(@battle.FE) && @move == :SMOG
        opponent.pbPoison(attacker,true)
        if attacker.ability == :POISONPUPPETEER && opponent.pbCanConfuse?
        	opponent.effects[:Confusion]=2+@battle.pbRandom(4)
        	@battle.pbCommonAnimation("Confusion",opponent,nil)
        	@battle.pbDisplay(_INTL("{1} was badly poisoned and confused!",opponent.pbThis))
        else
        	@battle.pbDisplay(_INTL("{1} was badly poisoned!",opponent.pbThis))
        end
      elsif attacker.ability == :VENOMOUS
...
################################################################################
# Badly poisons the target. (Poison Fang / Toxic / Malignant Chain)
################################################################################
...
# / Lick / Stun Spore / Body Slam / Glare / Wildbolt Storm / Volt Tackle / Combat Torque)
################################################################################
...
# / Will-O-Wisp / Scald / Steam Eruption / Sandsear Storm / Flare Blitz / Blazing Torque)
################################################################################
...
class PokeBattle_Move_00C < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanFrostbite?(false)
      opponent.pbFrostbite(attacker)
      @battle.pbDisplay(_INTL("{1} was frostbitten!",opponent.pbThis))
      return true
    end
...
class PokeBattle_Move_011 < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_013 < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && [:SUPERSONIC,:CHATTER].include?(@move)
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_019 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    for i in @battle.battlers
      ...
      case i.status
        ...
        when :FROSTBITE
          @battle.pbDisplay(_INTL("{1}  was cured of its frostbite.",i.pbThis))
        when :PETRFIED
    ...
    for i in 0...party.length
      ...
      case party[i].status
        ...
        when :FROSTBITE
          @battle.pbDisplay(_INTL("{1}  was cured of its frostbite.",party[i].name))
        when :PETRFIED
  ...
  def pbAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && @move == :HEALBELL
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_01B < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.status.nil? ||
      ...
      (attacker.status== :FROSTBITE && !opponent.pbCanFrostbite?(false)) ||
    ...
    case attacker.status
      ...
      when :FROSTBITE
        ...
        @battle.pbDisplay(_INTL("{1} was frostbitten!",opponent.pbThis))
        opponent.pbAbilityCureCheck
        @battle.synchronize=[-1,-1,0] if opponent.status!=:FROSTBITE
        attacker.status=nil
...
class PokeBattle_Move_01C < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && @move == :HOWL
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_020 < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && @move == :TORCHSONG
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_023 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    return super(attacker, opponent, hitnum, alltargets, showanimation) if @basedamage > 0
     # Gen 9 Mod - Now fails if focus energy is 1 (required so it doesn't stack with Dragon Cheer)
    ...
    for i in 0...4
      ...
      if @battle.battlers[i].item == :MIRRORHERB
        @battle.battlers[i].effects[:FocusEnergy] = attacker.effects[:FocusEnergy]
        @battle.pbDisplay(_INTL("{1} used its {2} to mirror its opponent's stat boosts!", @battle.battlers[i].pbThis, getItemName(@battle.battlers[i].item)))
      end
    end
...
class PokeBattle_Move_028 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if @battle.pbWeather == :SHADOWSKY
      @battle.pbDisplay(_INTL("The Shadow Sky stunted {1}'s growth!", attacker.pbThis))
      return -1
    end
    pbShowAnimation(@move, attacker, nil, hitnum, alltargets, showanimation)
...
class PokeBattle_Move_030 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    return super(attacker, opponent, hitnum, alltargets, showanimation) if @basedamage > 0
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::SPEED, true) && !(@move == :ROCKPOLISH && @battle.FE == :CRYSTALCAVERN && (attacker.pbCanIncreaseStatStage?(PBStats::ATTACK, true) || attacker.pbCanIncreaseStatStage?(PBStats::SPATK, true)))
...
class PokeBattle_Move_042 < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && @move == :GROWL
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_045 < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && @move == :SNARL
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_046 < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && @move == :BUGBUZZ
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_047 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if ([:BURNING, :CORROSIVEMIST, :VOLCANIC, :VOLCANICTOP, :BACKALLEY, :CITY].include?(@battle.FE) && @move == :SMOKESCREEN) ||
      ...
      opponent.pbReduceStat(PBStats::ACCURACY, 2, abilitymessage: false, statdropper: attacker)
      opponent.pbReduceStat(PBStats::SPDEF, 1, abilitymessage: false, statdropper: attacker)
    elsif @battle.pbWeather == :SHADOWSKY && @move == :FLASH
      opponent.pbReduceStat(PBStats::ACCURACY, 2, abilitymessage: false, statdropper: attacker)
    else
...
class PokeBattle_Move_049 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if attacker.pbOwnSide.effects[:FoundryStealthRock]
      attacker.pbOwnSide.effects[:FoundryStealthRock] = false
      if @battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("The pointed stones disappeared from around your opponent's team!"))
      else
        @battle.pbDisplay(_INTL("The pointed stones disappeared from around your team!"))
    ...
    if attacker.pbOpposingSide.effects[:FoundryStealthRock]
      attacker.pbOpposingSide.effects[:FoundryStealthRock] = false
      if !@battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("The pointed stones disappeared from around your opponent's team!"))
      else
        @battle.pbDisplay(_INTL("The pointed stones disappeared from around your team!"))
  ...
  def pbAdditionalEffect(attacker, opponent)
    ...
    opponent.pbOwnSide.effects[:FoundryStealthRock] = false
...
class PokeBattle_Move_04A < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    # Gen 9 Mod - Clear Amulet prevents stat losses.
    if opponent.hasWorkingItem(:CLEARAMULET)
      @battle.pbDisplay(_INTL("{1}'s {2} prevents stat loss!", opponent.pbThis, getItemName(opponent.item)))
      return -1
    end
    pbShowAnimation(@move, attacker, opponent, hitnum, alltargets, showanimation)
    ret = -1
...
class PokeBattle_Move_04F < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && @move == :METALSOUND
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_05B < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.pbOwnSide.effects[:Tailwind]>0
      ...
      if attacker.ability == :WINDRIDER && attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false)
...
class PokeBattle_Move_061 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if (opponent.ability == :MULTITYPE) ||
      ...
      # Gen 9 Mod - Can't change type of Mega (Terastalized) Ogerpon and Terapagos.
      (opponent.species == :OGERPON && opponent.form >= 8) || (opponent.species == :TERAPAGOS && @opponent.form == 2)
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@move,attacker,opponent,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_063 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if PBStuff::FIXEDABILITIES.include?(opponent.ability) || opponent.crested == :AVALUGG
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    # Gen 9 Mod - Added Ability Shield
    if opponent.hasWorkingItem(:ABILITYSHIELD)
      @battle.pbDisplay(_INTL("{1}'s Ability is protected by its {2}!", opponent.pbThis, getItemName(opponent.item)))
      return -1
    end
    pbShowAnimation(@move, attacker, opponent, hitnum, alltargets, showanimation)
...
class PokeBattle_Move_065 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if opponent.ability == 0 || attacker.ability == opponent.ability ||
       ...
       PBStuff::FIXEDABILITIES.include?(attacker.ability) || opponent.crested == :AVALUGG || attacker.crested == :AVALUGG
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    # Gen 9 Mod - Added Ability Shield
    if attacker.hasWorkingItem(:ABILITYSHIELD)
      @battle.pbDisplay(_INTL("{1}'s Ability is protected by its {2}!", attacker.pbThis, getItemName(attacker.item)))
      return -1
...
class PokeBattle_Move_066 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if attacker.ability == 0 || attacker.ability == opponent.ability ||
       ...
       opponent.ability == :TRUANT || opponent.crested == :AVALUGG || attacker.crested == :AVALUGG ||
...
class PokeBattle_Move_067 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    # Gen 9 Mod - Added exception for Poison Puppeteer
    # Resurgence - Delta Avalugg Crest preventing Ability Changing/Swapping/Copying
    if (attacker.ability.nil? && opponent.ability.nil?) ||
       (PBStuff::FIXEDABILITIES - [:ZENMODE] + [:ILLUSION] + [:POISONPUPPETEER]).include?(attacker.ability) ||
       (PBStuff::FIXEDABILITIES - [:ZENMODE] + [:ILLUSION] + [:POISONPUPPETEER]).include?(opponent.ability) ||
       attacker.crested == :AVALUGG || opponent.crested == :AVALUGG
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    # Gen 9 Mod - Added Ability Shield
...
class PokeBattle_Move_069 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.effects[:Transform] ||
       ...
       # Gen 9 Mod - Can't transform into a Tatsugiri in Dondozo's mouth
       opponent.effects[:Commander] ||
       (opponent.species == :OGERPON && (opponent.form == 8 ||opponent.form == 9 || opponent.form == 10 || opponent.form == 11)) || # Gen 9 Mod - Can't transform into Mega (Terastalized) Ogerpon.
       (opponent.species == :TERAPAGOS && (opponent.form == 0 ||opponent.form == 2)) # Gen 9 Mod - Can't transform into Mega (Terastalized) Terapagos, nor form 0.
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    battler = opponent
    if battler.pokemon.formCheck(:ExcludeDex)
      battler = @battle.ai.pbMakeFakeBattler(opponent.pokemon.clone, opponent.pokemonIndex)
      battler.form = 0
    ...
    for i in 0...4
      ...
      # Gen 9 Mod - Added Restriction to Transform for Revival Blessing
      ppamount = 5
      ppamount = 1 if opponent.moves[i].move == :REVIVALBLESSING
      attacker.moves[i].pp = ppamount
      attacker.moves[i].totalpp = ppamount
    end
    ...
    # Gen 9 Mod - Hit count for Rage Fist should be copied
    @battle.copyBattlerHit(attacker, opponent)
...
################################################################################
# Power is doubled if the target is asleep.  Wakes the target up. (Wake-up Slap / Wake-up Shock)
################################################################################
...
class PokeBattle_Move_07E < PokeBattle_Move
  def pbBaseDamageMultiplier(damagemult,attacker,opponent)
    damagemult*=2.0 if attacker.status== :POISON || attacker.status== :BURN || attacker.status== :PARALYSIS || attacker.status== :FROSTBITE
    return damagemult
  end
end

################################################################################
# Power is doubled if the target has a status problem. (Hex / Irritation)
################################################################################
...
class PokeBattle_Move_087 < PokeBattle_Move
  ...
  def pbType(attacker,type=@type)
    ...
    if !attacker.hasWorkingItem(:UTILITYUMBRELLA)
      ...
      type=((:DARK)  || type) if weather== :SHADOWSKY
...
class PokeBattle_Move_08C < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    if @move == :HARDPRESS
      return [(100*(opponent.hp.to_f)/opponent.totalhp).floor,1].max
    else
      return [attacker.happiness,250].min if attacker.crested == :LUVDISC
...
def pbHiddenPower(user)
  ...
  for i in $cache.types.keys
    types.push(i) if i != :NORMAL && i != :QMARKS && i != :SHADOW && i != :STELLAR && i != :CRYSTAL
  end
...
class PokeBattle_Move_0A5 < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && @move == :DISARMINGVOICE
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_0AD < PokeBattle_Move
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return if !showanimation
    if id == :ACHILLESHEEL
      @battle.pbAnimation(:SMARTSTRIKE,attacker,opponent,hitnum)
    else
      @battle.pbAnimation(id,attacker,opponent,hitnum)
    end
  end
...
class PokeBattle_Move_0BB < PokeBattle_Move
  ...
  def pbSecondAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && @move == :PSYCHICNOISE
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
################################################################################
# Hits twice. (Gear Grind / Double Hit / Double Kick / Dual Chop / Bonemerang / Twin Beam)
################################################################################
class PokeBattle_Move_0BD < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    if attacker.crested == :DUDUNSPARCE
      defenseDamageCategory(attacker, opponent)
    end
...
class PokeBattle_Move_0C2 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    # Resurgence - Delta Golem's Crest allowing for Recharge Moves to be used without needing a Recharge Turn
    if opponent.damagestate.calcdamage>0 && !(@battle.FE == :STARLIGHT && @move ==:METEORASSAULT) &&
      !(attacker.crested == :GOLEM || (attacker.crested == :CLAYDOL && [:HYPERBEAM,:PRISMATICLASER,:ETERNABEAM].include?(@move)))
...
class PokeBattle_Move_0C4 < PokeBattle_Move
  ...
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if (@battle.FE == :DARKCRYSTALCAVERN && @battle.pbWeather != :SUNNYDAY) || @battle.pbWeather == :SHADOWSKY
...
class PokeBattle_Move_0CD < PokeBattle_Move
  def pbTwoTurnAttack(attacker, checking = false)
    @immediate = false
    if attacker.effects[:TwoTurnAttack] == 0
      @immediate = true if @battle.FE == :HAUNTED || @battle.ProgressiveFieldCheck(PBFields::DARKNESS, 2, 3) || @battle.pbWeather == :SHADOWSKY
    end
    if !@immediate && attacker.hasWorkingItem(:POWERHERB)
      @immediate = true
      if !checking
...
class PokeBattle_Move_0D1 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if opponent.damagestate.calcdamage > 0 && attacker.effects[:Uproar] == 0 && attacker.status != :SLEEP # TODO: Not likely what actually happens, but good enough
      attacker.effects[:Uproar] = 3
        @battle.pbDisplay(_INTL("{1} caused an uproar!", attacker.pbThis))
        for i in 0...4
          if !@battle.battlers[i].isFainted? && @battle.battlers[i].status == :SLEEP
            @battle.battlers[i].effects[:Nightmare] = false
            @battle.battlers[i].status = nil
            @battle.battlers[i].statusCount = 0
            @battle.pbDisplay(_INTL("{1} woke up in the uproar!", @battle.battlers[i].pbThis))
          end
        end
        attacker.currentMove = @move
      elsif pbTypeModifier(@type, attacker, opponent) == 0
        # Cancel effect if attack is ineffective
        attacker.effects[:Uproar] = 0
        @battle.pbDisplay(_INTL("{1} calmed down.", attacker.pbThis))
      end
    return ret
  end
...
class PokeBattle_Move_0D7 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.effects[:Wish]>0 || attacker.effects[:Wish2]>0
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@move,attacker,nil,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_0D8 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if ([:DARKCRYSTALCAVERN,:STARLIGHT,:NEWWORLD,:BEWITCHED].include?(@battle.FE) && (@move == :MOONLIGHT)) || (Rejuv && @battle.FE == :GRASSY && @move == :SYNTHESIS)
    ...
    elsif (@battle.pbWeather== :SHADOWSKY && @move != :MOONLIGHT && (@battle.FE != :DARKCRYSTALCAVERN && @battle.FE != :DARKNESS2 && @battle.FE != :DARKNESS3))
      hpgain=(attacker.totalhp/3.0).floor
      ...
      elsif (@battle.pbWeather== :SHADOWSKY && @move == :MOONLIGHT && !attacker.hasWorkingItem(:UTILITYUMBRELLA))
        hpgain=(attacker.totalhp*2/3.0).floor
...
class PokeBattle_Move_0E5 < PokeBattle_AllTargetMove
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    failed = true
    for i in 0...4
      # Gen 9 Mod - Good as Gold prevents Perish Song on this mon.
      if @battle.battlers[i].effects[:PerishSong] == 0 &&
         (@battle.battlers[i].ability != :SOUNDPROOF || @battle.battlers[i].ability != :GOODASGOLD || @battle.battlers[i].moldbroken) &&
...
class PokeBattle_Move_0EB < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && @move == :ROAR
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_0F7 < PokeBattle_Move
  ...
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if opponent.damagestate.calcdamage > 0 && !opponent.damagestate.substitute &&
      ...
      elsif attacker.hasWorkingItem(:FROSTORB)
        if opponent.pbCanFrostbite?(false)
          opponent.pbFrostbite(attacker)
          @battle.pbDisplay(_INTL("{1} was frostbitten!", opponent.pbThis))
        end
...
class PokeBattle_Move_102 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    # Gen 9 Mod - Hail/Snow/Both option text change
    @battle.pbDisplay(_INTL("It started to " + HAILSNOWLOWMOD + "!"))
    @battle.pbDisplay(_INTL("The weather blocked out the starry sky!")) if @battle.FE == :STARLIGHT
...
class PokeBattle_Move_105 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    pbShowAnimation(@move,attacker,opponent,hitnum,alltargets,showanimation)
    if @battle.FE == :CLOUDS
      if attacker.ability == :FOUNDRY
        attacker.pbOwnSide.effects[:FoundryStealthRock]=true
        @battle.pbDisplay(_INTL("... but the clouds bounced the Stealth Rocks back!"))
        if @battle.pbIsOpposing?(attacker.index)
          @battle.pbDisplay(_INTL("Molten stones float in the air around your foe's team!"))
        else
          @battle.pbDisplay(_INTL("Molten stones float in the air around your team!"))
        end
      else
    ...
    if attacker.ability == :FOUNDRY
      attacker.pbOpposingSide.effects[:FoundryStealthRock]=true
      if !@battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("Molten stones float in the air around your foe's team!"))
      else
        @battle.pbDisplay(_INTL("Molten stones float in the air around your team!"))
...
################################################################################
# If attack misses, user takes crash damage of 1/2 of max HP. (High Jump Kick /
# Jump Kick / Supercell Slam)
################################################################################
...
################################################################################
# Removes trapping moves, entry hazards and Leech Seed on user/user's side. (Rapid Spin, Mortal Spin)
################################################################################
class PokeBattle_Move_110 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if !attacker.isFainted? && opponent.damagestate.calcdamage > 0
      ...
      if attacker.pbOwnSide.effects[:StealthRock] || attacker.pbOwnSide.effects[:FoundryStealthRock]
        attacker.pbOwnSide.effects[:StealthRock] = false
        attacker.pbOwnSide.effects[:FoundryStealthRock] = false
      ...
      attacker.pbIncreaseStat(PBStats::SPEED, 1, statsource: attacker) if Gen > 7 && move == :RAPIDSPIN
    end
    return ret
  end

  def pbAdditionalEffect(attacker,opponent)
    return false if !opponent.pbCanPoison?(false) || !@move == :MORTALSPIN
    if attacker.ability == :VENOMOUS
      opponent.pbPoison(attacker,true)
      @battle.pbDisplay(_INTL("{1} was badly poisoned!",opponent.pbThis))
    else
...
class PokeBattle_Move_111 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    if opponent.effects[:FutureSight] > 0 || opponent.effects[:FutureSight2] > 0
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@move, attacker, nil, hitnum, alltargets, showanimation)
    ...
    if attacker.ability == :PERIODICORBIT
      opponent.effects[:FutureSight2]=6
    end
...
class PokeBattle_Move_138 < PokeBattle_Move
  ...
  def pbAdditionalEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    if attacker.ability == :AMPLIFIER && attacker.effects[:Uproar] == 0 && @move == :NOBLEROAR
      attacker.effects[:Amplifier] = 1
      @battle.pbDisplay(_INTL("{1} amplified the sound for all to hear!", attacker.pbThis))
    end
    return ret
...
class PokeBattle_Move_13D < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.hp>0 && @battle.pbCanChooseNonActive?(attacker.index) && !@battle.pbAllFainted?(@battle.pbParty(attacker.index)) && @battle.FE != :COLOSSEUM
      @battle.pbDisplay(_INTL("{1} went back to {2}!",attacker.pbThis,@battle.pbGetOwner(attacker.index).name))
      #Going to switch, check for pursuit
      newpoke=0
      newpoke=@battle.pbSwitchInBetween(attacker.index,true,false)
...
class PokeBattle_Move_13E < PokeBattle_Move
  ...
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ...
    if attacker.effects[:TwoTurnAttack] == 0
      ...
      for stat in [PBStats::SPATK, PBStats::SPDEF, PBStats::SPEED]
        attacker.pbIncreaseStat(stat, boost_amount, statsource: attacker)
      end
    end
    return 0 if attacker.effects[:TwoTurnAttack] != 0
...
class PokeBattle_Move_146 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    if [:PLUS, :MINUS].include?(attacker.pbPartner.ability) || (Rejuv && @battle.FE == :ELECTERRAIN)
      partnerfail = false
      if attacker.pbPartner.pbCanIncreaseStatStage?(PBStats::DEFENSE, false) || attacker.pbPartner.pbCanIncreaseStatStage?(PBStats::SPDEF, false)
        pbShowAnimation(@move, attacker, nil, hitnum, alltargets, showanimation)
        statboost = 1
        statboost = 2 if @battle.FE == :DEEPEARTH || (Rejuv && @battle.FE == :ELECTERRAIN && [:PLUS, :MINUS].include?(attacker.pbPartner.ability))
    ...
    if [:PLUS, :MINUS].include?(attacker.ability) || (Rejuv && @battle.FE == :ELECTERRAIN)
      if attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE, false) || attacker.pbCanIncreaseStatStage?(PBStats::SPDEF, false)
        ...
        statboost = 1
        statboost = 2 if @battle.FE == :DEEPEARTH || (Rejuv && @battle.FE == :ELECTERRAIN && [:PLUS, :MINUS].include?(attacker.ability))
...
class PokeBattle_Move_163 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    if [:PLUS, :MINUS].include?(attacker.pbPartner.ability)
      partnerfail = false
    ...
    if [:PLUS, :MINUS].include?(attacker.ability)
...
class PokeBattle_Move_16D < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    ret = super(attacker, opponent, hitnum, alltargets, showanimation)
    # Gen 9 Mod - Added Covert Cloak (and missing Shield Dust)
    if opponent.damagestate.calcdamage > 0 && opponent.status == :BURN && opponent.ability != :SHIELDDUST && !opponent.hasWorkingItem(:COVERTCLOAK)
      opponent.pbCureStatus
    end
    return ret
  end
...
class PokeBattle_Move_176 < PokeBattle_Move
  ...
  def pbIsPhysical?(type = @type)
    ...
    if attacker.class == PokeBattle_Battler
      ...
      if attacker.ability == :WINTERJOY
        if @battle.pbWeather == :HAIL || [:ICY, :SNOWYMOUNTAIN, :FROZENDIMENSION].include?(@battle.FE)
          atkmult *= 1.4
        elsif (@battle.pbWeather == :SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || @battle.FE == :DESERT
          atkmult *= 0.7
        end
      end
    end
    ...
    if attacker.class == PokeBattle_Battler
      ...
      # Resurgence - Athenian and Winter Joy
      spatkmult *= 1.5 if attacker.ability == :FORTITUDE && !attacker.status.nil?
      spatkmult *= 2 if attacker.ability == :ATHENIAN
      if attacker.ability == :WINTERJOY
        if @battle.pbWeather == :HAIL || [:ICY, :SNOWYMOUNTAIN, :FROZENDIMENSION].include?(@battle.FE)
          spatkmult *= 1.4
        elsif (@battle.pbWeather == :SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || @battle.FE == :DESERT
      ...
      spatkmult *= 0.5 if attacker.status == :FROSTBITE && !(attacker.ability == :FORTITUDE && !attacker.status.nil?)
    end
...
################################################################################
# Power increases on Electric Terrain (Rising Voltage / Psyblade)
################################################################################
...
class PokeBattle_Move_903 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    @battle.pbDisplay(_INTL("{1} is preparing to tell a chillingly bad joke!",attacker.pbThis))

    if @battle.weather== :HAIL
      @battle.pbDisplay(_INTL("But it failed!"))
    elsif @battle.state.effects[:HeavyRain]
      @battle.pbDisplay(_INTL("There's no relief from this heavy rain!"))
    elsif @battle.state.effects[:HarshSunlight]
      @battle.pbDisplay(_INTL("The extremely harsh sunlight was not lessened at all!"))
    elsif @battle.weather== :STRONGWINDS && (@battle.pbCheckGlobalAbility(:DELTASTREAM))
...
class PokeBattle_Move_905 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    # Resurgence - Delta Avalugg Crest preventing Ability Changing/Swapping/Copying
    if opponent.ability==0 || PBStuff::ABILITYBLACKLIST.include?(opponent.ability) || PBStuff::FIXEDABILITIES.include?(attacker.ability) || PBStuff::FIXEDABILITIES.include?(attacker.pbPartner.ability) || opponent.crested == :AVALUGG || attacker.crested == :AVALUGG || attacker.pbPartner.crested == :AVALUGG
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@move,attacker,opponent,hitnum,alltargets,showanimation)
    ...
    if attacker.pbPartner && !attacker.pbPartner.isFainted?
      if attacker.ability == opponent.ability && attacker.pbPartner.ability == opponent.ability
        @battle.pbDisplay(_INTL("But it failed!"))
        return -1
      end
      ...
      if !attacker.pbPartner.hasWorkingItem(:ABILITYSHIELD)
        if attacker.pbPartner.ability != opponent.ability
          attacker.pbPartner.ability = opponent.ability
          attacker.pbPartner.pbAbilitiesOnSwitchIn(true)
        end
      else
        @battle.pbDisplay(_INTL("{1}'s Ability is protected by its {2}!", attacker.pbPartner.pbThis, getItemName(attacker.pbPartner.item)))
      end
    else
      if attacker.ability == opponent.ability
        @battle.pbDisplay(_INTL("But it failed!"))
        return -1
      end
      @battle.pbDisplay(_INTL("{1} copied {2}'s {3}!",attacker.pbThis,opponent.pbThis(true),abilityname))
...
class PokeBattle_Move_906 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !attacker.hasType?(:ELECTRIC) || attacker.effects[:DoubleShock]
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
...
class PokeBattle_Move_908 < PokeBattle_Move
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if @immediate || attacker.effects[:TwoTurnAttack]!=0
      @battle.pbCommonAnimation("Electro Shot charging",attacker,nil) #Charging animation
      showanim=true
...
class PokeBattle_Move_909 < PokeBattle_Move
  @randomchance=0
  def pbBaseDamage(basedmg,attacker,opponent)
    @randomchance=@battle.pbRandom(10)
    if @randomchance<7
      dmg = basedmg
    else
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return if !showanimation
    if @randomchance < 7
      @battle.pbAnimation(:FICKLEBEAM,attacker,opponent,hitnum)
    else
      @battle.pbAnimation(:FICKLEBEAMMAX,attacker,opponent,hitnum)
    end
  end
end
...
class PokeBattle_Move_90B < PokeBattle_Move
  ...
  # Always critting code handled in superclass, do not edit!.
end

################################################################################
# (Glaive Rush)
################################################################################
class PokeBattle_Move_90C < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.damagestate.calcdamage>0
      attacker.effects[:GlaiveRush] = true
      @battle.pbDisplay(_INTL("{1} is vulnerable to attacks!",attacker.pbThis))
    end
    return ret
  end
end
...
################################################################################
# Type depends on the user's form. (Ivy Cudgel)
################################################################################
class PokeBattle_Move_90F < PokeBattle_Move
  def pbType(attacker,type=@type)
    if attacker.species == :OGERPON
      type = :WATER if (attacker.form == 5 || attacker.form == 9)
      type = :FIRE if (attacker.form == 6 || attacker.form == 10)
      type = :ROCK if (attacker.form == 7 || attacker.form == 11)
    end
    return super(attacker,type)
  end
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return if !showanimation
    if attacker.species==:OGERPON
      if (attacker.form == 5 || attacker.form == 9)
        @battle.pbAnimation(:IVYCUDGELWATER,attacker,opponent,hitnum) # Water type
      elsif (attacker.form == 6 || attacker.form == 10)
        @battle.pbAnimation(:IVYCUDGELFIRE,attacker,opponent,hitnum) # Fire type
      elsif (attacker.form == 7 || attacker.form == 11)
        @battle.pbAnimation(:IVYCUDGELROCK,attacker,opponent,hitnum) # Rock type
      end
    else
      @battle.pbAnimation(:IVYCUDGEL,attacker,opponent,hitnum) # Grass type
    end
  end
end
...
class PokeBattle_Move_910 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    return basedmg + (attacker.pbOwnSide.effects[:LastRespects] * 50)
  end
end
...
# (Make It Rain)
################################################################################
# Gen 9 Mod - Added commented code to increase extra money on Big Top and
# Dragon's Den fields To match with Pay Day.
class PokeBattle_Move_911 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.damagestate.calcdamage>0
      if @battle.pbOwnedByPlayer?(attacker.index)
      ...
      if attacker.pbCanReduceStatStage?(PBStats::SPATK,false,true)
        attacker.pbReduceStat(PBStats::SPATK,1,abilitymessage:false, statdropper: attacker)
      end
    end
    return ret
  end
...
class PokeBattle_Move_913 < PokeBattle_Move
  def pbAdditionalEffect(attacker,opponent)
    return false if !(attacker.effects[:Commandee] && attacker.pbPartner.effects[:Commander])
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return if !showanimation
    if attacker.species==:DONDOZO && (attacker.effects[:Commandee] && attacker.pbPartner.effects[:Commander])
      if (attacker.pbPartner.form == 0)
        @battle.pbAnimation(:ORDERUPCURLY,attacker,opponent,hitnum)
      elsif (attacker.pbPartner.form == 1)
        @battle.pbAnimation(:ORDERUPDROOPY,attacker,opponent,hitnum)
      elsif (attacker.pbPartner.form == 2)
        @battle.pbAnimation(:ORDERUPSTRETCHY,attacker,opponent,hitnum)
      else
        @battle.pbAnimation(:ORDERUP,attacker,opponent,hitnum)
      end
    else
      @battle.pbAnimation(:ORDERUP,attacker,opponent,hitnum)
    end
...
class PokeBattle_Move_914 < PokeBattle_Move
  ...
  def pbNumHits(attacker)
    if attacker.hasWorkingItem(:LOADEDDICE)
      hitchances=[4,5,6,7,8,9,10]
      ret=hitchances[@battle.pbRandom(hitchances.length)]
      return ret
    else
      return 10
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if hitnum == 0
      ...
      @battle.pbAnimation(:POPULATIONBOMBNEXTHIT, attacker,opponent,hitnum)
    end
  end
end
...
class PokeBattle_Move_915 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    ...
    rageFistCounter = 6 if rageFistCounter > 6
    dmg = basedmg + (50*rageFistCounter)
    # Gen 9 Mod - Track Rage Fist use for evolution
    if attacker.species == :PRIMEAPE && @move == :RAGEFIST
      attacker.pokemon.usedMoves=0 if !attacker.pokemon.usedMoves
      attacker.pokemon.usedMoves +=1
    end
    return dmg
  end
end
...
################################################################################
# Type depends on the user's. Removes Reflect and similar. (Raging Bull)
################################################################################
class PokeBattle_Move_916 < PokeBattle_Move
  def pbType(attacker,type=@type)
    if attacker.species == :TAUROS
      type = :FIGHTING if attacker.form == 1
      type = :FIRE if attacker.form == 2
      type = :WATER if attacker.form == 3
    else
      type == :NORMAL
    end
    return super(attacker,type)
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.species==:TAUROS
      if (attacker.form == 1)
        @battle.pbAnimation(:RAGINGBULLFIGHTING,attacker,opponent,hitnum) # Fighting type
      elsif (attacker.form == 2)
        @battle.pbAnimation(:RAGINGBULLFIRE,attacker,opponent,hitnum) # Fire type
      elsif (attacker.form == 3)
        @battle.pbAnimation(:RAGINGBULLWATER,attacker,opponent,hitnum) # Water type
      end
    else
      @battle.pbAnimation(:RAGINGBULL,attacker,opponent,hitnum) # Normal type
    end
  end
end
...
class PokeBattle_Move_917 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    if @battle.pbOwnedByPlayer?(attacker.index) && $game_switches[:Nuzlocke_Mode] && !@battle.isOnline?
      @battle.pbDisplay(_INTL("It doesn't work."))
      return 0
    end
    ...
    if !canRevive || attacker.isbossmon || attacker.issossmon || @battle.pbGetOwner(attacker.index).nil?
      @battle.pbDisplay(_INTL("Doesn't have any effect."))
      return 0
    end
    pbShowAnimation(@move, attacker, nil, hitnum, alltargets, showanimation)
    party = @battle.pbParty(attacker.index)
    revivepoke = party[@battle.pbRevive(attacker.index)]
...
class PokeBattle_Move_919 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    attacker.effects[:UsingSubstituteRightNow]=true
    @battle.scene.pbAnimation(:SUBSTITUTE,attacker,nil)
    attacker.effects[:UsingSubstituteRightNow]=false
    attacker.effects[:MultiTurn]=0
    attacker.effects[:MultiTurnAttack]=0
    ...
    if pbIsBerry?(attacker.item)
      attacker.pbUseBerry(attacker.item,false)
    end
...
# (Silk Trap)
################################################################################
class PokeBattle_Move_91A < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !PBStuff::RATESHARERS.include?(attacker.previousMove)
      attacker.effects[:ProtectRate]=0
    end
...
# (Spicy Extract)
################################################################################
class PokeBattle_Move_91C < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
...
# (Spin Out)
################################################################################
class PokeBattle_Move_91D < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.damagestate.calcdamage>0
...
# (Syrup Bomb)
################################################################################
class PokeBattle_Move_91E < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    # Gen 9 Mod - Added Covert Cloak
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.pokemon.isShiny?
      @battle.pbAnimation(:SYRUPBOMBSHINY, attacker,opponent,hitnum)
    else
      @battle.pbAnimation(id, attacker, opponent, hitnum)
    end
  end
...
# (Tachyon Cutter)
################################################################################
class PokeBattle_Move_91F < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    if attacker.crested == :DUDUNSPARCE
      defenseDamageCategory(attacker, opponent)
    end
...
# (Tera Starstorm)
################################################################################
class PokeBattle_Move_920 < PokeBattle_Move
  def pbType(attacker,type=@type)
...
# (Tidy Up)
################################################################################
class PokeBattle_Move_921 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    tidy = false
    if attacker.pbOwnSide.effects[:StealthRock] || attacker.pbOpposingSide.effects[:StealthRock] || attacker.pbOwnSide.effects[:FoundryStealthRock] || attacker.pbOpposingSide.effects[:FoundryStealthRock]
      attacker.pbOwnSide.effects[:StealthRock]=false
      attacker.pbOpposingSide.effects[:StealthRock]=false
      attacker.pbOwnSide.effects[:FoundryStealthRock]=false
      attacker.pbOpposingSide.effects[:FoundryStealthRock]=false
      @battle.pbDisplay(_INTL("{1} cleared away any stealth rocks!",attacker.pbThis))
      tidy = true
    end
    if attacker.pbOwnSide.effects[:Spikes]>0 || attacker.pbOpposingSide.effects[:Spikes]>0
      attacker.pbOwnSide.effects[:Spikes]=0
      attacker.pbOpposingSide.effects[:Spikes]=0
      @battle.pbDisplay(_INTL("{1} cleared away any spikes!",attacker.pbThis))
      tidy = true
    end
    if attacker.pbOwnSide.effects[:ToxicSpikes]>0 || attacker.pbOpposingSide.effects[:ToxicSpikes]>0
      attacker.pbOwnSide.effects[:ToxicSpikes]=0
      attacker.pbOpposingSide.effects[:ToxicSpikes]=0
      @battle.pbDisplay(_INTL("{1} cleared away any poison spikes!",attacker.pbThis))
      tidy = true
    end
    if attacker.pbOwnSide.effects[:StickyWeb] || attacker.pbOpposingSide.effects[:StickyWeb]
      attacker.pbOwnSide.effects[:StickyWeb]=false
      attacker.pbOpposingSide.effects[:StickyWeb]=false
      @battle.pbDisplay(_INTL("{1} cleared away any of the sticky webbing!",attacker.pbThis))
      tidy = true
    end
    subs = false
    for i in 0...4
      if @battle.battlers[i].effects[:Substitute] > 0
        @battle.battlers[i].effects[:Substitute] = 0
        @battle.scene.pbUnSubstituteSprite(@battle.battlers[i],@battle.battlers[i].pbIsOpposing?(1))
        subs = true; tidy = true
      end
    end
    @battle.pbDisplay(_INTL("All substitutes were cleared up!")) if subs
...
# (Triple Dive)
################################################################################
class PokeBattle_Move_922 < PokeBattle_Move
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    if attacker.crested == :DUDUNSPARCE
      defenseDamageCategory(attacker, opponent)
    end
...
# (Upper Hand)
################################################################################
class PokeBattle_Move_923 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    oppmoveid = opponent.selectedMove
    oppmovedata = $cache.moves[oppmoveid]
    if opponent.hasMovedThisRound?
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    if oppmovedata.basedamage == 0
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    if !oppmovedata.priority
      pri = 0
    else
...
################################################################################
# Target becomes Dragon type. (Dragonify)
################################################################################
class PokeBattle_Move_951 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if opponent.effects[:Substitute]>0
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    ...
    opponent.type2=nil
    typename=getTypeName((:DRAGON))
    @battle.pbDisplay(_INTL("{1} transformed into the {2} type!",opponent.pbThis,typename))
    return 0
  end

  def pbAdditionalEffect(attacker, opponent)
    if @battle.FE == :FAIRYTALE && @move == :DRAGONIFY
      opponent.pbReduceStat(PBStats::SPATK, 1, abilitymessage: false)
    end
  end
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return if !showanimation
    if id == :DRAGONIFY
      @battle.pbAnimation(:DRAGONRAGE,attacker,opponent,hitnum)
    else
      @battle.pbAnimation(id,attacker,opponent,hitnum)
    end
  end
...
################################################################################
# Increases the priority of the user and its allies' moves by 1 during the next turn. (Jet Stream)
################################################################################
class PokeBattle_Move_952 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.pbOwnSide.effects[:JetStream]>0
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@move,attacker,nil,hitnum,alltargets,showanimation)
    attacker.pbOwnSide.effects[:JetStream]=2
    if !@battle.pbIsOpposing?(attacker.index)
    ...
    if attacker.pbOwnSide.effects[:JetStream]>0
      ...
      if attacker.ability == :WINDPOWER && attacker.effects[:Charge] == false
        attacker.effects[:Charge] = true
        @battle.pbCommonAnimation("Charge",attacker,nil)
        @battle.pbDisplay(_INTL("The Jet Stream charged {1} with power!", attacker.pbThis))
      end
      # Gen 9 Mod - Charge lasts until the user uses an electric move.
      if attacker.pbPartner.ability == :WINDPOWER && attacker.pbPartner.effects[:Charge] == false
        attacker.pbPartner.effects[:Charge] = true
        @battle.pbCommonAnimation("Charge",attacker.pbPartner,nil)
        @battle.pbDisplay(_INTL("The Jet Stream charged {1} with power!", attacker.pbPartner.pbThis))
      end
      if attacker.ability == :WINDRIDER && attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false)
        attacker.pbIncreaseStat(PBStats::ATTACK,1,abilitymessage:false)
      end
...
################################################################################
# Two turn attack.  Skips first turn, attacks second turn. (Lunar Cannon)
# In darkness, takes 1 turn instead.
################################################################################
class PokeBattle_Move_954 < PokeBattle_Move
  def pbTwoTurnAttack(attacker,checking=false)
    @immediate=false
    if attacker.effects[:TwoTurnAttack]==0
...
################################################################################
# Petrifies the target. (Medusa Ray)
################################################################################
class PokeBattle_Move_955 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
  ...
  def pbShowAnimation(id,attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return if !showanimation
    if id == :MEDUSARAY
      @battle.pbAnimation(:GLARE,attacker,opponent,hitnum)
    else
      @battle.pbAnimation(id,attacker,opponent,hitnum)
    end
  end
...
################################################################################
# Starts shadow sky weather. (Shadow Prayer)
################################################################################
class PokeBattle_Move_957 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    @battle.weatherduration=5
    @battle.weatherduration=8 if (attacker.hasWorkingItem(:SHADOWROCK)) || [:DARKCRYSTALCAVERN,:SHORTCIRCUIT,:DIMENSIONAL,:FROZENDIMENSION,:SKY,:DARKNESS1,:DARKNESS2,:DARKNESS3].include?(@battle.FE)

    @battle.pbCommonAnimation("ShadowSky",nil,nil)
    @battle.pbDisplay("The sky faded to pitch-black!")
    @battle.pbDisplay(_INTL("The darkness eclipsed the starry sky!")) if @battle.FE == :STARLIGHT
    return 0
  end
end

################################################################################
# Revert target's Mega form. (Retrograde)
################################################################################
class PokeBattle_Move_959 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if opponent.isMega?
...
# Target is also semi-invulnerable during use, and can't take any action.
################################################################################
class PokeBattle_Move_960 < PokeBattle_Move
  def pbTwoTurnAttack(attacker, checking = false)
    @immediate = false
    if attacker.effects[:TwoTurnAttack] == 0
      @immediate = true if @battle.FE == :HOLY || @battle.FE == :HAUNTED
    end
    return false if @immediate
  ...
  def pbEffect(attacker, opponent, hitnum = 0, alltargets = nil, showanimation = true)
    if opponent.nil?
      @battle.pbDisplay(_INTL("But it failed!"))
      attacker.effects[:TwoTurnAttack] = 0
      attacker.effects[:SpiritTaken] = nil
      return -1
    end
    if opponent.effects[:TwoTurnAttack] > 0
      @battle.pbDisplay(_INTL("But it failed!"))
    ...
    if attacker.effects[:TwoTurnAttack] != 0
      if opponent.effects[:SpiritAway]
        attacker.effects[:TwoTurnAttack] = 0
        attacker.effects[:SpiritTaken] = nil
        @battle.pbDisplay(_INTL("But it failed!"))
        return -1
      end
      @battle.pbCommonAnimation("Spirit Away Charging", attacker, opponent)
      @battle.scene.pbVanishSprite(attacker)
      @battle.scene.pbVanishSprite(opponent)
      @battle.pbDisplay(_INTL("{1} whisked {2} away!", attacker.pbThis, opponent.pbThis))
      @battle.pbClearChoices(opponent.index)
      @battle.choices[attacker.index][3] = opponent.index
      attacker.effects[:SpiritTaken] = opponent
      opponent.effects[:SpiritAway] = true
    end
    return 0 if attacker.effects[:TwoTurnAttack] != 0
    # @battle.scene.pbUnVanishSprite(attacker)
    # @battle.scene.pbUnVanishSprite(opponent)
    ret = super
    @battle.pbDisplay(_INTL("{1} returned from being spirited away!", opponent.pbThis))
    opponent.effects[:SpiritAway] = false
    attacker.effects[:SpiritTaken] = nil
    return ret
  end
end
...
# Regurgitation
################################################################################
class PokeBattle_Move_97F < PokeBattle_Move
  def pbType(attacker,type=@type)
    if attacker.species == :MUK
      type = :WATER   if attacker.form == 3
      type = :GRASS   if attacker.form == 4
      type = :FIRE    if attacker.form == 5
      type = :DARK    if attacker.form == 6
      type = :NORMAL  if attacker.form == 7
      type = :PSYCHIC if attacker.form == 8
    end
    return super(attacker,type)
  end
