class PokeBattle_Battler
  ...
  attr_accessor :partnercustap # Resurgence - Partner receiving Custap effect if partnered with Crested Delta Crustle
  ...
  SwitchEff = [:AquaRing, :BindingBand, :BeakBlast, :BurnUp, :ClangedScales, :Curse,
    ...
    :ShellTrap, :SkyDrop, :SpiritAway, :SmackDown, :Snatch, :Tantrum, :TarShot, :Torment,
  ...
  #other counter vars # Gen 9 Mod - Added Supreme Overlord. # Resurgence - Added Blaze Boost
  CountEff = [:BideDamage, :Damage, :FocusEnergy, :ProtectRate, :Rollout, :EchoedVoice, :SpeedSwap, :Stockpile, :StockpileDef, :StockpileSpDef, :WeightModifier, :SupremeOverlord, :BlazeBoost, :FaintedBoost]
  ...
  PermEff = [:FutureSight, :FutureSight2, :FutureSightMove, :FutureSightUser, :FutureSightPokemonIndex, :HealingWish, :LunarDance, :Wish, :Wish2, :WishAmount, :WishMaker, :ZHeal]

  SpeciesToBBForm = {:CHESNAUGHT => 1, :DELPHOX => 2, :GRENINJA => 1}

  @@battle_bond_flags = []
  @@battle_bond_trainer_flags = []
...
################################################################################
  ...
  def hasCrest?(illusion=false)
    ...
    return true if @battle.pbGetOwnerItems(@index).include?(:SPIRITCREST) && crestmon.species == :SPIRITOMB && !@battle.pbOwnedByPlayer?(@index)
	return true if @battle.pbGetOwnerItems(@index).include?(:CRYCREST) && crestmon.species == :CRYOGONAL && !@battle.pbOwnedByPlayer?(@index)
    return true if @battle.pbGetOwnerItems(@index).include?(:DEGOLCREST) && crestmon.species == :GOLEM && crestmon.form == 3 && !@battle.pbOwnedByPlayer?(@index)
    return true if @battle.pbGetOwnerItems(@index).include?(:DEDUNCREST) && crestmon.species == :DUDUNSPARCE && crestmon.form == 2 && !@battle.pbOwnedByPlayer?(@index)
    return true if @battle.pbGetOwnerItems(@index).include?(:DEDUNCREST) && crestmon.species == :DUDUNSPARCE && crestmon.form == 3 && !@battle.pbOwnedByPlayer?(@index)
    # Resurgence - Making the Delta Crustle Crest only need to be in the bag for it to be activated and exclusive to Delta Crustle (Berry)
    return true if $PokemonBag.pbQuantity(:DECRUSCREST)>0 && crestmon.species == :CRUSTLE && crestmon.form == 1 && @battle.pbOwnedByPlayer?(@index)
    return true if @battle.pbGetOwnerItems(@index).include?(:DECRUSCREST) && crestmon.species == :CRUSTLE && crestmon.form == 1 && !@battle.pbOwnedByPlayer?(@index)
    # Resurgence - Making the Noctowl/Luxray Crests and their Delta counterparts exclusive to their respective Forms
    return true if (crestmon.species == :NOCTOWL && crestmon.form == 0 && crestmon.item == :NOCCREST) || (crestmon.species == :NOCTOWL && crestmon.form == 1 && crestmon.item == :DENOCCREST)
    return true if (crestmon.species == :LUXRAY && crestmon.form == 0 && crestmon.item == :LUXCREST) || (crestmon.species == :LUXRAY && crestmon.form == 1 && crestmon.item == :DELUXCREST)
    ...
    return false if [:TYPHLOSION, :SAMUROTT, :ELECTRODE, :ZOROARK, :VESPIQUEN].include?(crestmon.species) && crestmon.form != 0
    ...
    # Resurgence - Making the Delta Scrafty, Avalugg, Plusle, Minun, Golem, and Dudunsparce Crests exlusive to their respective Forms
    return false if [:SCRAFTY, :AVALUGG, :PLUSLE, :MINUN, :GOLEM, :DUDUNSPARCE].include?(crestmon.species) && !crestmon.isDelta?
...
################################################################################
  def initialize(btl,index,fakebattler=false)
    ...
    @vanished     = false
    @custap       = false
    @partnercustap = false # Resurgence - Partner receiving Custap effect if partnered with Crested Delta Crustle
    @moldbroken   = false
    @corroded     = false
    @takegem      = false
  ...
  def pbInitEffects(batonpass, fakebattler=false)
    ...
    @effects[:Protoabsolution]  = [0,false] # Resurgence - Added Protoabsolution
    @effects[:Protocryophilic]  = [0,false] # Resurgence - Added Protocryophilic
  ...
  def pbUpdate(fullchange=false)
    ...
    # Resurgence - Delta Avalugg Crest calling all of it's Abilities
    @ability = @pokemon.ability if !@ability.nil? && !((@crested == :SILVALLY || @crested == :ZOROARK || @crested == :AVALUGG))
    @type1   = @pokemon.type1
    @type2   = @pokemon.type2
  end
...
# Update Pokémon who will gain EXP if this battler is defeated
  ...
  # Resurgence - Reminder for later to add Crystal-type forms for Arceus and Silvally
  def formFromItem
    if @species == :SILVALLY
      case @item
        ...
        # when :CRYSTALMEMORY   then return 19
        else return 0
      end
    elsif @species == :ARCEUS && self.ability == :MULTITYPE
      case @item
        ...
        # when :TRANSPARENTPLATE, :CRYSTALLIUMZ then arcform = 19
        else return 0
      end
    end
  end
...
# tiny bits of extra prcoessing work when called, for no gain - Falirion
  ...
  def nullsFlying?
    return [:WINDFORCE,:WINDBORNETYRANT].include?(@ability)
  end
  ...
  def pbSpeed()
    ...
    if self.ability.is_a?(PokeAbility) && self.ability.ability!=nil
      if self.crested == :AVALUGG || self.ability.ability.is_a?(Array)
    ...
    case self.ability
      ...
      when :ICECLEATS # Resurgence - Ice Cleats
        speed *= 2 if @battle.pbWeather == :HAIL || @battle.FE == :ICY || @battle.FE == :SNOWYMOUNTAIN || @battle.FE == :FROZENDIMENSION
      when :SLOWSTART
      ...
      # Resurgence - Added Protoabsolution
      when :PROTOABSOLUTION
        speed *= 1.5 if self.effects[:Protoabsolution][0] == PBStats::SPEED
      # Resurgence - Added Protocryophilic
      when :PROTOCRYOPHILIC
        speed *= 1.5 if self.effects[:Protocryophilic][0] == PBStats::SPEED
      # Resurgence - Shadow Dance
      when :SHADOWDANCE
        speed *= 2 if @battle.pbWeather == :SHADOWSKY || [:DARKCRYSTALCAVERN, :DARKNESS1, :DARKNESS2, :DARKNESS3, :SHORTCIRCUIT, :DIMENSIONAL].include?(@battle.FE)
    end
    case @battle.FE
      when :NEWWORLD
...
################################################################################
  ...
  def pbFaint(showMessage=true)
    ...
    if (pbPartner.ability == :POWEROFALCHEMY || pbPartner.ability == :RECEIVER) && !(self.crested == :AVALUGG || self.item == :ABILITYSHIELD) && pbPartner.hp > 0
    ...
    spiritreturn = self.effects[:SpiritTaken]
    if self.effects[:SpiritAway]
      for i in @battle.battlers
        next if i.isFainted?
        if i.effects[:SpiritTaken]==self
          @battle.scene.pbUnVanishSprite(i)
          i.effects[:TwoTurnAttack] = 0
          i.effects[:SpiritTaken] = nil
        end
      end
    end
    ...
    # Resurgence - Return Battle Bonded Pokemon to their normal form if it faints.
    if (@species==:CHESNAUGHT || @species==:GRENINJA) && @pokemon.form==1
      self.form = 0
    end
    if @species==:DELPHOX && @pokemon.form==2
      self.form = 0
    end
    ...
    if spiritreturn!=nil
      oppmon = spiritreturn
      oppmon.effects[:SpiritAway]=false
      @battle.scene.pbUnVanishSprite(oppmon)
      @battle.pbDisplay(_INTL("{1} had its spirit returned to the battlefield!",oppmon.pbThis))
    end
...
################################################################################
  def pbCheckForm(basemove = nil) # TODO: try and find a different way of doing this - I'm not liking this    return if @effects[:Transform]
    ...
    # Resurgence - Delta Muk random form after evolving
    if self.pokemon && self.pokemon.species == :MUK && self.form == 2 && self.ability == :REGURGITATION
      self.form = rand(3..8)
      @battle.pbAnimation("SWALLOW",self,nil)
      pbUpdate(false)
    ...
    # Giratina
    if (self.pokemon && (self.pokemon.species == :KYOGRE || self.pokemon.species == :GROUDON || self.pokemon.species == :REGIGIGAS || self.pokemon.species == :GIRATINA || self.pokemon.species == :ARCEUS)) && !self.isFainted?
      if self.form!=@pokemon.form
        self.form=@pokemon.form
        transformed=true
      end
   ...
   # Genesect
    ...
    # Resurgence - Lernean
    if (self.pokemon && self.pokemon.species == :HYDREIGON) && !self.isFainted?
      if self.ability == :LERNEAN
        if @battle.FE == :DRAGONSDEN
          loop do
            change=false
            if @hp<=((@totalhp * 0.9).floor) && self.form == 1
              self.form = 2
              change = true
            elsif @hp<=((@totalhp * 0.7).floor) && self.form == 2
              self.form = 3
              change = true
            elsif @hp<=((@totalhp * 0.5).floor) && self.form == 3
              self.form = 4
              change = true
            elsif @hp<=((@totalhp * 0.3).floor) && self.form == 4
              self.form = 5
              change = true
            end
            break unless change
            @battle.scene.pbChangePokemon(self,@pokemon)
            @battle.pbDisplay(_INTL("{1} grew a head!",pbThis))
          end
        else
          loop do
            change=false
            if @hp<=((@totalhp * 0.8).floor) && self.form == 1
              self.form = 2
              change = true
            elsif @hp<=((@totalhp * 0.6).floor) && self.form == 2
              self.form = 3
              change = true
            elsif @hp<=((@totalhp * 0.4).floor) && self.form == 3
              self.form = 4
              change = true
            elsif @hp<=((@totalhp * 0.2).floor) && self.form == 4
              self.form = 5
              change = true
            end
            break unless change
    ...
    # Resurgence - Blaze Boost Form Deactivation
    if (self.pokemon && self.pokemon.species == :EMOLGA) && !self.isFainted?
      if self.ability == :BLAZEBOOST && self.form != 1
        if [:WATERSURFACE, :UNDERWATER, :MURKWATERSURFACE].include?(@battle.FE)
          @battle.pbAnimation(:SOAK,self,nil)
          self.form = 1
          @battle.scene.pbChangePokemon(self,@pokemon)
          @battle.pbDisplay(_INTL("{1} was doused by the field!",pbThis))
        elsif @battle.FE == :FROZENDIMENSION
          @battle.pbAnimation(:MIST,self,nil)
          self.form = 1
          @battle.scene.pbChangePokemon(self,@pokemon)
          @battle.pbDisplay(_INTL("{1} was chilled out by the field!",pbThis))
        end
      end
    end
    # UPDATE 1/18/2014
  ...
  def pbCheckFormRoundEnd
    ...
    # Error
    if (self.species == :REGIGIGAS || self.species == :GIRATINA || self.species == :ARCEUS) && !self.isFainted?
      self.form = 0 if (self.species == :REGIGIGAS && self.form == 1 || self.species == :GIRATINA && self.form == 2 || self.species == :ARCEUS && self.form == 19)
      pbUpdate(true)
   end
  ...
  def pbResetForm
    ...
    if !@effects[:Transform]
      if (@pokemon.species == :CASTFORM)   ||
        ...
        @pokemon.species == :ARCEUS && self.ability == :PROTEAN
        self.form=0
  ...
  def pbAbilitiesOnField(onactive)
    ...
    if @battle.FE == :VOLCANIC
      ...
      # Resurgence - Delta Emolga with Blaze Boost
      if self.species == :EMOLGA && self.form == 1
        @battle.pbCommonAnimation("BeakBlast",self,nil)
        self.form = 2
        pbUpdate(true)
        @battle.scene.pbChangePokemon(self,@pokemon)
        @battle.pbDisplay(_INTL("{1} was engulfed in flames!",pbThis))
      end
    end
    # Burning Field Entry
    if @battle.FE == :BURNING
      # Resurgence - Delta Emolga with Blaze Boost
      if self.species == :EMOLGA && self.form == 1
        @battle.pbCommonAnimation("BeakBlast",self,nil)
        self.form = 2
    ...
    if @battle.FE == :SKY
      ...
      # Resurgence - Wind Force
      if (self.ability == :WINDFORCE || self.ability == :WINDBORNETYRANT) && onactive
        if !pbTooHigh?(PBStats::SPEED)
          pbIncreaseStatBasic(PBStats::SPEED, 1)
          @battle.pbCommonAnimation("StatUp", self, nil)
          @battle.pbDisplay(_INTL("{1}'s {2} raised its speed in the skies!", pbThis, getAbilityName(ability)))
        end
      end
    ...
    if @battle.ProgressiveFieldCheck(PBFields::CONCERT)
      ...
      if self.ability.is_a?(PokeAbility)
        if self.ability.ability.is_a?(Array)
          ...
          abil.push(self.ability.ability)
        end
      end
      if [:SOUNDPROOF, :PUNKROCK, :HEAVYMETAL, :SOLIDROCK, :ROCKHEAD, :AMPLIFIER].include?(self.ability) && onactive
        if !pbTooHigh?(PBStats::DEFENSE)
          pbIncreaseStatBasic(PBStats::DEFENSE, 1)
          @battle.pbCommonAnimation("StatUp", self, nil)
      ...
      if [:PLUS, :GALVANIZE, :HEAVYMETAL, :SOLIDROCK, :PUNKROCK, :AMPLIFIER].include?(self.ability) && onactive
        message = _INTL("{1}'s {2}", pbThis, getAbilityName(ability))
        @battle.growField(message, self)
      end
    ...
    self.isEntering = :EntryFieldAbilities if self.isEntering == :Entry
  end

  def pbAbilitiesOnSwitchIn(onactive)
    return if @hp <= 0

    #if (self.species == :KYOGRE && self.item == :BLUEORB) || (self.species == :GROUDON && self.item == :REDORB) || ((self.species == :REGIGIGAS || self.species == :GIRATINA || self.species == :ARCEUS) && self.item == :CRYSTALPIECE)
    #  if self.form == 0
    #    if self.species == :KYOGRE
    #      @battle.pbCommonAnimation("PrimalReversionKyogre", self, nil)
    #    elsif self.species == :GROUDON
    ...
    if onactive
      ...
      if (self.ability == :DELTASTREAM) && @battle.weather!=:STRONGWINDS && @battle.canSetWeather?
        ...
        # Resurgence - Delta Stream increases Wind Force and Windborne Tyrant users speed by 1
        for i in @battle.battlers
          next if i.isFainted?
          if (i.ability == :WINDFORCE || i.ability == :WINDBORNETYRANT) && !i.pbTooHigh?(PBStats::SPEED)
            if i.ability == :WINDFORCE
              @battle.pbDisplay(_INTL("{1}'s Wind Force activated!",i.pbThis))
            elsif i.ability == :WINDBORNETYRANT
              @battle.pbDisplay(_INTL("{1}'s Windborne Tyrant activated!",i.pbThis))
            end
      ...
      if @effects[:Trace] || onactive
        ...
        for i in 0...4
          if pbIsOpposing?(i) && !@battle.battlers[i].isFainted?
            abilitycheck = true
            # Resurgence - Delta Avalugg Crest preventing Ability Changing/Swapping/Copying
            abilitycheck = false if (PBStuff::ABILITYBLACKLIST).include?(@battle.battlers[i].ability) || @battle.battlers[i].ability==0 || @battle.battlers[i].crested == :AVALUGG
    ...
    # Resurgence - Sublimating Edge Type Switch In
    if self.ability == :SUBLIMATINGEDGE && onactive
      @battle.pbCommonAnimation("Charge", self, nil)
      self.type1 = :ELECTRIC
      self.type2 = nil
    ...
    # GrassySurge
      ...
      if @battle.FE== :DARKNESS3
        ...
        @battle.setField(:DARKNESS2, add_on: false, noEntryAbilities: true)
        @battle.pbDisplay(_INTL("{1}'s Dark Surge deepened the darkness!",pbThis))
      elsif @battle.FE== :DARKNESS2
        @battle.setField(:DARKNESS3, add_on: false, noEntryAbilities: true)
        @battle.pbDisplay(_INTL("{1}'s Dark Surge deepened the darkness!",pbThis))
      elsif @battle.canChangeFE?(:DARKNESS2)
        @battle.setField(:DARKNESS2,3)
        @battle.pbDisplay(_INTL("Darkness gathers..."))
      end
    elsif self.ability == :SPEEDSWAP && onactive
      if @battle.trickroom == 0
        @battle.trickroom=2
        if @battle.FE == :CHESSB || @battle.FE == :NEWW || @battle.FE == :PSYCHICT || self.hasWorkingItem(:AMPLIFIELDROCK)
          @battle.trickroom=3
        end
    ...
    if self.ability == :HADRONENGINE && onactive
      if @battle.FE == :FROZENDIMENSION
        ...
        @battle.pbAnimation(:ELECTRICTERRAIN,self,nil)
        @battle.setField(:ELECTERRAIN,duration)
        @battle.pbDisplay(_INTL("{1} turned the ground into Electric Terrain, energizing its futuristic engine!", pbThis))
      end
    end
    ...
    if (ability == :SNOWWARNING || ability == :SLEET) && onactive && @battle.weather!=:HAIL
      if @battle.state.effects[:HeavyRain]
        @battle.pbDisplay(_INTL("There's no relief from this heavy rain!"))
      elsif @battle.state.effects[:HarshSunlight]
        @battle.pbDisplay(_INTL("The extremely harsh sunlight was not lessened at all!"))
        ...
        # Gen 9 Mod - Hail/Snow/Both
        if ability == :SLEET
          @battle.pbDisplay(_INTL("{1}'s Sleet made it hail harder than ever!", pbThis))
        else
    ...
    if ability == :NOCTEM && onactive && @battle.weather != :SHADOWSKY
      if @battle.state.effects[:HeavyRain]
        @battle.pbDisplay(_INTL("There's no relief from this heavy rain!"))
      elsif @battle.state.effects[:HarshSunlight]
        @battle.pbDisplay(_INTL("The extremely harsh sunlight was not lessened at all!"))
        ...
        @battle.weatherduration = 5
        @battle.weatherduration = 8 if self.hasWorkingItem(:SHADOWROCK) || [:DARKCRYSTALCAVERN,:SHORTCIRCUIT,:DIMENSIONAL,:FROZENDIMENSION,:SKY,:DARKNESS1,:DARKNESS2,:DARKNESS3].include?(@battle.FE)
        @battle.weatherduration = -1 if $game_switches[:Gen_5_Weather] == true && !@battle.isOnline?
        ...
        if @battle.FE == :CRYSTALCAVERN
          @battle.setField(:DARKCRYSTALCAVERN, @battle.weatherduration)
          @battle.pbDisplay(_INTL("The crystals lost their light!"))
        end
      end
    end
    ...
    @battle.protoabsolutionCheck # Resurgence - Protoabsolution
    @battle.protocryophilicCheck # Resurgence - Protocryophilic
    # End of Update
    # Reflector - rejuv
    ...
    if !self.pbPartner.isFainted? && @battle.pbGetOwnerIndex(self.index) == @battle.pbGetOwnerIndex(self.pbPartner.index) #i.e., if both Pokemon are owned by the same Trainer
      ...
      if speciescheck && tatsugiri.ability == :COMMANDER && !dondozo.effects[:Commandee] && ![tatsugiri,dondozo].any? {|pkmn| pkmn.effects[:TwoTurnAttack]>0 || pkmn.effects[:SkyDrop] || pkmn.effects[:SkyDroppee] || pkmn.effects[:SpiritAway] || pkmn.effects[:SpiritTaken]}
    ...
    # Resurgence - Delta Typhlosion - Super Cell
    if (self.pokemon && self.pokemon.species == :TYPHLOSION) && self.ability == :SUPERCELL && onactive
      if self.form==4 && (self.ability == :SUPERCELL && (@battle.FE == :FACTORY || @battle.FE == :SHORTCIRCUIT || @battle.FE == :ELECTRICT)) || @battle.pbWeather== :RAINDANCE
        self.form=5
        @battle.scene.pbChangePokemon(self,@pokemon)
        @battle.pbDisplay(_INTL("{1}'s Supercell was activated!",pbThis))
      end
    ...
    pbAbilitiesOnField(onactive) if self.isEntering != :EntryFieldAbilities
    # Frisk
    ...
    if self.ability == :QUARKDRIVE && onactive
      ...
      if ([:ELECTERRAIN, :SHORTCIRCUIT, :FACTORY].include?(@battle.FE) || @battle.state.effects[:ELECTERRAIN] > 0) && self.effects[:Quarkdrive][0] == 0
    ...
    if self.ability == :PROTOABSOLUTION && onactive
      aBoost = (self.attack * PBStats::StageMul[@stages[PBStats::ATTACK]]).floor
      dBoost = (self.defense * PBStats::StageMul[@stages[PBStats::DEFENSE]]).floor
      saBoost = (self.spatk * PBStats::StageMul[@stages[PBStats::SPATK]]).floor
      sdBoost = (self.spdef * PBStats::StageMul[@stages[PBStats::SPDEF]]).floor
      ...
      stats = [aBoost, dBoost, saBoost, sdBoost, spdBoost]
      boostStat = stats.index(stats.max) + 1
      if (@battle.pbWeather== :SHADOWSKY) && self.effects[:Protoabsolution][0] == 0
        self.effects[:Protoabsolution] = [boostStat, false]
        @battle.pbDisplay(_INTL("{1}'s Protoabsolution heightened its {2}!", pbThis, pbGetStatName(boostStat)))
      end
      if self.item == :BOOSTERENERGY && self.effects[:Protoabsolution][0] == 0
        self.pbDisposeItem(false)
        @battle.pbDisplay(_INTL("{1}'s Booster Energy was used up...", pbThis))
        self.effects[:Protoabsolution] = [boostStat, true]
        @battle.pbDisplay(_INTL("{1}'s Protoabsolution heightened its {2}!", pbThis, pbGetStatName(boostStat)))
      end
    end
    # Resurgence - Protocryophilic
    if self.ability == :PROTOCRYOPHILIC && onactive
      aBoost = (self.attack * PBStats::StageMul[@stages[PBStats::ATTACK]]).floor
      dBoost = (self.defense * PBStats::StageMul[@stages[PBStats::DEFENSE]]).floor
      saBoost = (self.spatk * PBStats::StageMul[@stages[PBStats::SPATK]]).floor
      sdBoost = (self.spdef * PBStats::StageMul[@stages[PBStats::SPDEF]]).floor
      ...
      stats = [aBoost, dBoost, saBoost, sdBoost, spdBoost]
      boostStat = stats.index(stats.max) + 1
      if (@battle.pbWeather== :HAIL || [:ICY, :SNOWYMOUNTAIN, :FROZENDIMENSION].include?(@battle.FE)) && self.effects[:Protocryophilic][0] == 0
        self.effects[:Protocryophilic] = [boostStat, false]
        @battle.pbDisplay(_INTL("{1}'s Protocryophilic heightened its {2}!", pbThis, pbGetStatName(boostStat)))
      end
      if self.item == :BOOSTERENERGY && self.effects[:Protocryophilic][0] == 0
        self.pbDisposeItem(false)
        @battle.pbDisplay(_INTL("{1}'s Booster Energy was used up...", pbThis))
        self.effects[:Protocryophilic] = [boostStat, true]
        @battle.pbDisplay(_INTL("{1}'s Protocryophilic heightened its {2}!", pbThis, pbGetStatName(boostStat)))
      end
    end
    ...
    # Resurgence - Chlorofury
    if self.ability == :CHLOROFURY && onactive
      faintedParty = pbFaintedPokemonCount
      faintedParty = 5 if faintedParty > 5 # If more than 5 Pokémon have fainted, set to 5, else set to pbFaintedPokemonCount
      self.effects[:Chlorofury] = faintedParty
      self.effects[:FaintedBoost] = faintedParty
      for stat in [PBStats::SPATK, PBStats::SPEED]
        if !pbTooHigh?(stat)
          self.pbIncreaseStatBasic(stat, faintedParty)
        end
      end
      if pbFaintedPokemonCount > 0
        @battle.pbCommonAnimation("StatUp", self)
        @battle.pbDisplay(_INTL("{1} boosted its stats to avenge their fallen allies!", pbThis))
      end
    end
    # Resurgence - Unleafed
    if self.ability == :UNLEAFED && onactive
      faintedParty = pbFaintedPokemonCount
      faintedParty = 5 if faintedParty > 5 # If more than 5 Pokémon have fainted, set to 5, else set to pbFaintedPokemonCount
      self.effects[:Unleafed] = faintedParty
      self.effects[:FaintedBoost] = faintedParty
      ...
      if pbFaintedPokemonCount > 0
        if self.pokemon.species == :SUNFLORA && self.form==2
          self.form=3
          @battle.scene.pbChangePokemon(self,@pokemon)
        end
        @battle.pbCommonAnimation("StatUp", self)
        @battle.pbDisplay(_INTL("{1} unloaded its power to avenge their fallen allies!", pbThis))
      end
    end
    # Resurgence - Windborne Tyrant
    if self.ability == :WINDBORNETYRANT && onactive
      if self.pbOwnSide.effects[:Tailwind]>0
        return -1
      end
      @battle.pbCommonAnimation("Windborne Tyrant", self)
      self.pbOwnSide.effects[:Tailwind]=2
      self.pbOwnSide.effects[:Tailwind]=3 if (@battle.FE == :MOUNTAIN || @battle.FE == :SNOWYMOUNTAIN || @battle.FE == :VOLCANICTOP || @battle.FE == :CLOUDS)
      self.pbOwnSide.effects[:Tailwind]=4 if @battle.FE == :SKY
      @battle.pbDisplay(_INTL("{1} summoned the winds to its call!", pbThis))
      if (@battle.FE == :MOUNTAIN || @battle.FE == :SNOWYMOUNTAIN || @battle.FE == :VOLCANICTOP || @battle.FE == :SKY) && !@battle.state.effects[:HeavyRain] && !@battle.state.effects[:HarshSunlight]
        ...
        @battle.weatherduration=4 if @battle.FE == :SKY
        @battle.pbCommonAnimation("Wind",nil,nil)
  ...
  def pbEffectsOnDealingDamage(move, user, target, damage, innards)
    ...
    # Resurgence - Battle Bond Activation
    if SpeciesToBBForm.keys.include?(user.species) && user.form != SpeciesToBBForm[user.species] && user.ability == :BATTLEBOND && !user.effects[:Transform] && user.hp>0 && target.hp<=0 && !@battle.pbAllFainted?(@battle.pbParty(target.index)) && !@@battle_bond_trainer_flags.include?(@battle.pbGetOwnerIndex(user.index))
      ...
      user.form = SpeciesToBBForm[user.species]
      user.pbUpdate(true)
      @battle.scene.pbChangePokemon(user,user.pokemon) if user.effects[:Substitute] == 0
      @battle.pbDisplay(_INTL("{1} transformed into Battle Bond {2}!",user.pbThis,getMonName(user.species)))
      @@battle_bond_flags.push(user.pokemon)
      @@battle_bond_trainer_flags.push(@battle.pbGetOwnerIndex(user.index))
    end
    ...
    if target.damagestate.calcdamage > 0 && !target.damagestate.substitute && !move.zmove && move.contactMove? && user.ability != :LONGREACH && !(user.hasWorkingItem(:PUNCHINGGLOVE) && move.punchMove?)
      ...
      if user.ability == :CORROSION && @battle.FE == :WASTELAND
        if @battle.pbRandom(10) == 0
          case @battle.pbRandom(4)
            ...
            when 3 then target.pbFrostbite(user)  if target.pbCanFrostbite?(false)
          end
        end
      end
      if !user.hasWorkingItem(:PROTECTIVEPADS)
        ...
        # Resurgence - Slag
        if target.ability == :SLAG && !user.isFainted? && user.ability != :MAGICGUARD && !((user.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM) || @battle.FE == :UNDERWATER)
          eff = PBTypes.twoTypeEff(:FIRE, user.type1, user.type2)
          ...
          if eff > 0
            ...
            @battle.scene.pbDamageAnimation(user, 0)
            user.pbReduceHP((user.totalhp * eff / 32.0).floor)
        ...
        eschance = 1
        # Resurgence - Blaze form burning the Attacker if contact is made
        if target.ability == :BLAZEBOOST && @battle.pbRandom(10) < eschance && user.pbCanBurn?(false) && @battle.FE != :FROZENDIMENSION
          user.pbBurn(target)
          @battle.pbDisplay(_INTL("{1}'s flaming body burned {3}!", target.pbThis, user.pbThis(true)))
        end
        eschance = 3
        ...
        if target.ability == :MUMMY && !user.isFainted?
          # Resurgence - Delta Avalugg Crest preventing Ability Changing/Swapping/Copying
          if user.ability != :MUMMY && !PBStuff::FIXEDABILITIES.include?(user.ability) && user.crested != :AVALUGG
        ...
        if target.ability == :WANDERINGSPIRIT && !user.isFainted? # && !@user.isBoss
          # Resurgence - Delta Avalugg Crest preventing Ability Changing/Swapping/Copying
          if ![:WANDERINGSPIRIT, :NEUTRALIZINGGAS].include?(user.ability) && !PBStuff::FIXEDABILITIES.include?(user.ability) && user.crested != :AVALUGG
            tmp = user.ability
        ...
        if target.ability == :LINGERINGAROMA && !user.isFainted?
          # Resurgence - Delta Avalugg Crest preventing Ability Changing/Swapping/Copying
          if user.ability != :LINGERINGAROMA && !PBStuff::FIXEDABILITIES.include?(user.ability) && !user.hasWorkingItem(:ABILITYSHIELD) && user.crested != :AVALUGG # Gen 9 Mod - Added Ability Shield
            neutralgas = true if user.ability == :NEUTRALIZINGGAS
    ...
    # Resurgence - Prism Guard
    if target.damagestate.calcdamage > 0 && !target.damagestate.substitute && !move.zmove && !move.contactMove?
      if target.ability == :PRISMGUARD && !user.isFainted?
        @battle.scene.pbDamageAnimation(user, 0)
        user.pbReduceHP((user.totalhp / 8.0).floor)
        @battle.pbDisplay(_INTL("{1}'s {2} hurt {3}!", target.pbThis, getAbilityName(target.ability), user.pbThis(true)))
      end
    ...
    if target.damagestate.calcdamage > 0
      ...
      if move.recoil > 0 && !target.damagestate.disguise &&
        ...
        # Resurgence - Delta Scrafty's Crest causing recoil to Heal instead of Damage
        if user.crested == :SCRAFTY
          user.pbRecoverHP(recoildamage)
          @battle.pbDisplay(_INTL("{1} is healed by the recoil!", user.pbThis))
        else
          user.pbReduceHP(recoildamage)
          @battle.pbDisplay(_INTL("{1} is damaged by the recoil!", user.pbThis))
        end
      end
      # Resurgence - Superconduct Electric and Ice Move Charging
      if user.ability == (:SUPERCONDUCT)
        if !target.damagestate.substitute
          if user.ability == (:SUPERCONDUCT)
            eschance = 1
            eschance = 2 if [:ICY, :SNOWYMOUNTAIN, :FROZENDIMENSION].include?(@battle.FE)
            if move.type == :ELECTRIC && @battle.pbRandom(10) < eschance && target.pbCanFrostbite?(false)
              target.pbFrostbite(user)
              @battle.pbDisplay(_INTL("{1} was frostbitten!",target.pbThis))
            end
            eschance = 1
            eschance = 2 if @battle.FE == :SHORTCIRCUIT || (Rejuv && @battle.FE == :ELECTERRAIN)
            if move.type == :ICE && @battle.pbRandom(10) < eschance && target.pbCanParalyze?(false)
              target.pbParalyze(user)
              @battle.pbDisplay(_INTL("{1} is paralyzed! It may be unable to move!",target.pbThis))
            end
          end
        end
        if move.type == :ELECTRIC && user.effects[:CryoCharge] == false
          user.effects[:CryoCharge] = true
          @battle.pbCommonAnimation("Charge", user, nil)
          @battle.pbDisplay(_INTL("{1}'s {2} charged its next Ice-type attack!",move.name, user.pbThis))
        elsif move.type == :ICE && user.effects[:ElectroCharge] == false
          user.effects[:ElectroCharge] = true
          @battle.pbCommonAnimation("Charge", user, nil)
          @battle.pbDisplay(_INTL("{1}'s {2} charged its next Electric-type attack!",move.name, user.pbThis))
        end
      end
      # Resurgence - Delta Noctowl inflicting Frostbite if Target was faster
      if user.crested == :NOCTOWL && user.form == 1 && target.pbCanFrostbite?(false) && !target.damagestate.substitute
        if target.hasMovedThisRound? && !@battle.switchedOut[target.index]
          target.pbFrostbite(user)
          @battle.pbDisplay(_INTL("{1} was frostbitten!",target.pbThis))
        end
      end
      ...
      if !target.damagestate.substitute
        ...
        # Resurgence - Making base Noctowl Crest's effects exclusive to its base form
        if target.crested == :NOCTOWL && target.form == 0
        ...
        if !([:UNNERVE, :ASONECHILLING, :ASONEGRIM].include?(user.ability) || [:UNNERVE, :ASONECHILLING, :ASONEGRIM].include?(user.pbPartner.ability))
          if target.hasWorkingItem(:KEEBERRY) && move.pbIsPhysical?(movetypes[0]) && user.ability != :SHEERFORCE
            if target.pbCanIncreaseStatStage?(PBStats::DEFENSE)
              ...
              if target.crested == :CRUSTLE && target.pbPartner && @battle.doublebattle && target.pbPartner.pbCanIncreaseStatStage?(PBStats::DEFENSE)
                @battle.pbCommonAnimation("Nom", target.pbPartner)
                target.pbPartner.pbIncreaseStat(PBStats::DEFENSE, 1, statmessage: false, statsource: target.pbPartner)
                if target.pbPartner.ability == :CONTRARY && !target.pbPartner.moldbroken
                  @battle.pbDisplay(_INTL("{1}'s {2} lowered {3}'s Defense!", target.pbThis, getItemName(target.pbPartner.item), target.pbPartner.pbThis))
                else
                  @battle.pbDisplay(_INTL("{1}'s {2} raised {3}'s Defense!", target.pbThis, getItemName(target.pbPartner.item), target.pbPartner.pbThis))
                end
              end
          ...
          if target.hasWorkingItem(:MARANGABERRY) && move.pbIsSpecial?(movetypes[0]) && user.ability != :SHEERFORCE
            if target.pbCanIncreaseStatStage?(PBStats::SPDEF)
              ...
              if target.crested == :CRUSTLE && target.pbPartner && @battle.doublebattle && target.pbPartner.pbCanIncreaseStatStage?(PBStats::SPDEF)
                @battle.pbCommonAnimation("Nom", target.pbPartner)
                target.pbPartner.pbIncreaseStat(PBStats::SPDEF, 1, statmessage: false, statsource: target.pbPartner)
                if target.pbPartner.ability == :CONTRARY && !target.pbPartner.moldbroken
                  @battle.pbDisplay(_INTL("{1}'s {2} lowered {3}'s Special Defense!", target.pbThis, getItemName(target.pbPartner.item), target.pbPartner.pbThis))
                else
                  @battle.pbDisplay(_INTL("{1}'s {2} raised {3}'s Special Defense!", target.pbThis, getItemName(target.pbPartner.item), target.pbPartner.pbThis))
                end
              end
              target.pbDisposeItem(true)
            end
          ...
          if target.hasWorkingItem(:JABOCABERRY, true) && !user.isFainted? && move.pbIsPhysical?(movetypes[0])
            ...
            @battle.scene.pbDamageAnimation(user, 0)
            user.pbReduceHP((user.totalhp / 8.0).floor)
            if target.crested == :CRUSTLE && target.pbPartner && @battle.doublebattle
              @battle.pbCommonAnimation("Nom", target.pbPartner)
              @battle.scene.pbDamageAnimation(user.pbPartner, 0)
              user.pbPartner.pbReduceHP((user.pbPartner.totalhp / 8.0).floor)
              @battle.pbDisplay(_INTL("{1} and {2} was hurt by the {3}!", user.pbThis, user.pbPartner.pbThis, getItemName(target.item)))
            else
              @battle.pbDisplay(_INTL("{1} was hurt by the {2}!", user.pbThis, getItemName(target.item)))
            end
            target.pbDisposeItem(true)
  ...
  # Resurgence - Used to allow Battle Bonded move boosts (move boosts shouldn't be copied by Transform).
  def battle_bonded?
    return @@battle_bond_flags.include?(@pokemon) && self.form == SpeciesToBBForm[self.species]
  end

  # Resurgence - Used to revert all Battle Bond Pokemon after battle
  def self.revert_all_battle_bond_pokemon
    @@battle_bond_flags.each {|pkmn| pkmn.form = 0}
    @@battle_bond_flags.clear
    @@battle_bond_trainer_flags.clear
  end
  ...
  def pbSynchronize
    ...
    if s >= 0 && t >= 0 && @battle.battlers[s].ability == :SYNCHRONIZE &&
      ...
      elsif @battle.synchronize[2] == :FROSTBITE && tbattler.pbCanFrostbiteSynchronize?(sbattler, true)
        tbattler.pbFrostbite(sbattler)
        @battle.pbDisplay(_INTL("{1}'s {2} made {3} frostbitten!", sbattler.pbThis, getAbilityName(sbattler.ability), tbattler.pbThis(true)))
  ...
  def pbAbilityCureCheck
    ...
    if self.ability == :MAGMAARMOR && self.status== :FROSTBITE
      @battle.pbDisplay(_INTL("{1}'s Magma Armor cured its frostbite problem!",pbThis))
      self.status=nil
    end
  ...
  def pbBerryRecoverAmount
    ...
    # Resurgence - Delta Crustle (Berry) Crest doubling berry effects if in Singles
    healing *= 2 if self.ability == :RIPEN || (self.crested == :CRUSTLE && !@battle.doublebattle)
    return healing
  end
  ...
  def pbUseBerry(berry=nil,special=false)
    ...
    stat_berry = false
    status_berry = false
    # Resurgence - Delta Crustle (Berry) Crest for checking if berry can be shared with a Partner
    partner_status_berry = false
    partner_berry_eaten = false
    berry = self.item if berry == nil
    health_threshold = self.ability == :GLUTTONY ? (self.totalhp/2.0).floor : (self.totalhp/4.0).floor
    crustle_crest = self.crested == :CRUSTLE && self.pbPartner && @battle.doublebattle
    ...
    case berry
      ...
      when :CHERIBERRY
        if self.status== :PARALYSIS
          healing = 0
          status_berry = true
          if crustle_crest && self.pbPartner.status == :PARALYSIS
            partner_status_berry = true
          end
        end
        status = "paralysis"
      when :CHESTOBERRY
        if self.status== :SLEEP
          healing = 0
          status_berry = true
          if crustle_crest && self.pbPartner.status == :SLEEP
            partner_status_berry = true
          end
        end
        status = "sleep"
      when :PECHABERRY
        if self.status== :POISON
          healing = 0
          status_berry = true
          if crustle_crest && self.pbPartner.status == :POISON
            partner_status_berry = true
          end
        end
        status = "poison"
      when :RAWSTBERRY
        if self.status== :BURN
          healing = 0
          status_berry = true
          if crustle_crest && self.pbPartner.status == :BURN
            partner_status_berry = true
          end
        end
        status = "burn"
      when :ASPEARBERRY
        if self.status== :FROSTBITE
          healing = 0
          status_berry = true
          if crustle_crest && self.pbPartner.status == :FROSTBITE
            partner_status_berry = true
          end
        end
        status = "ice"
      when :PERSIMBERRY
        if @effects[:Confusion]>0
          healing = 0
          status_berry = true
          if crustle_crest && self.pbPartner.effects[:Confusion] > 0
            partner_status_berry = true
          end
        end
        status = "confusion"
      when :LUMBERRY
        if !self.status.nil? || @effects[:Confusion]>0
          healing = 0
          status_berry = true
          if crustle_crest && (!self.pbPartner.status.nil? || self.pbPartner.effects[:Confusion] > 0)
            partner_status_berry = true
          end
        end
        status = "status"
      ...
      when :LANSATBERRY #unique berry, doing the processing now
        if @effects[:FocusEnergy]<3 && (self.hp <= health_threshold || special)
          ...
          if crustle_crest && pbPartner.effects[:FocusEnergy] < 3
            partner_berry_eaten = true
            message = _INTL("{1} used {3}'s {2} to get pumped!", pbPartner.pbThis, itemname, pbThis)
            pbPartner.effects[:FocusEnergy]+=1
          end
        end
      when :LEPPABERRY #unique berry, doing the processing now # Gen 9 Mod - special use must consume it even when not 0 PP
        if @pokemon.moves.any?{|move| move.move && move.pp == 0} || special
          ...
          for i in 0...@pokemon.moves.length
            ...
            # Resurgence - Delta Crustle (Berry) Crest doubling berry effects in Singles
            pokemove.pp = (self.ability == :RIPEN || (self.crested == :CRUSTLE && !@battle.doublebattle)) ? 20 : 10
            recoveredpp = true
          ...
          if special && !recoveredpp && specialrecover != 0
            ...
            # Resurgence - Delta Crustle (Berry) Crest doubling berry effects in Singles
            recoveramount = (self.ability == :RIPEN || (self.crested == :CRUSTLE && !@battle.doublebattle)) ? 20 : 10
            pokemove.pp = pokemove.pp + recoveramount
            pokemove.pp = pokemove.totalpp if pokemove.pp > pokemove.totalpp
            self.moves[i].pp = pokemove.pp
          end
          # Resurgence - Delta Crustle (Berry) Crest sharing berry effects with Partner
          for j in 0...pbPartner.moves.length
            partnerpokemove = pbPartner.moves[j]
            partnerspecialrecover = 0 if !partnerspecialrecover
            partnerspecialmovnum = j if crustle_crest && partnerspecialrecover == 0 && partnerpokemove.pp < partnerpokemove.totalpp
            partnerspecialrecover = partnerpokemove if crustle_crest && partnerspecialrecover == 0 && partnerpokemove.pp < partnerpokemove.totalpp
            next if partnerpokemove.pp > 0 && partnerpokemove.move
            break
          end
          if crustle_crest && partnerspecialrecover != 0
            partner_berry_eaten = true
            j = partnerspecialmovnum
            partnerpokemove = partnerspecialrecover
            recoveramount = self.pbPartner.ability == :RIPEN ? 20 : 10
            partnerpokemove.pp = partnerpokemove.pp + recoveramount
            partnerpokemove.pp = partnerpokemove.totalpp if partnerpokemove.pp > partnerpokemove.totalpp
            self.pbPartner.moves[j].pp = partnerpokemove.pp
          end
          message = _INTL("{1}'s {2} restored {3}'s PP!", pbThis, itemname, getMoveName(pokemove.move))
          message = _INTL("{1}'s {2} restored the PP for its {3} and {4}'s {5}!", pbThis, itemname, getMoveName(pokemove.move), pbPartner.pbThis, getMoveName(partnerpokemove.move)) if crustle_crest && partner_berry_eaten
        end
    end
    #return if berry didn't trigger
    return if healing == -1 && !special
    @battle.pbCommonAnimation("Nom",self,nil)
    # Resurgence - Delta Crustle (Berry) Crest play nom for Partner if they ate a berry
    if crustle_crest && ((healing > 0 && self.pbPartner.hp != self.pbPartner.totalhp) || (status_berry && partner_status_berry) || 
       partner_berry_eaten || (stat_berry && !self.pbPartner.pbTooHigh?(chosen_stat)))
      @battle.pbCommonAnimation("Nom",self.pbPartner,nil)
    end
    # Resurgence - Delta Crustle (Berry) Crest doubling berry effects in Singles, sharing berry effects with Partner if in Doubles
    if healing > 0  #this berry is ACTUALLY a healing berry
      healing*=2 if self.ability == :RIPEN || (self.crested == :CRUSTLE && !@battle.doublebattle)
      self.pbRecoverHP(healing,true)
      if crustle_crest && self.pbPartner.hp != self.pbPartner.totalhp
        partner_berry_eaten = true
        healing*=2 if self.pbPartner.ability == :RIPEN
        self.pbPartner.pbRecoverHP(healing,true)
        message = _INTL("{1}'s {2} restored health for itself and its partner!", pbThis, itemname)
      else
    ...
    if partnerconfusion && crustle_crest && self.pbPartner.pbCanConfuseSelf?(true,false)
      @battle.pbDisplay(_INTL("For {1}, the {2} was too {3}!", pbPartner.pbThis(true), itemname, flavor_text, true))
      if pbPartner.effects[:Confusion] == 0 && self.pbPartner.ability != :OWNTEMPO
        pbPartner.effects[:Confusion]=2+@battle.pbRandom(4)
        @battle.pbCommonAnimation("Confusion",self.pbPartner,nil)
        @battle.pbDisplay(_INTL("{1} became confused!",pbPartner.pbThis))
      end
    end
    ...
    # Resurgence - Partner activates Cud Chew if they ate a berry
    if partner_berry_eaten && self.pbPartner.ability == :CUDCHEW && self.pbPartner.effects[:CudChew][0] == -1
      partner_berry_eaten = false
      self.pbPartner.effects[:CudChew][0] = 1
      self.pbPartner.effects[:CudChew][1] = berry
    end
    if special
      pbBurp
      pbSymbiosis
  ...
  def pbCustapBerry
    if (self.hasWorkingItem(:CUSTAPBERRY) && ((self.ability == :GLUTTONY && self.hp<=(self.totalhp/2.0).floor) || self.hp<=(self.totalhp/4.0).floor))
      @custap = true
      @battle.pbCommonAnimation("Nom",self,nil)
      if self.crested == :CRUSTLE && self.pbPartner && @battle.doublebattle && !@battle.switchedOut[pbPartner.index]
        @battle.pbCommonAnimation("Nom",self.pbPartner,nil)
        @battle.pbDisplay(_INTL("{1} and {2} ate their Custap Berry to move first!",pbThis, pbPartner.pbThis))
      else
  ...
  # Resurgence - Giving the Partner the Custap effect if in doubles with Crested Delta Crustle
  # AsN - I really hate the way I did this. I'm sure there's an easier way but this is the only way I was able to figure it out. Please rewrite if a better method comes up.
  def pbPartnerCustapBerry
    if self.pbPartner.crested == :CRUSTLE && self.pbPartner.custap == true && @battle.doublebattle && !@battle.switchedOut[pbPartner.index]
      @partnercustap = true
    else
      @partnercustap = false
    end
...
################################################################################
  ...
  def pbSuccessCheck(basemove, user, target, flags, accuracy = true, precheckedacc: nil)
    ...
    if !basemove.zmove # Z-Moves handle protection stuff elsewhere
      ...
      # Protect / King's Shield / Obstruct / Spiky Shield / Baneful Bunker / Gen 9 Mod - Added Silk Trap and Burning Bulwark / Resurgence - Added Fallout Shelter
      if !target.effects[:ProtectNegation] && !unseenfist && basemove.canProtect? && basemove.function!=0x116 &&
        ...
        target.effects[:Protect] == :SilkTrap || target.effects[:Protect] == :BurningBulwark || target.effects[:Protect] == :FalloutShelter)
        @battle.pbDisplay(_INTL("{1} protected itself!", target.pbThis))
        # physical contact
        if basemove.contactMove? && !(user.ability == :LONGREACH) && !(user.hasWorkingItem(:PROTECTIVEPADS)) && !((user.hasWorkingItem(:PUNCHINGGLOVE) && basemove.punchMove?)) # Gen 9 Mod - Added Punching Glove, Protective Pads
          ...
          elsif target.effects[:Protect] == :FalloutShelter
            if !(user.pbCanParalyze?(false) && user.pbCanSleep?(false)) && user.pbCanPoison?(false)
              user.pbPoison(target)
              @battle.pbDisplay(_INTL("{1}'s radioactive shield poisoned {2}!", target.pbThis, user.pbThis(true)))
            elsif !(user.pbCanPoison?(false) && user.pbCanParalyze?(false)) && user.pbCanSleep?(false)
              user.pbSleep
              @battle.pbDisplay(_INTL("{1}'s radioactive shield made {2} sleep!", target.pbThis, user.pbThis(true)))
            elsif !(user.pbCanPoison?(false) && user.pbCanSleep?(false)) && user.pbCanParalyze?(false)
              user.pbParalyze(target)
              @battle.pbDisplay(_INTL("{1}'s radioactive shield paralyzed {2}! It may be unable to move!", target.pbThis, user.pbThis(true)))
            elsif !user.pbCanPoison?(false)
              rnd = @battle.pbRandom(2)
              if rnd == 0 && user.pbCanSleep?(false)
                user.pbSleep
                @battle.pbDisplay(_INTL("{1}'s radioactive shield made {2} sleep!", target.pbThis, user.pbThis(true)))
              elsif rnd == 1 && user.pbCanParalyze?(false)
                user.pbParalyze(target)
                @battle.pbDisplay(_INTL("{1}'s radioactive shield paralyzed {2}! It may be unable to move!", target.pbThis, user.pbThis(true)))
              end
            elsif !user.pbCanSleep?(false)
              rnd = @battle.pbRandom(2)
              if rnd == 0 && user.pbCanPoison?(false)
                user.pbPoison(target)
                @battle.pbDisplay(_INTL("{1}'s radioactive shield poisoned {2}!", target.pbThis, user.pbThis(true)))
    ...
    if accuracy
      ...
      case invulmove
        ...
        when 0x960 # Resurgence - Spirit Away
          (invulmiss=true)
      end
      ...
      if target.effects[:SpiritAway]
        invulmiss=true unless basemove.function==0x960 || (basemove.function==0x10D && !user.hasType?(:GHOST))
      end
    ...
    if basemove.basedamage>0 && basemove.function!=0x02 && basemove.function!=0x111 # Struggle / Future Sight
      ...
      if target.ability == (:ETHEREALSHROUD) && basemove.contactMove? && !(target.moldbroken)
        @battle.pbDisplay(_INTL("{1} makes contact moves miss with {2}!",target.pbThis,getAbilityName(target.ability)))
        return false
      end
    ...
    if accuracy
      ...
      if !basemove.pbAccuracyCheck(user, target, precheckedacc: precheckedacc)  # Includes Counter/Mirror Coat
        if targetchoices == :AllOpposing && (!user.pbOpposing1.isFainted? ? 1 : 0) + (!user.pbOpposing2.isFainted? ? 1 : 0) > 1
          ...
          elsif user.effects[:SpiritTaken] != nil
            target.effects[:SpiritAway] = false
            user.effects[:SpiritTaken] = nil
            @battle.scene.pbUnVanishSprite(target)
            @battle.pbDisplay(_INTL("{1} had its spirit returned to the battlefield!", target.pbThis))
          end
  ...
  def pbTryUseMove(choice,basemove,flags={passedtrying: false, instructed: false})
    ...
    if self.status== :FROSTBITE
      if basemove.canThawUser?
        ...
        @battle.pbDisplay(_INTL("{1} was cured of its frostbite by {2}!",pbThis,basemove.name))
        pbCheckForm
      end
    end
    ...
    if (self.ability == :PROTEAN || self.ability == :LIBERO) && basemove.move != :STRUGGLE
      ...
      if !self.hasType?(protype) || (!prot2.nil? && prot1 != prot2)
        ...
        self.type2=nil
        # Delta Ditto
        if self.species == :DITTO && self.form > 0
          change = true
          new_form = case protype
          when :NORMAL   then 1
          when :FIGHTING then 2
          when :FLYING   then 3
          when :POISON   then 4
          when :GROUND   then 5
          when :ROCK     then 6
          ...
          when :WATER    then 11
          when :GRASS    then 12
          when :ELECTRIC then 13
          when :PSYCHIC  then 14
          when :ICE      then 15
          when :DRAGON   then 16
          when :DARK     then 17
          when :FAIRY    then 18
          ...
          if new_form && self.form != new_form
            self.form = new_form
          end
          if change
            transformed = true
        ...
        # Protean Arceus
        if self.species == :ARCEUS
          change = true
          new_form = case protype
          when :NORMAL   then 0
          when :FIGHTING then 1
          when :FLYING   then 2
          when :POISON   then 3
          when :GROUND   then 4
          when :ROCK     then 5
  ...
  def pbProcessMoveAgainstTarget(basemove, user, target, numhits, flags = { totaldamage: 0 }, nocheck = false, alltargets = nil, showanimation = true, precheckedacc: nil)
    ...
    for i in 0...numhits
      ...
      checkAccuracyEachHit = (basemove.function==0xBF || basemove.function==0x914) && user.ability != :SKILLLINK && !user.hasWorkingItem(:LOADEDDICE) # Triple Kick, Triple Axel, Thunder Raid, Population Bomb
      if !nocheck && !pbSuccessCheck(basemove, user, target, flags, i == 0 || basemove.function == 0xBF || basemove.function==0x914, precheckedacc: precheckedacc) # Triple Kick, Population Bomb
       if (0xC9...0xCE).to_a.include?(basemove.function) || basemove.function==0x960
        ...
        user.effects[:Uproar] = 0 if basemove.function == 0x0D1 # Uproar
        return 0
      end
      ...
      if target.vanished && !((basemove.function == 0xC9 || basemove.function == 0xCA || basemove.function == 0xCB ||
        ...
        revanish = false if basemove.function == 0x960
        revanish = false if basemove.function == 0x11C
        revanish = false if (basemove.function == 0x10D && !user.hasType?(:GHOST)) # Curse
        @battle.scene.pbUnVanishSprite(target) unless (basemove.function == 0x10D && !user.hasType?(:GHOST)) || basemove.function == 0xCE || basemove.function == 0x960 # Curse
      end
      ...
      if !basemove.zmove && target.damagestate.calcdamage > 0 && user.ability != :SHEERFORCE &&
        ...
        addleffect = 0 if (user.effects[:Lernean] && i>1)
  ...
  def pbUseMove(choice, flags = { danced: false, totaldamage: 0, specialusage: false, specialZ: false })
    ...
    if @effects[:TwoTurnAttack] != 0 || @effects[:HyperBeam] > 0 || @effects[:Outrage] > 0 || (@effects[:Rollout] > 0 && self.ability != :JUGGERNAUT) || @effects[:Uproar] > 0 || @effects[:Bide] > 0
    ...
    # Resurgence - Spirit Away Invulnerability
    return false if self.effects[:SkyDrop] || self.effects[:Commander] || self.effects[:SpiritAway] # Gen 9 Mod - Tatsugiri can't use move if in Dondozo's mouth
    ...
    # Resurgence - Delta Emolga with Blaze Boost
    if self.ability == :BLAZEBOOST && basemove.type == :FIRE
      if self.effects[:BlazeBoost] < 1
        self.effects[:BlazeBoost] += 1
        @battle.pbCommonAnimation("BeakBlast",self,nil)
        if self.species == :EMOLGA && self.form == 1 && ![:WATERSURFACE,:UNDERWATER,:MURKWATERSURFACE,:FROZENDIMENSION].include?(@battle.FE)
          self.form = 2
          pbUpdate(true)
          @battle.scene.pbChangePokemon(self,@pokemon)
          @battle.pbDisplay(_INTL("{1} was engulfed in flames!",pbThis))
        ...
        for stat in [PBStats::ATTACK, PBStats::SPATK, PBStats::SPEED]
          if !pbTooHigh?(stat)
            statgain = 1
            self.pbIncreaseStatBasic(stat, statgain)
          end
        end
        @battle.pbCommonAnimation("StatUp", self, nil)
        @battle.pbDisplay(_INTL("{1}'s Blaze Boost boosted its stats!",pbThis))
      end
    end
    ...
    if basemove.pbTwoTurnAttack(self)
      ...
      @effects[:SpiritTaken] = nil if basemove.move != :SPIRITAWAY
    end
    # "X used Y!" message
    ...
    if targets.length == 0 && @effects[:TwoTurnAttack] == 0
      ...
      loop do
        ...
        # Lernean
        if numhits == 1 && user.species == :HYDREIGON && user.ability == :LERNEAN && !choice[2].zmove
           user.effects[:Lernean] = true
           if self.form == 1
            numhits = 5
           elsif self.form == 2
            numhits = 6
        ...
        # Resurgence - Regurgitation
        if user.species == :MUK && user.ability == :REGURGITATION && (3..8).include?(self.form)
          if (basemove.basedamage > 0 || basemove.category == :status) && basemove.move != :REGURGITATION && !basemove.zmove
            @battle.pbDisplay(_INTL("{1}'s regurgitated an attack!", user.pbThis))
            if basemove.target == :AllOpposing || basemove.target == :AllNonUsers
              movetarget = target
              movetarget = user.pbCrossOpposing if user.pbPartner == target
              movetarget = movetarget.pbPartner if movetarget.isFainted?
            ...
            if movetarget.isFainted?
              ...
              user.pbUseMoveSimple(:REGURGITATION, -1, movetarget.index)
            end
          end
        end
        i += 1
      ...
      for target in killtracker
        if user.hp > 0 && target.hp <= 0 && flags[:totaldamage] > 0
          ...
          if !@battle.pbAllFainted?(@battle.pbParty(target.index))
            if [:GRIMNEIGH, :ASONEGRIM, :HUBRIS].include?(user.ability)
      ...
      # Metronome item
      if (user.hasWorkingItem(:METRONOME) || @battle.FE == :CONCERT4 || user.ability == :PENDULUM) && movesucceeded
      ...
      if !(user.ability == :SHEERFORCE && (basemove.effect > 0 || [0x908].include?(basemove.function))) # Gen 9 Mod - Electro Shot needs to be affected by Sheer Force.
        ...
        # Resurgence - Vampiric
        if user.ability == :VAMPIRIC && flags[:totaldamage]>0 && basemove.contactMove? && @effects[:HealBlock]==0
          hpgain = flags[:totaldamage] / 4.0
          hpgain=user.pbRecoverHP([hpgain.floor,1].max,true)
          if hpgain>0
            @battle.pbDisplay(_INTL("{1}'s {2} restored its HP a little!",user.pbThis,getAbilityName(self.ability)))
          end
        end
...
################################################################################
  ...
  def getSpecialStat(unaware = false, which_is_higher: false, attacker: false, crit: false)
    ...
    spatkmult *= 1.5 if self.ability == :SUPERCELL && (@battle.pbWeather == :RAINDANCE || @battle.pbWeather == :SHADOWSKY)
    spatkmult *= 1.5 if self.ability == :ABSOLUTION && @battle.pbWeather == :SHADOWSKY
    ...
    if self.ability == :WINTERJOY
      if @battle.pbWeather == :HAIL 
        spatkmult *= 1.4
      elsif @battle.pbWeather == :SUNNYDAY
        spatkmult *= 0.7
      end
    end
    spdefmult = 1
...
################################################################################
  ...
  # Resurgence - Added Resurgence Status Moves
  def pbZStatus(move,attacker) # Gen 9 Mod - Added Gen 9 status moves
    z_effect_hash = pbHashForwardizer({
      [PBStats::ATTACK,1] => [:BULKUP,:HONECLAWS,:HOWL,:LASERFOCUS,:LEER,:MEDITATE,:ODORSLEUTH,:POWERTRICK,:ROTOTILLER,:SCREECH,:SHARPEN,
        :TAILWHIP, :TAUNT,:TOPSYTURVY,:WILLOWISP,:WORKUP,:COACHING,:POWERSHIFT,:DESERTSMARK,:DRAGONIFY],
      ...
      [PBStats::DEFENSE,1] =>   [:AQUARING,:BABYDOLLEYES,:BANEFULBUNKER,:BLOCK,:CHARM,:DEFENDORDER,:FAIRYLOCK,:FEATHERDANCE,
        ...
        :TEARFULLOOK,:TICKLE,:TORMENT,:TOXIC,:TOXICSPIKES,:VENOMDRENCH,:WIDEGUARD,:WITHDRAW,:ARENITEWALL,:BURNINGBULWARK,:MEDUSARAY],
      [PBStats::SPATK,1] => [:CONFUSERAY,:ELECTRIFY,:EMBARGO,:FAKETEARS,:GEARUP,:GRAVITY,:GROWTH,:INSTRUCT,:IONDELUGE,
        ...
        :TEETERDANCE,:TELEKINESIS,:MAGICPOWDER,:HYPOTHERMIA],
      ...
      [PBStats::SPDEF,1] => [:CHARGE,:CONFIDE,:COSMICPOWER,:CRAFTYSHIELD,:EERIEIMPULSE,:ENTRAINMENT,:FLATTER,:GLARE,:INGRAIN,
        ...
        :WATERSPORT,:WHIRLWIND,:WISH,:WONDERROOM,:CORROSIVEGAS,:SHELTER,:FALLOUTSHELTER],
      ...
      [PBStats::SPEED,1] => [:AFTERYOU,:AURORAVEIL,:ELECTRICTERRAIN,:ENCORE,:GASTROACID,:GRASSWHISTLE,:GUARDSPLIT,:GUARDSWAP,
        ...
        :SUPERSONIC,:TOXICTHREAD,:WORRYSEED,:YAWN,:SHADOWPRAYER],
      [PBStats::SPEED,2] => [:ALLYSWITCH,:BESTOW,:MEFIRST,:RECYCLE,:SNATCH,:SWITCHEROO,:TRICK,:JETSTREAM],
