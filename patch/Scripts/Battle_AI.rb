...
class PokeBattle_AI
  ...
  PARTNERFUNCTIONS = [0x40, 0x41, 0x55, 0x63, 0x66, 0x67, 0xA0, 0xC1, 0xDF, 0x142, 0x162, 0x164, 0x167, 0x169, 0x170, 0x11d, 0x185, 0x317, 0x91C, 0x907]
  ...
  # Gen 9 Mod - Added Spicy Extract and Dragon Cheer
  ...
  def pbCloneBattler(index, illusion = false)
    ...
    battler.effects = original.effects
    return battler
  end
  ...
  def checkMega
    ...
    if @attacker.pokemon.species == :REUNICLUS && @battle.trickroom > 0 && @attacker.pbSpeed < pbRoughStat(@opponent, PBStats::SPEED)
      want_to_mega = false
    end
      ...
      if @mondata.shouldswitchscore > maxmovescore && @mondata.switchscore.max > 100 # arbitrary
        if battler.index == 3 && @battle.choices[1][0] == :switch && @battle.choices[1][1] == @mondata.switchscore.index(@mondata.switchscore.max)
          if @mondata.switchscore.max(2)[1] > 100 && shouldHardSwitch?(battler, @mondata.switchscore.index(@mondata.switchscore.max(2)[1]))
            ...
            PBDebug.log(sprintf("Switching to %s", getMonName(highestscoremon.species))) if $INTERNAL
            $ai_log_data[battler.index].chosen_action = sprintf("Switching to %s", getMonName(highestscoremon.species))
  ...
  def coordinateActions(ai_l, ai_r, op_l, op_r) # changes some scores doesn't choose
    ...
    if bestmove1.move == bestmove2.move &&
       [:STEALTHROCK, :STICKYWEB, :TAILWIND, :GRAVITY, :LIGHTSCREEN, :REFLECT, :AURORAVEIL, :TRICKROOM, :WONDERROOM, :MAGICROOM, :SUNNYDAY, :RAINDANCE, :HAIL, :SANDSTORM, :SAFEGUARD, :SHADOWSKY, :SHADOWPRAYER, :JETSTREAM].include?(bestmove1.move)
      if !nextbest1.nil? && !nextbest2.nil?
  ...
  def threatAssessment(ai_l, ai_r, op_l, op_r)
    ...
    @battle.battlers.each_with_index { |opp, i|
      ...
      # Gen 9 Mod - Added Zero to Hero
      threatscore[i] *= 2.0 if opp.ability == :ZEROTOHERO
      # Boosts
      ...
      # Status
      threatscore[i] *= 0.6 if opp.status == :SLEEP
      threatscore[i] *= 0.8 if opp.status == :PARALYSIS && ![:GUTS, :FORTITUDE, :MARVELSCALE, :QUICKFEET].include?(opp.ability)
    }
  ...
  def getMoveScore(initialscores=[],scoreindex=-1)
    ...
    if @move.basedamage>0
      # Resurgence - Flicker
      if @opponent.ability == :FLICKER && @opponent.effects[:Flicker] == 3
        if [0xad, 0x157, 0x159, 0x90E, 0x80A].include?(move.function)
          score *= 3
        else
          return 0
        end
    ...
    #Hell check: Can you hit this pokemon that has an ability that nullifies your move? # Gen 9 Mod - Added myceliumMightCheck
    if @mondata.skill>=MEDIUMSKILL && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker)) &&
      ...
      if (@move.pbType(@attacker) == :FIRE && @opponent.nullsFire?) || (@move.pbType(@attacker) == :GRASS && @opponent.nullsGrass?) ||
        ...
        (@move.pbType(@attacker) == :GROUND && @opponent.nullsGround?) || (@move.pbType(@attacker) == :FLYING && @opponent.nullsFlying?)
    ...
    if @move.pbIsPriorityMoveAI(@attacker) && @attacker != @opponent.pbPartner
      ...
      if (@battle.doublebattle || (@opponent.status!=:SLEEP && !@opponent.effects[:Truant] && @opponent.effects[:HyperBeam] == 0)) && !seedProtection?(@attacker) # This line might be in the wrong place, but we're trying our best here-- skip priority if opponent is incapacitated
        if score>100
    ...
    # Sound move checks # Gen 9 Mod - Added myceliumMightCheck
    if !@move.zmove && @move.isSoundBased?
      $ai_log_data[@attacker.index].final_score_moves.push(0) if (@opponent.ability == :SOUNDPROOF && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))) || @attacker.effects[:ThroatChop]!=0
      return 0 if (@opponent.ability == :SOUNDPROOF && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))) || @attacker.effects[:ThroatChop]!=0
      score *= 0.6 if checkAImoves([:THROATCHOP])
    end
    # Wind Move Checks
    # Gen 9 Mod - Added Wind Rider # Gen 9 Mod - Added myceliumMightCheck
    if !@move.zmove && @move.windMove?
      $ai_log_data[@attacker.index].final_score_moves.push(0) if (@opponent.ability == :WINDRIDER && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker)))
      return 0 if (@opponent.ability == :WINDRIDER && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker)))
    end
    # Resurgence - Contact Moves check for Ethereal Shroud
    if !@move.zmove && @move.contactMove?
      $ai_log_data[@attacker.index].final_score_moves.push(0) if (@opponent.ability == :ETHEREALSHROUD && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker)))
      return 0 if (@opponent.ability == :ETHEREALSHROUD && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker)))
    end
    ...
    if @mondata.skill >= HIGHSKILL && @opponent.index != @attacker.index
      for j in getAIMemory(@opponent)
        ...
        # Resurgence - Added Fallout Shelter
        shieldcheck = true if j.move == :SPIKYSHIELD || j.move == :KINGSSHIELD || j.move == :BANEFULBUNKER || j.move == :OBSTRUCT || j.move == :SILKTRAP || j.move == :BURNINGBULWARK || j.move == :FALLOUTSHELTER
      end
      ...
      ioncheck = checkAImovesAllOpponents([:IONDELUGE, :PLASMAFISTS])
      ...
      score *= 0.2 if checkAImovesAllOpponents([:WIDEGUARD]) && [:AllOpposing, :AllNonUsers].include?(@attacker.pbTarget(@move))
    ...
    if !@move.zmove && @move.contactMove? && !(@attacker.item == :PROTECTIVEPADS || (@attacker.item == :PUNCHINGGLOVE && move.punchMove?)) && @attacker.ability != :LONGREACH
      contactscore = 1.0
      ...
      case @opponent.ability
        ...
        when :BLAZEBOOST then contactscore *= 0.75 if @attacker.pbCanBurn?(false) && @opponent.form == 2
        ...
        when :SLAG then contactscore *= @attacker.hp < 0.2 * @attacker.totalhp ? 0.5 : 0.85 unless @battle.FE == :UNDERWATER
        ...
        when :MUMMY, :WANDERINGSPIRIT, :LINGERINGAROMA
          # Resurgence - Delta Avalugg Crest preventing Ability Changing/Swapping/Copying
          if !((PBStuff::FIXEDABILITIES).include?(@attacker.ability)) && !(@attacker.ability == :MUMMY || @attacker.ability == :SHIELDDUST || @attacker.ability == :LINGERINGAROMA) && @attacker.crested != :AVALUGG
        ...
        when :TOXICDEBRIS then contactscore *= 0.8 if @attacker.pbOwnSide.effects[:ToxicSpikes] < 2
      end
      ...
      # Resurgence - Added Fallout Shelter
      contactscore *= 0.7 if checkAImoves([:FALLOUTSHELTER]) && !PBStuff::RATESHARERS.include?(@opponent.lastMoveUsed) && (@attacker.pbCanPoison?(false) || @attacker.pbCanParalyze?(false) || @attacker.pbCanSleep?(false))
      # Gen 9 Mod - Added Shield Dust & Covert Cloak
      contactscore *= 1.1 if @attacker.ability == :POISONTOUCH && @opponent.pbCanPoison?(false) && !(@opponent.ability == :SHIELDDUST || @opponent.item == :COVERTCLOAK)
    ...
    if @opponent.effects[:TwoTurnAttack]!=0 && @mondata.skill>=HIGHSKILL
      ...
      # Resurgence - Spirit Away AI Invulnerability
      if (@move.accuracy>0 || @move.function==0xA5 || @move.zmove && @move.basedamage > 0 || @move.move == :WHIRLWIND) && (PBStuff::TWOTURNMOVE.include?(invulmove) || @opponent.effects[:SkyDrop] || @opponent.effects[:SpiritAway]) &&
    ...
    if @attacker.status == :FROSTBITE && @mondata.skill>=MEDIUMSKILL
    ...
    # Resurgence prefer Fire moves if Ability is Blaze Boost and Counter is <2
		if @attacker.ability == :BLAZEBOOST && @move.type == :FIRE && @attacker.effects[:BlazeBoost] < 2 && @move.basedamage > 0
      score+=80
		end
    ...
    if @mondata.skill>=MEDIUMSKILL && @attacker.ability == :PRANKSTER && (@opponent.hasType?(:DARK) && @battle.FE != :BEWITCHED)
      ...
      if @move.basedamage==0 && @move.priority>-1 && ((@attacker.pbTarget(@move) && 0x486 != 0)) # Fixes Prankster on Opponents, so uh, tread lightly # Gen 9 Mod - Prankster Fix
        $ai_log_data[@attacker.index].final_score_moves.push(0)
        return 0
      end
    end
    ...
    case @move.function
      when 0x00 # No effect
        if @mondata.skill >= BESTSKILL && @battle.FE != :INDOOR
          ...
          # Resurgence - Added Glinting Beam
          when :MIRROR
            miniscore*=2 if (@move.move == :DAZZLINGGLEAM || @move.move == :GLINTINGBEAM) && mirrorNeverMiss
            miniscore*=0.3 if @move.move == :BOOMBURST || @move.move == :HYPERVOICE
          when :FLOWERGARDEN2,:FLOWERGARDEN3,:FLOWERGARDEN4,:FLOWERGARDEN5
            if (@move.move == :CUT || @move.move == :XSCISSOR || @move.move == :SHADOWPRAYER) && @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN,2,5)
      ...
      when 0x02 # Struggle
        miniscore*=0.2
      when 0x03 # Sleep, Dark Void, Grass Whistle, Sleep Powder, Spore, Relic Song, Lovely Kiss, Sing, Hypnosis, # Gen 9 Mod - Wicked Torque
        miniscore = sleepcode()
      ...
      when 0x04 # Yawn
        miniscore = sleepcode()
      when 0x05 # Poison, Gunk Shot, Sludge Wave, Sludge Bomb, Poison Jab, Sludge, Poison Tail, Smog, Poison Sting, Poison Gas, Poison Powder, # Gen 9 Mod - Noxious Torque
      ...
      when 0x06 # Toxic, Poison Fang, # Gen 9 Mod - Malignant Chain
        miniscore = poisoncode()
        if @move.move==:TOXIC
          miniscore*=1.1 if @attacker.hasType?(:POISON)
        end
      when 0x07 # Paralysis, Dragon breath, Bolt Strike, Zap Cannon, Thunderbolt, Discharge, Thunder Punch, Spark, Thunder Shock, Thunder Wave, Force Palm, Lick, Stun Spore, Body Slam, Glare, Nuzzle, Wildbolt Storm, # Gen 9 Mod - Combat Torque
        miniscore = paracode()
      when 0x08 # Thunder
        miniscore = paracode()
      ...
      when 0x0A # Burn Blue Flare, Fire Blast, Heat Wave, Inferno, Sacred Fire, Searing Shot, Flamethrower, Blae kick, Lava Plume, Fire Punch, Flame Wheel, Ember, Will-O-Wist, Scald, Steam Eruption, Sandsear Storm, # Gen 9 Mod - Blazing Torque
        miniscore = burncode()
        if @mondata.skill >= BESTSKILL
      ...
      when 0x13 # Confusion, Signal Beam, Dynamic Punch, Chatter, Confuse Ray, Rock Climb, Dizzy Punch, Supersonic, Sweet Kiss, Teeter Dance, Psybeam, Water Pulse, Strange Steam, # Gen 9 Mod - Magical Torque
        miniscore = confucode()
        if @mondata.skill >= BESTSKILL
      ...
      when 0x21 # Charge
        ...
        miniscore*=1.5 if @attacker.moves.any?{|moveloop| moveloop!=nil && moveloop.pbType(@attacker)==:ELECTRIC} && @attacker.effects[:Charge]==true
      ...
      when 0x29 # Hone Claws
        statarray = [1,0,0,0,0,1,0]
        statarray = [2,0,0,0,0,2,0] if @mondata.skill >= BESTSKILL && @battle.pbWeather== :SHADOWSKY
        miniscore = selfstatboost(statarray)
      ...
      when 0x35 # Shell Smash
        ...
        miniscore*= selfstatdrop([0,1,0,1,0,0,0],score) if !(@mondata.attitemworks && @attacker.item == :WHITEHERB) # Gen 9 Mod - Slight edit to more cleanly, I think.
      ...
      when 0x3c # Close Combat, Dragon Ascent, Headlong Rush, # Gen 9 Mod - Armor Cannon
        statarray = [0,1,0,1,0,0,0]
        if @attacker.ability == :CONTRARY
          miniscore = selfstatboost(statarray)
        else
      ...
      when 0x42 # Growl, Aurora Beam, Baby-Doll Eyes, Play Nice, Play Rough, Lunge, Trop Kick, # Gen 9 Mod - Chilling Water
        statarray = [1,0,0,0,0,0,0]
      ...
      when 0x44 # Rock Tomb, Electroweb, Low Sweep, Bulldoze, Mud Shot, Glaciate, Icy Wind, Constrict, Bubble Beam, Bubble, # Gen 9 Mod - Pounce
        statarray = [0,0,0,0,1,0,0]
      ...
      when 0x47 # Sand Attack, Night Daze, Leaf Tornado, Mod Bomb, Mud-Slap, Flash, Smokescreen, Kinesis, Mirror Shot, Muddy Water, Octazooka
        ...
        if @mondata.skill >= BESTSKILL
          ...
          statarray = [0,0,0,0,0,2,0] if @move.move==:FLASH && ([:DARKCRYSTALCAVERN,:SHORTCIRCUIT,:MIRROR,:STARLIGHT,:NEWWORLD,:DARKNESS1].include?(@battle.FE) || @battle.pbWeather == :SHADOWSKY)
      ...
      #when 0x9f # Judgment, Techno Blast, Multi-Attack
      ...
      when 0xa5 # Shock Wave, Feint Attack, Aura Sphere, Vital Throw, Aerial Ace, Shadow Punch, Swift, Magnet Bomb, Disarming Voice, Smart Strike, False Surrender, # Gen 9 Mod - Kowtow Cleave
        miniscore = nevermisscode(initialscores[scoreindex])
      ...
      when 0xba # Taunt
        miniscore = tauntcode()
      when 0xbb # Heal Block, # Gen 9 Mod - Psychic Noise
      ...
      # Resurgence - LAWDS Juggernaut added
      when 0xc2 # Hyper Beam, Roar of Time, Blast Burn, Frenzy Plant, Giga Impact, Rock Wrecker, Hydro Cannon, Prismatic Laser, Meteor Assault
        miniscore = hypercode() unless (@move.move == :METEORASSAULT && @battle.FE == :STARLIGHT) || (@attacker.ability == :JUGGERNAUT)
        miniscore = 1 if @attacker.ability == :JUGGERNAUT
      ...
      when 0xc4 # Solar Beam, Solar Blade
        ...
        if @battle.pbWeather!=:SUNNYDAY && @attacker.pbHasMove?(:SUNNYDAY) && !(@battle.pbCheckGlobalAbility(:AIRLOCK) || @battle.pbCheckGlobalAbility(:CLOUDNINE) || @battle.pbCheckGlobalAbility(:DELTASTREAM) ||
          ...
          miniscore = weaselslashcode() unless @battle.pbWeather == :SUNNYDAY || @battle.FE == :RAINBOW
        end
        miniscore = 0 if @battle.FE == :DARKCRYSTALCAVERN || @battle.pbWeather==:SHADOWSKY
      ...
      when 0xcd # Phantom Force, Shadow Force
        if @attacker.ability == :NOGUARD || @opponent.ability == :NOGUARD || (@opponent.ability==:FAIRYAURA && @battle.FE == :FAIRYTALE)
          miniscore = weaselslashcode() unless @battle.FE == :HAUNTED || @battle.ProgressiveFieldCheck(PBFields::DARKNESS,2,3) || @battle.pbWeather==:SHADOWSKY
        else
      ...
      when 0xd2 # Outrage, Petal Dance, Thrash
        ...
        miniscore = 1 if @attacker.ability == :JUGGERNAUT # Resurgence - LAWDS Juggernaut added
      ...
      when 0xd8 # Synthesis, Moonlight, Morning Sun
        ...
        recoveramount = (@attacker.totalhp*0.66).floor  if @battle.pbWeather == :SHADOWSKY && @move.move==:MOONLIGHT && !@attacker.hasWorkingItem(:UTILITYUMBRELLA)
        recoveramount = (@attacker.totalhp*0.33).floor  if @battle.pbWeather == :SHADOWSKY && (@move.move==:SYNTHESIS || @move.move==:MORNINGSUN) && !@attacker.hasWorkingItem(:UTILITYUMBRELLA) && ![:DARKCRYSTALCAVERN,:DARKNESS2,:DARKNESS3].include?(@battle.FE)
      ...
      when 0xdc # Leech Seed
        miniscore = leechcode()
      when 0xdd # Absorb. Leech Life, Drain Punch, Giga Drain, Horn Leech, Mega Drain, Parabolic Charge, # Gen 9 Mod - Bitter Blade
      ...
      when 0xed # Baton Pass
        miniscore = pivotcode()
      when 0xee # U-turn, Volt Switch/ Gen 9 Mod - Flip Turn
        miniscore = pivotcode()
      ...
      when 0x105 # Stealth Rocks
        if !(@attacker.pbOpposingSide.effects[:StealthRock] || @attacker.pbOpposingSide.effects[:FoundryStealthRock])
          miniscore = hazardcode()
          ...
          if @mondata.skill>=BESTSKILL
            ...
            miniscore*=1.3 if @attacker.ability == :FOUNDRY # Resurgence - Foundry Stealth Rock AI Interactions
          end
        else
          miniscore=0
        end
      ...
      when 0x10b # Hi Jump Kick, Jump Kick, # Gen 9 Mod - Supercell Slam
        miniscore = jumpcode(score)
        if @attacker.index != 2 && @mondata.skill>=BESTSKILL
          miniscore*= 0.5 if @battle.FE != :INVERSE && pbPartyHasType?(:GHOST, @opponent.index) && (@move.move == :HIJUMPKICK || @move.move == :JUMPKICK)
          miniscore*= 0.5 if @battle.FE != :INVERSE && pbPartyHasType?(:GROUND, @opponent.index) && @move.move == :SUPERCELLSLAM
        end
      ...
      when 0x110 # Rapid Spin, # Gen 9 Mod - Mortal Spin
        score+=20 if @attacker.effects[:LeechSeed]>=0
        score+=10 if @attacker.effects[:MultiTurn]>0
        if @attacker.pbNonActivePokemonCount>0
          score+=25 if @attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock]
        ...
        # Gen 9 Mod - Added poisoncode for mortal spin.
        if @move.move == :MORTALSPIN
          score*=poisoncode()
        end
      ...
      when 0x115 # Focus Punch
        miniscore = focuscode()
      when 0x116 # Sucker Punch, # Gen 9 Mod - Thunderclap
      ...
      when 0x13e # Geomancy
        ...
        statarray = [0,0,2,2,2,0,0]
        statarray = [0,0,1,1,1,0,0] if @mondata.skill >= BESTSKILL && @battle.pbWeather== :SHADOWSKY
        miniscore *= selfstatboost(statarray)
        if @battle.FE == :NEWWORLD
      ...
      when 0x171 # Stomping Tantrum, # Gen 9 Mod - Temper Flare
        miniscore = 1.0
        miniscore*=0.8 if Rejuv && @battle.FE == :CHESS && @attacker.ability != :SHELLARMOR && @attacker.ability != :BATTLEARMOR && @move.move == :STOMPINGTANTRUM
      ...
      when 0x17A # Stuff Cheeks
        if pbIsBerry?(@attacker.item)
          ...
          when :ASPEARBERRY then miniscore*=2 if @attacker.status == :FROSTBITE
      ...
      when 0x189 # Jaw Lock
        miniscore = meanlookcode()
      when 0x306 # Steel Roller, # Gen 9 Mod - Ice Spinner
        miniscore = getFieldDisruptScore(@attacker,@opponent)
        miniscore = 0 if @battle.FE == :INDOOR && @move.move == :STEELROLLER
      ...
      when 0x313 # Burning Jealousy, # Gen 9 Mod - Alluring Voice
        miniscore = burncode() if ((checkAImoves(PBStuff::SETUPMOVE) && !pbAIfaster?()) || @opponent.effects[:Jealousy]) && @move.move == :BURNINGJEALOUSY
        miniscore = confucode() if ((checkAImoves(PBStuff::SETUPMOVE) && !pbAIfaster?()) || @opponent.effects[:Jealousy]) && @move.move == :ALLURINGVOICE
      when 0x314 # Lash Out
      ...
      when 0x506 # Axe Kick
        ...
        miniscore *= confucode()
      when 0x507 # Gen 9 Mod - Alluring Voice
        miniscore = confucode() if ((checkAImoves(PBStuff::SETUPMOVE) && !pbAIfaster?()) || @opponent.effects[:Jealousy]) && @move.move == :ALLURINGVOICE
      # Gen 9 Mod - New Gen 9 Moves
      when 0x902 # Burning Bulwark
        miniscore = protecteffectcode()
        if !@opponent.status.nil?
          miniscore *= 0.8
        elsif @opponent.pbCanBurn?(false)
          miniscore *= 1.3
      ...
      when 0x908 # Electro Shot
        ...
        miniscore *= weaselslashcode() unless @battle.pbWeather == :RAINDANCE
      when 0x90A # Fillet Away
        miniscore = selfstatboost([2,0,2,0,2,0,0])
        miniscore *= 0.3 if !@attacker.moves.any?{|moveloop| moveloop!=nil && moveloop.basedamage > 0 && moveloop.pbIsPriorityMoveAI(@attacker)} && !pbAIfaster?()
        miniscore *= 1.2 if @attacker.turncount<1
        miniscore = 1 if (@attacker.hp.to_f)/@attacker.totalhp <= 0.5
      ...
      when 0x90C # Glaive Rush
        miniscore*= 1.4 if ((pbRoughDamage > @opponent.hp) && pbAIfaster?())    # If we're going to knock out the opponent and are faster, the downside of Glaive Rush isn't a huge deal, since we're comparatively fast, so the next thing sent in likely won't act before us. (Kinda flimsy logic, but it works well enough in practice).
        miniscore*= 1.1 if ((pbRoughDamage > @opponent.hp) && !pbAIfaster?())   # If we're going to knock out the opponent and aren't faster, the downside of Glaive Rush isn't a big deal, but we might have to switch out or get knocked out next turn since we're comparatively slow, so the next thing sent in likely would be able to act before us. (Kinda flimsy logic, but it works well enough in practice).
        miniscore*= 0.9 if (!(pbRoughDamage > @opponent.hp) && !pbAIfaster?())  # if we're not going to knock out the opponent and aren't faster, next turn their move will hurt a lot so we might have to switch or get knocked out.
        miniscore*= 0.7 if (!(pbRoughDamage > @opponent.hp) && pbAIfaster?())   # If we're not going to knock out the opponent and are faster, their move this turn will hurt a lot and we might get knocked out.
      when 0x90E # Hyper Drill / Mighty Cleave
         # They technically don't break also protect for other moves, but
      ...
      # Resurgence Moves
      when 0x951 # Dragonify
        miniscore = opptypechangecode(:DRAGON)
      when 0x952 # Jet Stream
        miniscore = jetstreamcode()
      when 0x954 # Umbral Cannon
        if @battle.pbWeather!=:SHADOWSKY && @attacker.pbHasMove?(:SHADOWSKY) && !(@battle.pbCheckGlobalAbility(:AIRLOCK) || @battle.pbCheckGlobalAbility(:CLOUDNINE) || @battle.pbCheckGlobalAbility(:DELTASTREAM) ||
          @battle.pbCheckGlobalAbility(:DESOLATELAND) || @battle.pbCheckGlobalAbility(:PRIMORDIALSEA) || @attacker.item == :POWERHERB || @battle.FE == :UNDERWATER || @battle.FE == :NEWWORLD)
          miniscore = 0.3
        else
          miniscore = weaselslashcode() unless @battle.pbWeather == :SHADOWSKY
        end
        miniscore = 0 if @battle.pbWeather==:SUNNYDAY
      when 0x955 # Medusa Ray
        miniscore *= petrifycode()
      when 0x957 # Shadow Prayer
        miniscore = weathercode()
        miniscore*=shadowskycode()
        if @mondata.skill>=BESTSKILL
          miniscore*=1.5 if @battle.FE == :RAINBOW # Rainbow
          miniscore*=2   if @battle.FE == :STARLIGHT && !pbPartyHasType?(:DARK) && !pbPartyHasType?(:FAIRY) && !pbPartyHasType?(:PSYCHIC)  # Starlight
        end
      #when 0x959 # Retrograde # On hold because the opppnent thinks it is a mega even after being unmega'd
      #  if opponent.isMega?
      #    return score*=10
      #  else
      #    score*=0
      #  end
      when 0x960 # Spirit Away
        if @attacker.ability == :NOGUARD || @opponent.ability == :NOGUARD || (@opponent.ability==:FAIRYAURA && @battle.FE == :FAIRYTALE)
          miniscore = weaselslashcode()
        else
          miniscore = twoturncode()
        end
      when 0x972 # Fallout Shelter
        miniscore = protecteffectcode()
        if !@opponent.status.nil?
          miniscore *= 0.8
        elsif (@opponent.pbCanSleep?(false) || @opponent.pbCanParalyze?(false) || @opponent.pbCanPoison?(false))
          miniscore *= 1.3
        end
...
#All functions here return a modifier to the original score, similar to miniscore
  def sleepcode
    ...
    return @move.basedamage > 0 ? 1 : 0 if !(@opponent.pbCanSleep?(false, false, false, moldbroken) && @opponent.effects[:Yawn]==0) # Gen 9 Mod - Proper Mold Braker Check
  ...
  def poisoncode
    moldbroken = moldBreakerCheck(@attacker) # Gen 9 Mod - Proper Mold Braker Check
    moldbroken = myceliumMightCheck(@attacker) if !moldbroken # Gen 9 Mod - Added Mycelium Might
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanPoison?(false, false, @move.move==:TOXIC && @attacker.ability==:CORROSION, moldbroken) # Gen 9 Mod - Proper Mold Braker Check
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
    miniscore=1.2
    ministat=0
    ...
    # Gen 9 Mod - Discourage status moves when current opponent has Good as Gold
    miniscore*=0 if @opponent.ability == :GOODASGOLD && @move.category == :status && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))
    miniscore*=0.2 if @opponent.ability == :TOXICBOOST || @opponent.ability == :GUTS || @opponent.ability == :FORTITUDE || @opponent.ability == :QUICKFEET
  ...
  def paracode
    moldbroken = moldBreakerCheck(@attacker) # Gen 9 Mod - Proper Mold Braker Check
    moldbroken = myceliumMightCheck(@attacker) if !moldbroken # Gen 9 Mod - Added Mycelium Might
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanParalyze?(false, moldbroken) # Gen 9 Mod - Proper Mold Braker Check
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
    ...
    # Gen 9 Mod - Discourage status moves when current opponent has Good as Gold
    miniscore*=0 if @opponent.ability == :GOODASGOLD && @move.category == :status && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))
    miniscore*=0.2 if @opponent.ability == :GUTS || @opponent.ability == :FORTITUDE || @opponent.ability == :QUICKFEET
    miniscore*=0.7 if @opponent.ability == :SHEDSKIN
  ...
  def burncode
    moldbroken = moldBreakerCheck(@attacker) # Gen 9 Mod - Proper Mold Braker Check
    moldbroken = myceliumMightCheck(@attacker) if !moldbroken # Gen 9 Mod - Added Mycelium Might
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanBurn?(false, false, moldbroken) # Gen 9 Mod - Proper Mold Braker Check
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
    miniscore=1.2
    ministat=0
    ...
    # Gen 9 Mod - Discourage status moves when current opponent has Good as Gold
    miniscore*=0 if @opponent.ability == :GOODASGOLD && @move.category == :status && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))
    miniscore*=0.1 if @opponent.ability == :GUTS || @opponent.ability == :FORTITUDE || @opponent.ability == :FLAREBOOST
    miniscore*=0.7 if @opponent.ability == :SHEDSKIN
  ...
  def freezecode
    moldbroken = moldBreakerCheck(@attacker) # Gen 9 Mod - Proper Mold Braker Check
    moldbroken = myceliumMightCheck(@attacker) if !moldbroken # Gen 9 Mod - Added Mycelium Might
    return @move.basedamage > 0 ? 1 : 0 if !@opponent.pbCanFrostbite?(false, false, moldbroken) # Gen 9 Mod - Proper Mold Braker Check
    return @move.basedamage > 0 ? 1 : 0 if hydrationCheck(@opponent)
    return @move.basedamage > 0 ? 1 : 0 if secondaryEffectNegated?()
    miniscore=1.2
    ministat=0
    ...
    # Gen 9 Mod - Discourage status moves when current opponent has Good as Gold
    miniscore*=0 if @opponent.ability == :GOODASGOLD && @move.category == :status && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))
    miniscore*=0.1 if @opponent.ability == :GUTS || @opponent.ability == :FORTITUDE
    miniscore*=0.7 if @opponent.ability == :SHEDSKIN
    miniscore*=0.5 if @opponent.ability == :SYNCHRONIZE && @attacker.pbCanFrostbite?(false)
    miniscore*=0.5 if @opponent.ability == :MAGICGUARD || (@opponent.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
    miniscore*=0.3 if @opponent.ability == :QUICKFEET
    miniscore*=1.1 if (@opponent.ability == :STURDY || (@battle.FE == :CHESS && @opponent.pokemon.piece==:PAWN) || (@battle.FE == :COLOSSEUM && @opponent.ability == :STALWART)) && @move.basedamage>0
    miniscore*=0.1 if checkAImoves([:REST])
  ...
  def flinchcode
    ...
    # Resurgence - Added Crystal-typing to Sandstorm Immunity
    if @opponent.status== :POISON || @opponent.status== :BURN || @opponent.status== :FROSTBITE || (@battle.pbWeather == :HAIL && !@opponent.hasType?(:ICE) && $HAILSNOWMOD != "Snow") || (@battle.pbWeather == :SANDSTORM && !@opponent.hasType?(:ROCK) && !@opponent.hasType?(:GROUND) && !@opponent.hasType?(:STEEL) && !@opponent.hasType?(:CRYSTAL)) || (@battle.pbWeather == :SHADOWSKY && !@opponent.hasType?(:SHADOW) && !@opponent.hasType?(:DARK) && !@opponent.hasType?(:GHOST)) || @opponent.effects[:LeechSeed]>-1 || @opponent.effects[:Curse] || @opponent.effects[:SaltCure]
      miniscore *= 1.1
  ...
  def refreshcode
    miniscore = 1.0
    if @attacker.status== :BURN || @attacker.status== :FROSTBITE || @attacker.status== :POISON || @attacker.status== :PARALYSIS
      miniscore*=3
    else
      return 0
    end
  ...
  def partyrefreshcode
    ...
    for mon in @battle.pbPartySingleOwner(@attacker.index)
      ...
      miniscore*=0.8 if mon.ability == :GUTS || mon.ability == :FORTITUDE || mon.ability == :QUICKFEET || mon.knowsMove?(:FACADE)
      miniscore*=1.1 if mon.status== :SLEEP
      ...
      miniscore*=1.2 if mon.spatk>mon.attack && mon.status== :FROSTBITE
    end
  ...
  def psychocode
    ...
    return 0 if @attacker.status== :BURN && !@opponent.pbCanBurn?(false) || @attacker.status== :FROSTBITE && !@opponent.pbCanFrostbite?(false) || @attacker.status== :PARALYSIS && !@opponent.pbCanParalyze?(false) || @attacker.status== :POISON && !@opponent.pbCanPoison?(false)
    miniscore=1.3*1.3
    ...
    if @attacker.status== :FROSTBITE && @opponent.pbCanFrostbite?(false)
      miniscore*=1.2 if pbRoughStat(@opponent,PBStats::SPATK)>pbRoughStat(@opponent,PBStats::ATTACK)
    end
    ...
    miniscore*=0.7 if @opponent.ability == :SHEDSKIN || @opponent.ability == :NATURALCURE || @opponent.ability == :GUTS || @opponent.ability == :FORTITUDE || @opponent.ability == :QUICKFEET || @opponent.ability == :MARVELSCALE
  ...
  def selfstatboost(stats)
    ...
    for i in 1...stats.length
      ...
      case i
        ...
        when PBStats::DEFENSE
          tank = true
          miniscore*=1.1 if @opponent.status== :FROSTBITE
          if pbRoughStat(@opponent,PBStats::SPATK)<pbRoughStat(@opponent,PBStats::ATTACK)
        ...
        when PBStats::SPATK
          ...
          miniscore*=0.5 if @attacker.status== :FROSTBITE && @attacker.ability != :FORTITUDE
          miniscore*=1.4 if notOHKO?(@attacker,@opponent)
          miniscore*=0.6 if checkAIpriority()
        when PBStats::SPDEF
          tank = true
        ...
        when PBStats::SPEED
          ...
          if @attacker.moves.any? { |moveloop|
            ...
            # Moxie/Soul Heart
            miniscore *= 1.5 if (physmove && [:MOXIE, :CHILLINGNEIGH, :ASONECHILLING].include?(@attacker.ability)) || (specmove && [:SOULHEART, :GRIMNEIGH, :ASONEGRIM, :HUBRIS].include?(@attacker.ability))
            if @attacker.attack < @attacker.spatk
    ...
    miniscore *= 0.3 if checkAImovesAllOpponents([:SNATCH])
    if sweep
      ministat = @opponent.stages[PBStats::ATTACK] + @opponent.stages[PBStats::SPATK] + @opponent.stages[PBStats::SPEED]
  ...
  def oppstatboost(stats, confusionmove = false, alsodecrease = false)
    ...
    if @opponent.index != @attacker.pbPartner.index
      ...
      # Gen 9 Mod - Opportunist and Mirror Herb
      if @attacker.ability == :OPPORTUNIST || @attacker.pbPartner.ability == :OPPORTUNIST || @attacker.item == :MIRRORHERB || @attacker.pbPartner.item == :MIRRORHERB
        if miniscore == 0
          miniscore = 1.5
        else
          miniscore *= 1.5
      ...
      # Gen 9 Mod - Opportunist
      miniscore *= 0.3 if @opponent.ability == :OPPORTUNIST || @opponent.pbPartner.ability == :OPPORTUNIST || @opponent.item == :MIRRORHERB || @opponent.pbPartner.item == :MIRRORHERB
  ...
  def oppstatdrop(stats)
    ...
    # Gen 9 Mod - Discourage status moves when current opponent has Good as Gold, but not when abilities are nullified because other abilities.
    return @move.basedamage > 0 ? 1 : 0 if (@opponent.ability == :CLEARBODY || @opponent.ability == :WHITESMOKE || @opponent.ability == :GOODASGOLD) && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))
    # Gen 9 Mod - Added Clear Amulet
    return @move.basedamage > 0 ? 1 : 0 if @opponent.item == :CLEARAMULET
    #stats should be an array of the stat boosts like so: [ATK,DEF,SPE,SPA,SPD,ACC,EVA] with nils in unaffected stats
    #coil, for example, would be [1,1,0,0,0,1,0]
    stats.unshift(0) #this is required to make the next line work correctly
    #Start by eliminating pointless stats
    ...
    if stats[PBStats::DEFENSE]>0 || stats[PBStats::ATTACK]>0 || stats[PBStats::SPDEF]>0
      miniscore*=1.2 if @opponent.status== :POISON || @opponent.status == :FROSTBITE
    end
    ...
    if stats[PBStats::SPATK]>0
      miniscore*=1.2 if @opponent.status== :POISON || @opponent.status == :BURN
      miniscore*=0.5 if @opponent.status== :FROSTBITE
    end
    if stats[PBStats::SPEED]>0
  ...
  def focusenergycode
    ...
    miniscore*=2 if @attacker.ability == :SUPERLUCK || @attacker.ability == :SNIPER || @attacker.ability == :SUBLIMATINGEDGE
  ...
  def defogcode
    ...
    if yourpartycount>1
      miniscore*=2 if @attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock]
    ...
    if theirpartycount>1
      minimini*=0.5 if @opponent.pbOwnSide.effects[:StealthRock] || @opponent.pbOwnSide.effects[:FoundryStealthRock]
  ...
  def volcanoeruptioncode
    ...
    miniscore = 1.0
    if PBTypes.twoTypeEff(:FIRE,@attacker.type1,@attacker.type2) == 0 || [:MAGMAARMOR,:FLASHFIRE,:FLAREBOOST,:BLAZE,:FLAMEBODY,:SOLIDROCK,:STURDY,:BATTLEARMOR,:SHELLARMOR,:WATERBUBBLE,:MAGICGUARD,:WONDERGUARD,:PRISMARMOR,:BLAZEBOOST].include?(@attacker.ability) || @attacker.effects[:AquaRing]
      miniscore*=1.0
    ...
    if !@attacker.pbPartner.pokemon.nil?
      if PBTypes.twoTypeEff(:FIRE,@attacker.pbPartner.type1,@attacker.pbPartner.type2) == 0 || [:MAGMAARMOR,:FLASHFIRE,:FLAREBOOST,:BLAZE,:FLAMEBODY,:SOLIDROCK,:STURDY,:BATTLEARMOR,:SHELLARMOR,:WATERBUBBLE,:MAGICGUARD,:WONDERGUARD,:PRISMARMOR,:BLAZEBOOST].include?(@attacker.pbPartner.ability) || @attacker.pbPartner.effects[:AquaRing]
        miniscore*=1.0
    ...
    minimini1 = 1.0
    if yourpartycount>1
      minimini1*=1.5 if @attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock]
    ...
    minimini2 = 1.0
    if theirpartycount>1
      minimini2*=0.75 if @opponent.pbOwnSide.effects[:StealthRock] || @opponent.pbOwnSide.effects[:FoundryStealthRock]
  ...
  # Resurgence - Jet Stream
  def jetstreamcode
    return 0 if @attacker.pbOwnSide.effects[:JetStream] > 0

    miniscore = 1.0
    miniscore *= 1.4 if @mondata.roles.include?(:TANK)
    miniscore *= 0.5 if !@battle.doublebattle || @attacker.pbPartner.hp==0
    if pbAIfaster?()
      miniscore *= 0.9
      miniscore *= 0.4 if @attacker.pbNonActivePokemonCount == 0
    end
  ...
  def opptypechangecode(type)
    ...
    # Gen 9 Mod - Added Ogerpon forms 8 or higher since Terastalized Ogerpon's type cannot be changed.
    # Gen 9 Mod - Added Terapagos form 2 since Terastalized Terapagos's type cannot be changed.
    return 0 if @opponent.ability == :MULTITYPE || @opponent.ability == :RKSSYSTEM || @attacker.crested == :SILVALLY || @opponent.ability == :PROTEAN || @opponent.ability == :LIBERO || @opponent.ability == :COLORCHANGE || (@opponent.ability == :DOWNLOAD && @battle.FE == :DIMENSIONAL) || (@opponent.species == :OGERPON && @opponent.form >= 8) || (@opponent.species == :TERAPAGOS && @opponent.form == 2)
  ...
  def abilitychangecode(ability)
    # Resurgence - Delta Avalugg Crest preventing Ability Changing/Swapping/Copying
    return 0 if @opponent.ability == ability || (PBStuff::FIXEDABILITIES).include?(@opponent.ability) || @opponent.crested == :AVALUGG
    # Gen 9 Mod - Added Ability Shield
  ...
  def roleplaycode # Role Play
    ...
    # Resurgence - Delta Avalugg Crest preventing Ability Changing/Swapping/Copying
    return 0 if @opponent.crested == :AVALUGG
    # Gen 9 Mod - Added Ability Shield
  ...
  def skillswapcode
    ...
    # Resurgence - Delta Avalugg Crest preventing Ability Changing/Swapping/Copying
    return 0 if @opponent.crested == :AVALUGG || @attacker.crested == :AVALUGG
  ...
  def ohkode
    ...
    return 3.5 if @opponent.effects[:LockOn]>0 || (@attacker.ability==:FAIRYAURA && @battle.FE == :FAIRYTALE)
    return 0.7
  end
  ...
  def helpinghandcode
    ...
    # Gen 9 Mod - Discourage helping hand when partner has Good as Gold
    return 0 if @attacker.pbPartner.ability == :GOODASGOLD && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))
    miniscore = 1.0
  ...
  def screencode
    ...
    # Resurgence - Fortify
    miniscore *= 1.1 if (@mondata.attitemworks && @attacker.item == :LIGHTCLAY) || (@mondata.skill >= BESTSKILL && @battle.FE == :MIRROR) || @attacker.ability == :FORTIFY
    if pbAIfaster?(@move)
      miniscore *= 1.1
      if @mondata.skill >= MEDIUMSKILL
    ...
    if livecount <= 2
      ...
      # Resurgence - Fortify
      miniscore *= 1.4 if (@mondata.attitemworks && @attacker.item == :LIGHTCLAY) || @attacker.ability == :FORTIFY
    end
  ...
  def forecode5me #after doing hundreds of these this is how i survive
    ...
    if @opponent.hasType?(:GHOST)
      miniscore*=1.5
      # Gen 9 Mod - Added Mind's Eye
      miniscore*=5 if !(@attacker.ability == :SCRAPPY || @attacker.ability == :MINDSEYE) && !@attacker.moves.any? {|moveloop| moveloop!=nil && moveloop.basedamage > 0 && moveloop.pbType(@attacker) != :NORMAL && moveloop.pbType(@attacker) != :FIGHTING}
    end
    return miniscore
  end
  ...
  def protectcode
    ...
    if @opponent.status== :POISON || @opponent.status== :BURN || @opponent.status== :FROSTBITE || @opponent.effects[:SaltCure] # Gen 9 Mod - Added Salt Cure
      miniscore*=1.2
    ...
    if @attacker.status== :POISON || @attacker.status== :BURN || @attacker.status== :FROSTBITE || @attacker.effects[:SaltCure] # Gen 9 Mod - Added Salt Cure
      miniscore*=0.7
    ...
    miniscore*=0.3 if @opponent.status== :SLEEP
    ...
    if @move.function == 0x133 || @move.function == 0x188 # obstruct now
      miniscore*=0.1 if checkAImoves([:WILLOWISP,:THUNDERWAVE,:TOXIC,:HYPOTHERMIA])
    end
    return miniscore
  end
  ...
  def feintcode
    ...
    return 1 if @opponent.ability == :FLICKER && @opponent.effects[:Flicker] == 3
    miniscore = 1.1
  ...
  def tormentcode # Gen 9 Mod - Added myceliumMightCheck
    return 0 if @opponent.effects[:Torment] || ((@opponent.ability == :AROMAVEIL || @opponent.pbPartner.ability == :AROMAVEIL) && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker)))
    ...
    if oldmove!=-1 && oldmove.basedamage > 0
      ...
      miniscore*=1.5 if @attacker.moves.any? {|moveloop| moveloop!=nil && (PBStuff::PROTECTMOVE).include?(moveloop.move)} && !unseenFistCheck(@opponent) # Gen 9 Mod - Method that also checks Punching Glove
  ...
  def disablecode # Gen 9 Mod - Added myceliumMightCheck
    return 0 if @opponent.effects[:Disable]>0 || ((@opponent.ability == :AROMAVEIL || @opponent.pbPartner.ability == :AROMAVEIL) && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker)))
    if @opponent.lastMoveUsed.is_a?(Symbol)
      oldmove = PokeBattle_Move.pbFromPBMove(@battle,PBMove.new(@opponent.lastMoveUsed.intern),@opponent)
    else
      oldmove = -1
  ...
  def encorecode # Gen 9 Mod - Added myceliumMightCheck
    return 0 if @opponent.effects[:Encore]>0 || ((@opponent.ability == :AROMAVEIL || @opponent.pbPartner.ability == :AROMAVEIL) && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker)))
    # Gen 9 Mod - Discourage status moves when current opponent has Good as Gold
  ...
  def multihitcode
    miniscore = 1.0
    miniscore*=0.7 if @move.contactMove? && ((@mondata.oppitemworks && @opponent.item == :ROCKYHELMET) || @opponent.ability == :IRONBARBS || @opponent.ability == :ROUGHSKIN || (@opponent.ability == :SLAG && @battle.FE != :UNDERWATER)) && !(@mondata.attitemworks && (@attacker.item == :PROTECTIVEPADS || (@attacker.item == :PUNCHINGGLOVE && @move.punchMove?))) # Gen 9 Mod - Added Protective Pads and Punching Gloves/ attitemworks
    miniscore*=0.7 if !@move.contactMove? && @opponent.ability == :PRISMGUARD # Resurgence - Prism Guard
    ...
    miniscore*=0.5 if @attacker.ability == :TERASHELL
    return miniscore
  end
  ...
  def hypercode()
    ...
    # Resurgence - Prefer using Recharge Moves for Crested Delta Golem
    miniscore*=2.0 if @attacker.crested == :GOLEM
    if @initial_scores[@score_index] < 100
  ...
  def weaselslashcode
    ...
    miniscore*=0.5 if @battle.doublebattle
    if @move.basedamage > 0
      miniscore*=0.1 if checkAImoves(PBStuff::PROTECTMOVE) && !(@move.contactMove? && unseenFistCheck(@attacker)) # Gen 9 Mod - Method that also checks Punching Glove
  ...
  def twoturncode
    ...
    if @opponent.status== :POISON || @opponent.status== :BURN || @opponent.status== :FROSTBITE || @opponent.effects[:LeechSeed]>=0 || @opponent.effects[:MultiTurn]>0 || @opponent.effects[:Curse]
      miniscore*=1.2
    else
  ...
  def cooldownmovecode # Blood Moon, Gigaton Hammer
    miniscore = 1.0
    return 1.0 if @attacker.ability == :JUGGERNAUT # Resurgence - LAWDS Juggernaut added
    miniscore *= 0.7 if @opponent.effects[:Substitute] > 0
  ...
  def firespincode()
    ...
    miniscore*=1.2 if @mondata.roles.include?(:PHYSICALWALL) || @mondata.roles.include?(:SPECIALWALL)
    miniscore*=1.1 if @attacker.moves.any? {|moveloop| moveloop!=nil && (PBStuff::PROTECTMOVE).include?(moveloop.move)} && !unseenFistCheck(@opponent) # Gen 9 Mod - Method that also checks Punching Glove
  ...
  def recovercode(amount=@attacker.totalhp/2.0)
    ...
    return 0 if @attacker.effects[:Wish]>0 || @attacker.effects[:Wish2]>0
    miniscore = 1.0
    ...
    if (@attacker.hp.to_f)/@attacker.totalhp<0.5
      ...
      if @attacker.hp*4<@attacker.totalhp
        ...
        if @attacker.hp<@attacker.totalhp*0.13
          miniscore*=2 if @attacker.status== :BURN || @attacker.status== :FROSTBITE
          # Gen 9 Mod - Hail/Snow/Both consider HAIL only when it is damaging
          # Resurgence - Crystal-typing Added to Sandstorm Immunity
          miniscore*=2 if (@battle.pbWeather== :HAIL && !@attacker.hasType?(:ICE) && HAILSNOWMOD != "Snow") || (@battle.pbWeather== :SANDSTORM && !@attacker.hasType?(:ROCK) && !@attacker.hasType?(:GROUND) && !@attacker.hasType?(:STEEL) && !@attacker.hasType?(:CRYSTAL)) || (@battle.pbWeather== :SHADOWSKY && !@attacker.hasType?(:SHADOW) && !@attacker.hasType?(:DARK) && !@attacker.hasType?(:GHOST))
        end
      end
    else
      miniscore*=0.9
    ...
    # Gen 9 Mod - Recovering is discentivized if Salt Cured
    if @attacker.effects[:SaltCure]
      miniscore*=0.8 if @attacker.ability != :MAGICGUARD
      miniscore*=0.8 if @attacker.ability != :MAGICGUARD && (@attacker.hasType?(:WATER) || @attacker.hasType?(:STEEL))
    end
    miniscore*=1.1 if @attacker.status== :PARALYSIS || @attacker.effects[:Attract]>=0 || @attacker.effects[:Confusion]>0
    if @opponent.status== :POISON || @opponent.status== :BURN || @opponent.status== :FROSTBITE || @opponent.effects[:LeechSeed]>=0 || @opponent.effects[:Curse]
      miniscore*=1.3
      miniscore*=1.3 if @opponent.effects[:Toxic]>0
    end
  ...
  def wishcode
    ...
    # Resurgence always start wish a wish
    if @attacker.ability == :PERIODICORBIT
      miniscore*=100
    else
      if @attacker.moves.any? {|moveloop| moveloop!=nil && (PBStuff::PROTECTMOVE).include?(moveloop.move)} && !unseenFistCheck(@opponent) # Gen 9 Mod - Method that also checks Punching Glove # if we have protect
  ...
  def restcode
    ...
    miniscore*=1.2 if (@mondata.roles.include?(:PHYSICALWALL) || @mondata.roles.include?(:SPECIALWALL))
    if @opponent.status== :POISON || @opponent.status== :BURN || @opponent.status== :FROSTBITE || @opponent.effects[:LeechSeed]>=0 || @opponent.effects[:Curse] || @opponent.effects[:SaltCure]
      miniscore*=1.3
      miniscore*=1.3 if @opponent.effects[:Toxic]>0
      # Gen 9 Mod - Additional Checks for Salt Cure
      miniscore*=1.2 if @opponent.effects[:SaltCure] && @opponent.ability != :MAGICGUARD
  ...
  def aquaringcode
    ...
    miniscore*=1.2 if @attacker.moves.any? {|moveloop| moveloop!=nil && (PBStuff::PROTECTMOVE).include?(moveloop.move)} && !unseenFistCheck(@opponent) # Gen 9 Mod - Method that also checks Punching Glove
  ...
  def healpulsecode
    ...
    if @opponent.status== :POISON || @opponent.status== :BURN || @opponent.status== :FROSTBITE || @opponent.effects[:LeechSeed]>=0 || @opponent.effects[:Curse] || @opponent.effects[:SaltCure]
      ...
      miniscore*=0.9 if @opponent.effects[:SaltCure] && @opponent.ability != :MAGICGUARD
      miniscore*=0.9 if @opponent.effects[:SaltCure] && @opponent.ability != :MAGICGUARD && (@opponent.hasType?(:WATER) || @opponent.hasType?(:STEEL))
    end
    return miniscore
  end
  ...
  def deathcode
    ...
    miniscore*=0.3 if checkAImoves(PBStuff::PROTECTMOVE) && !(@move.contactMove? && unseenFistCheck(@attacker)) # Gen 9 Mod - Method that also checks Punching Glove
  ...
  def endurecode
    ...
    # Gen 9 Mod - Hail/Snow/Both consider HAIL only when it is damaging
    # Resurgence - Crystal-typing Added to Sandstorm Immunity
    return 0 if (@battle.pbWeather== :HAIL && !@attacker.hasType?(:ICE) && HAILSNOWMOD != "Snow") || (@battle.pbWeather== :SANDSTORM && !(@attacker.hasType?(:ROCK) || @attacker.hasType?(:GROUND) || @attacker.hasType?(:STEEL) || @attacker.hasType?(:CRYSTAL))) || (@battle.pbWeather== :SHADOWSKY && !@attacker.hasType?(:SHADOW) && !@attacker.hasType?(:DARK) && !@attacker.hasType?(:GHOST))
    return 0 if @attacker.status== :POISON || @attacker.status== :BURN || @attacker.status== :FROSTBITE || @attacker.effects[:LeechSeed]>=0 || @attacker.effects[:Curse]
    ...
    miniscore*=1.5 if @opponent.status== :POISON || @opponent.status== :BURN || @opponent.status== :FROSTBITE || @opponent.effects[:LeechSeed]>=0 || @opponent.effects[:Curse]
    miniscore/=(@attacker.effects[:ProtectRate]*2.0) if @attacker.effects[:ProtectRate] > 0
    return miniscore
  end
  ...
  def phasecode
    ...
    miniscore*=(@opponent.pbOwnSide.effects[:StealthRock] || @opponent.pbOwnSide.effects[:FoundryStealthRock]) ? 1.3 : 0.8
  ...
  def pivotcode
    ...
    miniscore=1.0
    miniscore*=0.7 if @attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock]
    ...
    else    #U-turn / Volt Switch / Parting Shot / Gen 9 Mod - Added Flip Turn
  ...
  def knockcode # Gen 9 Mod - Added myceliumMightCheck
    return @move.basedamage > 0 ? 1 : 0 if @opponent.effects[:Substitute]>0
    return @move.basedamage > 0 ? 1 : 0 unless (@opponent.ability != :STICKYHOLD || (moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))) && @opponent.item && !@battle.pbIsUnlosableItem(@opponent,@opponent.item)
    if @opponent.item == :LEFTOVERS || (@opponent.item == :BLACKSLUDGE) && @opponent.hasType?(:POISON)
      ...
      return 1.2
    # Gen 9 Mod - Added Cud Chew --> Add Ripen and Cud Chew to the knock off code
    elsif pbIsBerry?(@opponent.item) && (@opponent.ability == :HARVEST || @opponent.ability == :RIPEN || @opponent.ability == :CUDCHEW)
      if @opponent.crested == :CRUSTLE
        return 1.6
      else
        return 1.3
      end
  ...
  def covetcode # Gen 9 Mod - Added myceliumMightCheck
    return @move.basedamage > 0 ? 1 : 0 if @opponent.ability == :STICKYHOLD && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))
    ...
    case @opponent.item
      ...
      when :TOXICORB, :FLAMEORB, :FROSTORB, :LAGGINGTAIL, :IRONBALL, :STICKYBARB
        miniscore*=0.5
    end
    return miniscore
  end

  def bestowcode # Gen 9 Mod - Added myceliumMightCheck
    return 0 if ((@opponent.ability == :STICKYHOLD || @opponent.ability == :GOODASGOLD) && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker)))
  ...
  def recoilcode
    ...
    miniscore = 0.9
    # Resurgence - Delta Scrafty's Crest should make AI prefer Recoil Moves
    miniscore *= 1.2 if @attacker.crested == :SCRAFTY
    ...
    miniscore *= 0.4 if @initial_scores[@score_index] * recoilamount > @attacker.hp && @opponent.status == :SLEEP
    return miniscore
  end
  ...
  def suncode
    ...
    miniscore*=1.5 if @attacker.pbHasMove?(:MOONLIGHT) || @attacker.pbHasMove?(:SYNTHESIS) || @attacker.pbHasMove?(:MORNINGSUN) || @attacker.pbHasMove?(:GROWTH) || @attacker.pbHasMove?(:SOLARBEAM) || @attacker.pbHasMove?(:SOLARBLADE) || @attacker.pbHasMove?(:HYDROSTEAM) # Gen 9 Mod - Added Hydro Steam
    ...
    # Gen 9 Mod - Added Protosynthesis & Orichalcum Pulse
  ...
  def raincode
    ...
    miniscore*=1.3 if @attacker.ability == :SUPERCELL
    ...
    # Gen 9 Mod - Added Electro Shot
    miniscore*=1.5 if @attacker.pbHasMove?(:THUNDER) || @attacker.pbHasMove?(:HURRICANE) || @attacker.pbHasMove?(:ELECTROSHOT)
  ...
  def sandcode
    ...
    # Resurgence - Crystal-typing Added to Sandstorm Immunity
    miniscore*= (@attacker.hasType?(:ROCK) || @attacker.hasType?(:GROUND) || @attacker.hasType?(:STEEL) || @attacker.hasType?(:CRYSTAL)) ? 1.3 : 0.7
  ...
  def hailcode
    ...
    if @attacker.ability == :SLUSHRUSH || @attacker.crested == :EMPOLEON || (@attacker.crested == :CASTFORM && @attacker.form == 3) || @attacker.ability == :ICECLEATS # Resurgence
      miniscore*=2
      miniscore*=2 if notOHKO?(@attacker, @opponent, true)
      miniscore*=3 if seedProtection?(@attacker)
    end
    miniscore*=1.3 if [:SNOWCLOAK,:ICEBODY,:LUNARIDOL].include?(@attacker.ability) || (@attacker.ability == :ICEFACE && @attacker.form == 1)
    miniscore*=1.5 if [:PROTOCRYOPHILIC,:WINTERJOY].include?(@attacker.ability)
    miniscore*=0.5 if @attacker.pbHasMove?(:MOONLIGHT) || @attacker.pbHasMove?(:SYNTHESIS) || @attacker.pbHasMove?(:MORNINGSUN) || @attacker.pbHasMove?(:GROWTH) || @attacker.pbHasMove?(:SOLARBEAM) || @attacker.pbHasMove?(:SOLARBLADE)
  ...
  def shadowskycode
    return 0 if @battle.pbWeather== :SHADOWSKY
    miniscore = 1.0
    miniscore*=1.3 if (@mondata.attitemworks && @attacker.item == :SHADOWROCK) || [:DARKCRYSTALCAVERN,:DIMENSIONAL,:FROZENDIMENSION,:SKY,:DARKNESS1,:DARKNESS2,:DARKNESS3].include?(@battle.FE)
    miniscore*=2 if @battle.pbWeather!=0 && @battle.pbWeather!=:SHADOWSKY
    miniscore*= (@attacker.hasType?(:DARK) || @attacker.hasType?(:GHOST)) ? 1.3 : 0.7
    if @attacker.ability == :SHADOWDANCE
      miniscore*=2
      miniscore*=2 if notOHKO?(@attacker, @opponent, true)
      miniscore*=3 if seedProtection?(@attacker)
    end
    miniscore*=1.3 if @attacker.ability == :ILLUMINATE # Resurgence - Illuminate increases evasion
    miniscore*=0.5 if @attacker.pbHasMove?(:SYNTHESIS) || @attacker.pbHasMove?(:MORNINGSUN) || @attacker.pbHasMove?(:GROWTH) || @attacker.pbHasMove?(:SOLARBEAM) || @attacker.pbHasMove?(:SOLARBLADE) || @attacker.pbHasMove?(:GEOMANCY)
    miniscore*=1.3 if @attacker.ability == :SUPERCELL
  ...
  def subcode
    ...
    # Gen 9 Mod - Added Clanging Scales, Overdrive, Torch Song and Psychic Noise
  ...
  def futurecode
    return 0 if @opponent.effects[:FutureSight]>0 || @opponent.effects[:FutureSight2]>0
    miniscore=0.6
    ...
    miniscore*=1.2 if @attacker.moves.any? {|moveloop| moveloop!=nil && [:PROTECT,:DETECT,:BANEFULBUNKER,:SPIKYSHIELD,:SILKTRAP,:BURNINGBULWARK,:FALLOUTSHELTER].include?(moveloop.move) } # Gen 9 Mod - Added Silk Trap and Burning Bulwark / Resurgence - Added Fallout Shelter
    ...
    miniscore*= 2.0 if @attacker.ability == :PERIODICORBIT
    return miniscore
  end
  ...
  def dinglenugget
    ...
    miniscore*=0.5 if @attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock] || @attacker.pbOwnSide.effects[:Spikes]>0
    return miniscore
  end
  ...
  def moldbreakeronalaser
    ...
    case @opponent.ability
      ...
      when :ILLUMINATE
        miniscore*=1.1 if @battle.pbWeather!=:SHADOWSKY # Resurgence - Illuminate increases evasion
      ...
      # Resurgence - Ethereal Shroud Mold Breaker check for Contact Moves
      when :ETHEREALSHROUD
        miniscore*=3 if @move.contactMove?
      when :THICKFAT
      ...
      when :MULTISCALE
        miniscore*=1.5 if @attacker.hp==@attacker.totalhp
      when :TERASHELL # Gen 9 Mod - Tera Shell same consideration as Multiscale
        miniscore*=1.5 if @attacker.hp==@attacker.totalhp
      when :SAPSIPPER
      ...
      when :WINDFORCE, :WINDBORNETYRANT
        miniscore*=3 if @move.pbType(@attacker)==:FLYING && damcount==1
        miniscore*=2 if @move.pbType(@attacker)==:FLYING && PBTypes.twoTypeEff((:FLYING),@opponent.type1,@opponent.type2)>4
      when :SNOWCLOAK
      ...
      when :WINDPOWER # Gen 9 Mod - Added Wind Power
        miniscore*=1.3 if @move.windMove? || @attacker.pbOwnSide.effects[:Tailwind] > 0
      when :WINDRIDER # Gen 9 Mod - Added Wind Rider
        miniscore*=3 if @move.windMove? || @attacker.pbOwnSide.effects[:Tailwind] > 0
      # when :MYCELIUMMIGHT # Gen 9 Mod - Added Mycelium Might
      #   miniscore*=2 if !(@move.pbIsPhysical?(opponent) || @move.pbIsSpecial?(opponent))
      when :ICESCALES
        miniscore*=1.5 if @attacker.spatk>@attacker.attack
    end
    return miniscore
  ...
  def pussydeathcode(initialscore)
    ...
    if @attacker.ability != :MAGICGUARD && !(@attacker.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
      ...
      miniscore*=0.5 if checkAImoves(PBStuff::PROTECTMOVE) && !(@move.contactMove? && unseenFistCheck(@attacker)) # Gen 9 Mod - Method that also checks Punching Glove
      miniscore*=(1-0.1*@opponent.stages[PBStats::EVASION])
  ...
  def almostuselessmovecode
    ...
    # Gen 9 Mod - Additional Checks for Salt Cure
    miniscore*=1.1 if @opponent.effects[:SaltCure] && @opponent.ability != :MAGICGUARD
    miniscore*=1.1 if @opponent.effects[:SaltCure] && @opponent.ability != :MAGICGUARD && (@opponent.hasType?(:WATER) || @opponent.hasType?(:STEEL))
    miniscore*=1.3 if checkAImoves([:HEX])
    return miniscore
  end
  ...
  def nightmarecode
    ...
    miniscore*=6 if @battle.pbWeather == :SHADOWSKY
  ...
  def hazardcode
    ...
    if @mondata.skill >= BESTSKILL
      if !@battle.pbIsWild?
        ...
        miniscore *= 0.3 if (movecheck || checkAImovesAllOpponents([:DEFOG, :RAPIDSPIN])) && @move.basedamage == 0
      end
    elsif @mondata.skill >= MEDIUMSKILL
      miniscore *= 0.3 if checkAImovesAllOpponents([:DEFOG, :RAPIDSPIN]) && @move.basedamage == 0
    end
    return miniscore
  end
  ...
  def flowershieldcode(score)
    ...
    if @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN,2,5)|| @battle.FE == :FAIRYTALE
      ...
      miniscore*=1.3 if opp1.status== :SLEEP
  ...
  def flingcode
    ...
    case @attacker.item
      ...
      when :FLAMEORB
        if @opponent.pbCanBurn?(false) && (@opponent.ability != :GUTS || @opponent.ability != :FORTITUDE)
          miniscore *= 1.3
          miniscore *= 2 if @attacker.pbCanBurn?(false) && (@attacker.ability != :GUTS || @attacker.ability != :FORTITUDE)
        end
      when :FROSTORB
        if @opponent.pbCanFrostbite?(false) && (@opponent.ability != :GUTS || @opponent.ability != :FORTITUDE)
          miniscore *= 1.3
          miniscore *= 2 if @attacker.pbCanFrostbite?(false) && (@attacker.ability != :GUTS || @attacker.ability != :FORTITUDE)
        end
  ...
  def perishcode
    ...
    return 0 if @attacker.pbNonActivePokemonCount==0
    miniscore=1.0
    # Gen 9 Mod - Added Shed Tail and Flip Turn
    miniscore*=1.5 if @attacker.pbHasMove?(:UTURN) || @attacker.pbHasMove?(:VOLTSWITCH) || @attacker.pbHasMove?(:FLIPTURN) || @attacker.pbHasMove?(:PARTINGSHOT) || @attacker.pbHasMove?(:SHEDTAIL)
    ...
    miniscore*=0 if @opponent.ability == :GOODASGOLD && @move.category == :status && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))
    # Gen 9 Mod - Added Shed Tail and Flip Turn
    miniscore*=0.1 if (PBStuff::TRAPPINGABILITIESAI.include?(@attacker.ability) || (@attacker.ability == :MAGNETPULL && @opponent.hasType?(:STEEL))  || @attacker.effects[:MeanLook]>0) && !(@attacker.pbHasMove?(:UTURN) || @attacker.pbHasMove?(:VOLTSWITCH)|| @attacker.pbHasMove?(:FLIPTURN) || @attacker.pbHasMove?(:PARTINGSHOT)) || @attacker.pbHasMove?(:SHEDTAIL)
  ...
  def leechcode
    ...
    miniscore*=1.1 if @opponent.status== :POISON || @opponent.status== :BURN || @opponent.status== :FROSTBITE
  ...
  def rolloutcode
    ...
    miniscore*=0.8 if checkAImoves(PBStuff::PROTECTMOVE) && !(@move.contactMove? && unseenFistCheck(@attacker)) # Gen 9 Mod - Method that also checks Punching Glove
    return miniscore
  end
  ...
  def outragecode(score)
    ...
    miniscore*=0.7 if checkAImoves(PBStuff::PROTECTMOVE) && !(@move.contactMove? && unseenFistCheck(@attacker)) # Gen 9 Mod - Method that also checks Punching Glove
  ...
  def spotlightcode
    ...
    elsif battler.pbPartner.crested == :SKUNTANK || @opponent.ability == :EARTHEATER # Gen 9 Mod - Added Earth Eater
      miniscore*=3 if bestmove1.pbType(@attacker.pbOpposing1) ==:GROUND || bestmove2.pbType(@attacker.pbOpposing2) ==:GROUND
    elsif @opponent.ability == :WINDFORCE || @opponent.ability == :WINDBORNETYRANT # Resurgence - Added Wind Force and Windborne Tyrant
      miniscore*=3 if bestmove1.pbType(@attacker.pbOpposing1) ==:FLYING || bestmove2.pbType(@attacker.pbOpposing2) ==:FLYING
    end
    # Resurgence - Added Fallout Shelter
    miniscore*=2 if (bestmove1.contactMove? || bestmove2.contactMove?) && checkAImoves([:KINGSSHIELD, :BANEFULBUNKER, :SPIKYSHIELD, :SILKTRAP, :BURNINGBULWARK,:FALLOUTSHELTER]) # Gen 9 Mod - Added Silky Trap and Burning Bulwark
  ...
  def doodlecode # Gen 9 Mod - Doodle Code
    ...
    # Resurgence - Delta Avalugg Crest preventing Ability Changing/Swapping/Copying
    return 0 if @opponent.crested == :AVALUGG || @attacker.crested == :AVALUGG || @attacker.pbPartner.crested == :AVALUGG
    miniscore = getAbilityDisruptScore(@opponent,@attacker)
  ...
  def saltcurecode # Salt Cure
    return 1 if (@opponent.effects[:SaltCure] || secondaryEffectNegated?())
      miniscore=1.3
      miniscore*=3 if @opponent.hasType?(:WATER) || @opponent.hasType?(:STEEL)
    return miniscore
  end
  ...
  def dragoncheercode # Dragon Cheer - Modified version of focusenergycode that is affected by us having a
                      # Dragon type ally, and checks for effects on our ally instead of us.
    partnerDragon = @attacker.pbPartner.hasType?(:DRAGON)
    return 0 if @attacker.pbPartner.hp==0 || !@battle.doublebattle
    return 0 if @attacker.pbPartner.effects[:FocusEnergy]>0
    miniscore = 1.0
    miniscore*=4.0 if partnerDragon
    ...
    miniscore*=1.3 if @opponent.status== :SLEEP
    miniscore*=1.5 if @opponent.effects[:Encore]>0 && @opponent.moves[(@opponent.effects[:EncoreIndex])].basedamage==0
    miniscore*=2 if @attacker.pbPartner.ability == :SUPERLUCK || @attacker.pbPartner.ability == :SNIPER || @attacker.pbPartner.ability == :SUBLIMATINGEDGE
...
######################################################
  ...
  def pbGetMonRoles(targetmon=nil)
    ...
    for mon in party
      ...
      for i in movelist
        ...
        weathermove=true if (i == :SUNNYDAY || i == :RAINDANCE || i == :HAIL || i == :SANDSTORM || i == :SHADOWSKY || i == :SHADOWPRAYER)
      ...
      # Resurgence - Fortify
      monRoles.push(:SCREENER)   if (mon.item==(:LIGHTCLAY) || mon.ability == :FORTIFY) && screenmove
      # Gen 9 Mod - Added Hospitality
      ...
      monRoles.push(:STATUSABSORBER) if restmove || (mon.ability == :COMATOSE) || mon.item==(:TOXICORB) || mon.item==(:FLAMEORB) || mon.item==(:FROSTORB) || (mon.ability == :GUTS) || (mon.ability == :FORTITUDE) || (mon.ability == :QUICKFEET)|| (mon.ability == :FLAREBOOST) || (mon.ability == :TOXICBOOST) || (mon.ability == :NATURALCURE) || (mon.ability == :MAGICGUARD) || (mon.ability == :MAGICBOUNCE) || (mon.species == :ZANGOOSE && mon.item == :ZANGCREST) || hydrationCheck(mon) || (mon.ability == :GOODASGOLD) # Gen 9 Mod - Added Good as Gold
      monRoles.push(:TRAPPER)     if PBStuff::TRAPPINGABILITIES.include?(mon.ability)
      # Gen 9 Mod - Added Orichalcum Pulse
      monRoles.push(:WEATHERSETTER) if weathermove || (mon.ability == :DROUGHT) || (mon.ability == :SANDSPIT)  || (mon.ability == :SANDSTREAM) || (mon.ability == :DRIZZLE) || (mon.ability == :SNOWWARNING) || (mon.ability == :PRIMORDIALSEA) || (mon.ability == :DESOLATELAND) || (mon.ability == :DELTASTREAM) || (mon.ability == :ORICHALCUMPULSE) || (mon.ability == :NOCTEM) || (mon.ability == :SLEET)
      # Gen 9 Mod - Added Seed Sower & Hadron Engine
      monRoles.push(:FIELDSETTER)   if fieldmove || (mon.ability == :GRASSYSURGE) || (mon.ability == :ELECTRICSURGE) || (mon.ability == :MISTYSURGE) || (mon.ability == :PSYCHICSURGE) || mon.item==(:AMPLIFIELDROCK)|| (mon.ability == :DARKSURGE) || (mon.ability == :SEEDSOWER) || (mon.ability == :HADRONENGINE)
  ...
  def pbRoughAccuracy(move,attacker,opponent)
    ...
    accstage=0 if opponent.ability == :UNAWARE && !(moldBreakerCheck(attacker) || myceliumMightCheck(attacker, move)) # Gen 9 Mod - Added myceliumMightCheck
    ...
    evastage=0 if opponent.effects[:Foresight] || opponent.effects[:MiracleEye] || move.function==0xA9 || attacker.ability == :UNAWARE && !(moldBreakerCheck(opponent) || myceliumMightCheck(opponent, move)) # Gen 9 Mod - Added myceliumMightCheck
    ...
    if @mondata.skill>=MEDIUMSKILL
      ...
      if @mondata.skill>=BESTSKILL
        ...
        accuracy*= @battle.FE == :RAINBOW ? 0 : 0.5 if opponent.ability == :WONDERSKIN && @basedamage==0 && attacker.pbIsOpposing?(opponent.index) && !(moldBreakerCheck(attacker) || myceliumMightCheck(attacker, move)) # Gen 9 Mod - Added myceliumMightCheck
        accuracy*=0.5 if Rejuv && @battle.FE == :PSYTERRAIN && opponent.ability == :MAGICIAN && @basedamage==0 && attacker.pbIsOpposing?(opponent.index) && !(moldBreakerCheck(attacker) || myceliumMightCheck(attacker, move)) # Gen 9 Mod - Added myceliumMightCheck
        evasion*=1.2 if opponent.ability == :TANGLEDFEET && opponent.effects[:Confusion]>0 && !(moldBreakerCheck(attacker) || myceliumMightCheck(attacker, move)) # Gen 9 Mod - Added myceliumMightCheck
        evasion*=1.2 if (@battle.pbWeather== :SANDSTORM || @battle.FE == :DESERT || @battle.FE == :ASHENBEACH) && opponent.ability == :SANDVEIL && !(moldBreakerCheck(attacker) || myceliumMightCheck(attacker, move)) # Gen 9 Mod - Added myceliumMightCheck
        evasion*=1.2 if (@battle.pbWeather== :HAIL || @battle.FE == :ICY || @battle.FE == :SNOWYMOUNTAIN) && opponent.ability == :SNOWCLOAK && !(moldBreakerCheck(attacker) || myceliumMightCheck(attacker, move)) # Gen 9 Mod - Added myceliumMightCheck
      end
      ...
      if attacker.pbPartner.itemWorks?
        if attacker.pbPartner.item == :MICLEBERRY && attacker.pbPartner.crested == :CRUSTLE
          accuracy*=1.2 if (attacker.pbPartner.ability == :GLUTTONY && attacker.pbPartner.hp<=(attacker.pbPartner.totalhp/2.0).floor) || attacker.pbPartner.hp<=(attacker.pbPartner.totalhp/4.0).floor
        end
      end
    end
    evasion = 100 if attacker.ability == :KEENEYE || attacker.ability == :MINDSEYE # Gen 9 Mod - Added Mind's Eye
    evasion = 100 if @mondata.skill >= BESTSKILL && @battle.FE == :ASHENBEACH && [:OWNTEMPO, :INNERFOCUS, :PUREPOWER, :SANDVEIL, :STEADFAST, :FORTITUDE].include?(attacker.ability) && ![:UNNERVE, :ASONECHILLING, :ASONEGRIM].include?(opponent.ability)
  ...
  def pbAIfaster?(attackermove = nil, opponentmove = nil, attacker = @attacker, opponent = @opponent)
    ...
    for battler in [attacker, opponent]
      ...
      priorityarray[index][1] = 1 if (battler.hasWorkingItem(:CUSTAPBERRY) && ((battler.ability == :GLUTTONY && battler.hp <= (battler.totalhp / 2.0).floor) || battler.hp <= (battler.totalhp / 4.0).floor)) || battler.partnercustap
      ...
      pri += 1 if battler.pbOwnSide.effects[:JetStream] == 1 # Resurgence - Jet Stream
      pri += 2 if battler.pbOwnSide.effects[:JetStream] == 1 && @battle.FE == :SKY # Resurgence - Jet Stream
  ...
  def pbMoveOrderAI #lol it's just pbPriority
    ...
    for i in 0..3
      ...
      priorityarray[i][1] = 1 if (battler.hasWorkingItem(:CUSTAPBERRY) && ((battler.ability == :GLUTTONY && battler.hp<=(battler.totalhp/2.0).floor) || battler.hp<=(battler.totalhp/4.0).floor)) || battler.partnercustap
  ...
  def hpGainPerTurn(attacker=@attacker)
    ...
    if attacker.ability != :MAGICGUARD && !(attacker.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
      ...
      healing -= 0.125 if @battle.FE == :HOLY &&  attacker.ability == :HELIOPHOBIA # Resurgence - Blessed Field damages Heliophobia

      # weather induced
      healing -= 0.125 if @battle.pbWeather == :SUNNYDAY && (attacker.ability == :SOLARPOWER || attacker.ability == :DRYSKIN)
      healing -= 0.125 if ([:FACTORY, :SHORTCIRCUIT, :ELECTERRAIN].include?(@battle.FE) || [:RAINDANCE, :SHADOWSKY].include?(@battle.pbWeather)) && attacker.ability == :SUPERCELL # Resurgence
      healing -= 0.125 if @battle.pbWeather == :SUNNYDAY && (attacker.crested == :CASTFORM && attacker.form == 1)
      # Resurgence - Crystal-typing Added to Sandstorm Immunity
      healing -= (Rejuv && @battle.FE == :DESERT) ? 0.125 : 0.0625 if @battle.pbWeather == :SANDSTORM && (!(attacker.hasType?(:GROUND) || attacker.hasType?(:ROCK) || attacker.hasType?(:STEEL) || attacker.hasType?(:CRYSTAL) || [:SANDFORCE,:SANDRUSH,:SANDVEIL,:MAGICGUARD,:OVERCOAT,:TEMPEST,:SANDSTREAM].include?(attacker.ability)) || attacker.effects[:DesertsMark])
      # Gen 9 Mod - Hail/Snow/Both
      #healing -= (@battle.FE == :FROZENDIMENSION) ? 0.125 : 0.0625 if @battle.pbWeather == :HAIL && !(attacker.hasType?(:ICE) || [:SNOWCLOAK,:ICEBODY,:LUNARIDOL,:SLUSHRUSH,:MAGICGUARD,:OVERCOAT,:TEMPEST,:ICECLEATS,:SNOWWARNING,:SLEET].include?(attacker.ability) || HAILSNOWMOD == "Snow") # Resurgence added Ice Cleats and Snow Warning
      if @battle.pbWeather == :HAIL && !(attacker.hasType?(:ICE) || [:SNOWCLOAK,:ICEBODY,:LUNARIDOL,:SLUSHRUSH,:MAGICGUARD,:OVERCOAT,:TEMPEST,:ICECLEATS,:SNOWWARNING,:SLEET].include?(attacker.ability) || HAILSNOWMOD == "Snow") # Resurgence added Ice Cleats and Snow Warning
        if @battle.FE == :FROZENDIMENSION && @battle.pbCheckGlobalAbility(:SLEET)
          healing -= 0.166
        elsif @battle.FE == :FROZENDIMENSION || @battle.pbCheckGlobalAbility(:SLEET)
          healing -= 0.125
        else
          healing -= 0.0625
        end
      end
      healing -= (@battle.FE == :DIMENSIONAL || @battle.FE == :FROZENDIMENSION) ? 0.125 : 0.0625 if (@battle.pbWeather== :SHADOWSKY && (!@attacker.hasType?(:SHADOW) || !@attacker.hasType?(:DARK) || !@attacker.hasType?(:GHOST)))
      ...
      healing -= 0.0625 if attacker.status == :BURN || attacker.status == :FROSTBITE
      if attacker.status == :SLEEP || (attacker.ability == :COMATOSE && @battle.FE != :ELECTERRAIN)
        ...
        if (attacker.pbOpposing1.ability == :BADDREAMS || attacker.pbOpposing2.ability == :BADDREAMS) && @battle.FE != :RAINBOW
          if @battle.FE == :INFERNAL || @battle.FE == :DARKNESS3 || @battle.pbWeather == :SHADOWSKY
            sleepdmg += 0.25
      ...
      if attacker.effects[:Nightmare] && attacker.status == :SLEEP
        if @battle.FE != :RAINBOW
          healing -= (@battle.FE == :DARKNESS3 || @battle.FE == :HAUNTED) ? 0.33 : 0.25
        end
        healing -= 0.5 if @battle.pbWeather == :SHADOWSKY
      end      
      ...
      healing -= 0.125 if attacker.effects[:SaltCure] && (attacker.hasType?(:WATER) || attacker.hasType?(:STEEL))
    ...
    healing += 0.0625 if attacker.ability == :HELIOPHOBIA && (@battle.pbWeather== :SHADOWSKY || @battle.FE == :DARKCRYSTALCAVERN || @battle.FE == :DARKNESS1 || @battle.FE == :DARKNESS2 || @battle.FE == :DARKNESS3 || @battle.FE == :SHORTCIRCUIT || @battle.FE == :DIMENSIONAL)
  ...
  def illusionCheck(attacker = @attacker, opponent = @opponent)
    return false if opponent.index == attacker.pbPartner.index
    ...
    for i in @battle.pbPartySingleOwner(opponent.index)
      ...
      faintedcount += 1 if i.hp == 0
    end
    if @mondata.skill >= BESTSKILL
      deceived = opponent.turncount <= 1 && faintedcount <= 2
      if $cache.moves[opponent.lastMoveTaken]
        move = PokeBattle_Move.pbFromPBMove(@battle, PBMove.new(opponent.lastMoveTaken.intern), @attacker)
        typemod = move.pbTypeModifier(move.type, attacker, opponent, false)
        typemod = move.irregularTypeMods(attacker, opponent, typemod, move.type)
        ...
        deceived = false if opponent.turncount >= 0 && !attacker.missAcc && !attacker.effects[:Tantrum] && move.basedamage > 0 && typemod == 0
      end
    elsif @mondata.skill >= MEDIUMSKILL
      deceived = faintedcount <= 4
    else
      deceived = true
    end
    return deceived
  ...
  def pbTypeModNoMessages(type=@move.type,attacker=@attacker,opponent=@opponent,move=@move,skill=@mondata.skill)
    ...
    if !(moldBreakerCheck(attacker) || myceliumMightCheck(attacker, move)) && !opponent.hasWorkingItem(:ABILITYSHIELD) && !(move.isSoundBased? && attacker.ability==:LIQUIDVOICE) # Gen 9 Mod - Added myceliumMightCheck
      # Resurgence - Delta Avalugg Crest calling all of it's Abilities using LAWDS' P3 Code
      abil = []
      if opponent.ability.is_a?(PokeAbility)
			  if opponent.crested == :AVALUGG || opponent.ability.ability.is_a?(Array)
				  for i in opponent.ability.ability
				    abil.push(i)
				  end
			  else
      ...
      case opponent.ability
        ...
        # Resurgence - Wind Force
        when :WINDFORCE,:WINDBORNETYRANT       then return -1 if type == :FLYING || (!secondtype.nil? && secondtype.include?(:FLYING))
    ...
    typemod = move.pbTypeModifier(type, attacker, opponent)
  ...
  def pbChangeMove(move,attacker)
    ...
    case move.move
      when :WEATHERBALL
        ...
        move.type = :DARK if weather == :SHADOWSKY # Resurgence - Shadow Sky
      ...
      when :IVYCUDGEL
        move.type = :WATER if (attacker.species == :OGERPON && attacker.item == :WELLSPRINGMASK)
        move.type = :FIRE if (attacker.species == :OGERPON && attacker.item == :HEARTHFLAMEMASK)
        move.type = :ROCK if (attacker.species == :OGERPON && attacker.item == :CORNERSTONEMASK)
      when :RAGINGBULL
        move.type = :FIGHTING if (attacker.species == :TAUROS && attacker.form == 1)
        move.type = :FIRE if (attacker.species == :TAUROS && attacker.form == 2)
        move.type = :WATER if (attacker.species == :TAUROS && attacker.form == 3)
      when :TERASTARSTORM
        move.type = :STELLAR if (attacker.species == :TERAPAGOS && attacker.form == 2)
    end
    return move
  end
  ...
  def totalHazardDamage(pkmn)
    ...
    if pkmn.pbOwnSide.effects[:StealthRock] && !pkmn.ability == :MAGICGUARD && !(pkmn.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM) && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS)
      ...
      if @mondata.skill>BESTSKILL && @battle.FE == :CRYSTALCAVERN
        ...
        eff=PBTypes.twoTypeEff(:FIRE,pkmn.type1,pkmn.type2) # Gen 9 Mod - Ported fix from Reborn 19.5
      end
      if eff>0
    ...
    if pkmn.pbOwnSide.effects[:FoundryStealthRock] && !pkmn.ability == :MAGICGUARD && !(pkmn.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM) && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS)
      ...
      if @mondata.skill>BESTSKILL && @battle.FE == :CRYSTALCAVERN
        ...
        eff=PBTypes.twoTypeEff(:POISON,pkmn.type1,pkmn.type2)
      elsif @mondata.skill>BESTSKILL && (@battle.FE == :ROCKY || @battle.FE == :CAVE)
        eff=PBTypes.twoTypeEff(:ROCK,pkmn.type1,pkmn.type2) # Gen 9 Mod - Ported fix from Reborn 19.5
      end
      if eff>0
        eff*=2 if @mondata.skill>BESTSKILL && (@battle.FE == :VOLCANICTOP || @battle.FE == :INFERNAL || (Rejuv && @battle.FE == :DRAGONSDEN))
        percentdamage += 100*(eff/32.0)
      end
    end
    if @mondata.skill>=BESTSKILL
  ...
  def getAbilityDisruptScore(attacker,opponent)
    ...
    return 0 if PBStuff::FIXEDABILITIES.include?(opponent.ability) || opponent.crested == :AVALUGG # Gen 9 Mod - Ported change from Reborn 19.5
    # Resurgence - Delta Avalugg Crest calling all of it's Abilities using LAWDS' P3 Code
    abil = []
		if opponent.ability.is_a?(PokeAbility)
		  if opponent.crested == :AVALUGG || opponent.ability.ability.is_a?(Array)
    ...
    case opponent.ability
      ...
      when :ILLUMINATE # Resurgence - Illuminate increases evasion
        abilityscore*=1.3 if @battle.pbWeather== :SHADOWSKY
      ...
      when :WINDFORCE,:WINDBORNETYRANT # Resurgence - Wind Force
        for i in attacker.moves
          next if i.nil?
          flyingmove=i if i.pbType(attacker) == :FLYING
        end
        if attacker.moves.any? {|moveloop| moveloop!=nil && moveloop.pbType(attacker) == :FLYING}
          abilityscore*=3 if attacker.moves.all? {|moveloop| moveloop!=nil && moveloop.pbType(attacker) == :FLYING}
          abilityscore*=2 if pbTypeModNoMessages(flyingmove.pbType(attacker),attacker,opponent,flyingmove)>4
        end
      when :SHADOWTAG
      ...
      # Resurgence - Ethereal Shroud check for Contact Moves
      when :ETHEREALSHROUD
        abilityscore*=3 if attacker.moves.all? {|moveloop| moveloop!=nil && (moveloop.contactMove? || moveloop.basedamage==0)}
      when :THICKFAT
        abilityscore*=1.5 if attacker.moves.all? {|moveloop| moveloop!=nil && (moveloop.pbType(attacker) == :FIRE || moveloop.pbType(attacker) == :ICE) }
      when :TRUANT
        abilityscore*=0.1
      when :GUTS, :FORTITUDE, :QUICKFEET, :MARVELSCALE
  ...
  def getFieldDisruptScore(attacker=@attacker,opponent=@opponent,fieldeffect=@battle.FE, violent=false)
    ...
    case fieldeffect
      ...
      when :ELECTERRAIN # Electric Terrain
        ...
        fieldscore*=1.5 if opponent.ability == :HADRONENGINE
        fieldscore*=0.7 if attacker.ability == :HADRONENGINE
      ...
      when :DARKCRYSTALCAVERN # Dark Crystal Cavern
        ...
        fieldscore*=1.3 if opponent.ability == :SHADOWDANCE && @battle.pbWeather!=:SHADOWSKY
        fieldscore*=0.7 if attacker.ability == :SHADOWDANCE && @battle.pbWeather!=:SHADOWSKY
      ...
      when :ICY # Icy field
        ...
        fieldscore*=1.3 if (opponent.ability == :SLUSHRUSH || opponent.ability == :ICECLEATS || opponent.crested == :EMPOLEON || (opponent.crested == :CASTFORM && opponent.form == 3)) && @battle.pbWeather!=:HAIL
        fieldscore*=0.7 if (attacker.ability == :SLUSHRUSH || attacker.ability == :ICECLEATS || attacker.crested == :EMPOLEON || (attacker.crested == :CASTFORM && attacker.form == 3)) && @battle.pbWeather!=:HAIL
      ...
      when :DARKNESS1 # Darkness
        fieldscore*=1.3 if opponent.ability == :SHADOWDANCE && @battle.pbWeather!=:SHADOWSKY
        fieldscore*=0.7 if attacker.ability == :SHADOWDANCE && @battle.pbWeather!=:SHADOWSKY
      when :DARKNESS2 # Darkness
        fieldscore*=1.3 if opponent.ability == :SHADOWDANCE && @battle.pbWeather!=:SHADOWSKY
        fieldscore*=0.7 if attacker.ability == :SHADOWDANCE && @battle.pbWeather!=:SHADOWSKY
      when :DARKNESS3 # Darkness
        fieldscore*=1.3 if opponent.ability == :SHADOWDANCE && @battle.pbWeather!=:SHADOWSKY
        fieldscore*=0.7 if attacker.ability == :SHADOWDANCE && @battle.pbWeather!=:SHADOWSKY
    end
    fieldscore*=0.01
  ...
  def getItemScore
    ...
    for i in items
      ...
      if PBStuff::HPITEMS.include?(i)
        ...
        # Gen 9 Mod - Added Hospitality
        itemscore*=0.7 if @attacker.ability == :HOSPITALITY && partynumber>1 && @attacker.pbPartner
      end
      if PBStuff::STATUSITEMS.include?(i)
        ...
        if !(i== :FULLRESTORE)
          ...
          if @attacker.status== :PARALYSIS
            itemscore*=0.5 if @attacker.ability == :QUICKFEET || @attacker.ability == :GUTS || @attacker.ability == :FORTITUDE
          ...
          if @attacker.status== :BURN
            ...
            itemscore*=0.6 if @attacker.ability == :GUTS || @attacker.ability == :FORTITUDE
          ...
          if @attacker.status== :POISON
            itemscore*=1.1
            itemscore*=0.5 if @attacker.ability == :GUTS || @attacker.ability == :FORTITUDE
      ...
      # Gen 9 Mod - Item use disincentivized if attacker is being salt cured.
      itemscore*=0.7 if @attacker.effects[:SaltCure] && @attacker.ability != :MAGICGUARD
      itemscore*=0.7 if @attacker.effects[:SaltCure] && @attacker.ability != :MAGICGUARD && (@attacker.hasType?(:WATER) || @attacker.hasType?(:STEEL))
...
################################################################################
  ...
  def pbDefaultChooseNewEnemy(index, party)
    ...
    @index = index
    @opponent = illusionCheck(@attacker, firstOpponent()) ? firstOpponent().effects[:Illusion] : firstOpponent()
  ...
  def getSwitchInScoresParty(hard_switch, revival = false)
    party = revival ? @battle.pbParty(@attacker.index).find_all {|mon| mon && !mon.egg? && mon.hp  == 0} : @battle.pbParty(@attacker.index)
    if @mondata.skill < MEDIUMSKILL
      # Bad trainers likely don't know what a pikachu is, and just swap in random mons
      partyScores = Array.new(party.length, -10000000)
      # if there are no switch-ins at all
      return partyScores if hard_switch || !party.any? { |pkmn| @battle.pbCanSwitchLax?(@attacker.index, party.index(pkmn), false, revival) }
    ...
    for partyindex in 0...party.length
      ...
      if revival
        i.hp = [(i.totalhp / 2.0).floor, 1].max
        nonmegaform.hp = [(nonmegaform.totalhp / 2.0).floor, 1].max
      end
      ...
      if hard_switch
        ...
        if !@battle.pbCanSwitchLax?(@attacker.index, partyindex, false, revival)
          partyScores.push(-10000000)
          PBDebug.log(sprintf("Score: -10000000\n")) if $INTERNAL
          $ai_log_data[@attacker.index].switch_scores.push(-10000000)
          $ai_log_data[@attacker.index].switch_name.push(getMonName(i.pokemon.species))
      ...
      # Gen 9 Mod - Preserve a pokemon with Supreme Overlord for later.
      if i.ability == :SUPREMEOVERLORD
        partyScores.push(-5000)
        PBDebug.log(sprintf("Supreme Overlord later- Score: -5000\n")) if $INTERNAL
        $ai_log_data[@attacker.index].switch_scores.push(-5000)
        $ai_log_data[@attacker.index].switch_name.push(getMonName(i.pokemon.species))
        next
      end
      # Gen 9 Mod - Preserve a pokemon with Last Respects for later.
      if i.pbHasMove?(:LASTRESPECTS) && !(@battle.opponent && !@battle.opponent.is_a?(Array) && @battle.opponent.trainertype == :DESERTHORDE && i.pbFaintedPokemonCount > 8)
        partyScores.push(-5000)
      ...
      # Resurgence - Preserve a pokemon with Chlorofury / Unleafed for later.
      if (i.ability == :CHLOROFURY || i.ability == :UNLEAFED)
        partyScores.push(-3000)
        PBDebug.log(sprintf("Chlorofury / Unleafed later- Score: -3000\n")) if $INTERNAL
        $ai_log_data[@attacker.index].switch_scores.push(-3000)
        $ai_log_data[@attacker.index].switch_name.push(getMonName(i.pokemon.species))
        next
      end
      ...
      if (i.ability == :IMPOSTER)
        transformed = true
        i = pbMakeFakeBattler(@opponent.pokemon, @opponent.pokemonIndex)
        i.hp = nonmegaform.hp
      ...
      if @mondata.skill >= HIGHSKILL
        if theseRoles.include?(:SWEEPER)
          ...
          rolescore+=50 if @opponent.status== :SLEEP
          rolescore+=50 if @opponent.pbPartner.status== :SLEEP
          ...
          rolescore+= (25*(@attacker.pbFaintedPokemonCount - 2)) if @attacker.ability == :CHLOROFURY
          rolescore+= (25*(@attacker.pbFaintedPokemonCount - 2)) if @attacker.ability == :UNLEAFED
        ...
        if theseRoles.include?(:PHYSICALWALL) || theseRoles.include?(:SPECIALWALL)
          ...
          rolescore+=30 if @opponent.status== :BURN || @opponent.status== :FROSTBITE || @opponent.status== :POISON || @opponent.effects[:LeechSeed]>0
          rolescore+=30 if @opponent.pbPartner.status== :BURN || @opponent.pbPartner.status== :FROSTBITE || @opponent.pbPartner.status== :POISON || @opponent.pbPartner.effects[:LeechSeed]>0
        end
        ...
        if theseRoles.include?(:SPINNER)
          if !@opponent.hasType?(:GHOST) && (@opponent.pbPartner.hp==0 || !@opponent.pbPartner.hasType?(:GHOST))
            ...
            rolescore+=30 if @attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock]
          end
        end
        ...
        if theseRoles.include?(:STATUSABSORBER)
          for specificmemory in [aimem, aimem2]
            ...
            for j in specificmemory
              statusmove = PBStuff::BURNMOVE.include?(j.move) || PBStuff::FROSTEBITEMOVE.include?(j.move) || PBStuff::PARAMOVE.include?(j.move) || PBStuff::SLEEPMOVE.include?(j.move) || PBStuff::SCREENMOVE.include?(j.move)
            end
          end
        ...
        if theseRoles.include?(:WEATHERSETTER)
          ...
          elsif (i.ability == :SNOWWARNING) || (nonmegaform.ability == :SNOWWARNING) || (i.ability == :SLEET) || (nonmegaform.ability == :SLEET) || i.pbHasMove?(:HAIL)
            rolescore+=60 if @battle.weather!=:HAIL
          elsif (i.ability == :NOCTEM) || (nonmegaform.ability == :NOCTEM) || i.pbHasMove?(:SHADOWPRAYER)
            rolescore+=60 if @battle.weather!=:SHADOWSKY
        ...
        # TODO: Add field setters Gen 9 - Mod
      end
      monscore += rolescore
      ...
      case @battle.weather
        when :HAIL
          ...
          weatherscore+=80 if (i.ability == :SLUSHRUSH) || (i.ability == :ICECLEATS) || (i.item == :EMPCREST && i.species == :EMPOLEON) || (i.ability == :PROTOCRYOPHILIC) || (i.ability == :WINTERJOY)
          weatherscore+=30 if (i.ability == :ICEFACE) && i.form == 1
        when :RAINDANCE
          weatherscore+=50 if (i.ability == :DRYSKIN) || (i.ability == :HYDRATION) || (i.ability == :RAINDISH) || (i.ability == :SUPERCELL)
          weatherscore+=80 if (i.ability == :SWIFTSWIM)
        when :SUNNYDAY
          weatherscore-=40 if (i.ability == :DRYSKIN) || (i.ability == :WINTERJOY && i.item != :UTILITYUMBRELLA)
          weatherscore+=50 if (i.ability == :SOLARPOWER) || (i.ability == :SOLARIDOL)
          weatherscore+=80 if (i.ability == :CHLOROPHYLL)
          # Gen 9 Mod - Added Protosynthesis & Orichalcum Pulse
          weatherscore+=80 if (i.ability == :PROTOSYNTHESIS) || (i.ability == :ORICHALCUMPULSE)
        when :SANDSTORM
          # Resurgence - Crystal-typing Added to Sandstorm Immunity
          weatherscore+=25 if (i.ability == :MAGICGUARD) || (i.ability == :OVERCOAT) || i.hasType?(:ROCK) || i.hasType?(:GROUND) || i.hasType?(:STEEL) || i.hasType?(:CRYSTAL) || (i.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
          weatherscore+=50 if (i.ability == :SANDVEIL) || (i.ability == :SANDFORCE)
          weatherscore+=80 if (i.ability == :SANDRUSH)
        when :SHADOWSKY
          weatherscore+=25 if (i.ability == :MAGICGUARD) || (i.ability == :OVERCOAT) || i.hasType?(:DARK) || i.hasType?(:GHOST) || (i.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
          weatherscore+=50 if (i.ability == :ABSOLUTION) || (i.ability == :HELIOPHOBIA)
          weatherscore+=80 if (i.ability == :SHADOWDANCE) || (i.ability == :PROTOABSOLUTION)
      end
      if @battle.trickroom>0
      ...
      if @mondata.skill >= HIGHSKILL
        ...
        case i.ability
          ...
          when :COMATOSE
            ...
            abilityscore += 20 if checkAImoves(PBStuff::FROSTEBITEMOVE, aimem)
          ...
          when :MAGICGUARD
            ...
            abilityscore += 20 if checkAImoves([:HYPOTHERMIA], aimem)
            abilityscore += 20 if checkAImoves(PBStuff::POISONMOVE, aimem)
          ...
          when :CURIOUSMEDICINE
            abilityscore -= 10 * statchangecounter(@opponent, 1, 7, 1)
          when :COSTAR # Gen 9 Mod - Added Costar
            abilityscore += 10 * statchangecounter(@opponent, 1, 7, 1)
          ...
          when :WONDERGUARD
            ...
            if i.status == :BURN || i.status == :POISON || i.status == :FROSTBITE
              dievar = true
              instantdievar = true
            end
            if @attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock] || @attacker.pbOwnSide.effects[:Spikes] > 0 || @attacker.pbOwnSide.effects[:ToxicSpikes] > 0
              dievar = true
              instantdievar = true
            end
          ...
          # Gen 9 Mod - Added Lingering Aroma
          when :EFFECTSPORE, :STATIC, :POISONPOINT, :FLAMEBODY, :CUTECHARM, :MUMMY, :AFTERMATH, :GOOEY, :FLUFFY, :PERISHBODY, :WANDERINGSPIRIT, :LINGERINGAROMA
          ...
          when :IRONBARBS, :ROUGHSKIN, :SLAG
            if checkAIbestMove(@opponent).contactMove? || (@opponent.pbPartner.hp > 0 && checkAIbestMove(@opponent.pbPartner).contactMove?)
              abilityscore += 30 unless i.ability == :SLAG && @battle.FE == :UNDERWATER
            end
          ...
          # Resurgence - Ethereal Shroud check for Contact Moves
          when :ETHEREALSHROUD
            abilityscore+=60 if checkAIbestMove(@opponent).contactMove? || (@opponent.pbPartner.hp>0 && checkAIbestMove(@opponent.pbPartner).contactMove?)
          when :THICKFAT
          ...
          when :WELLBAKEDBODY # Gen 9 Mod - Added Well-Baked Body
            abilityscore+=60 if :FIRE ==checkAIbestMove().pbType(@opponent) || (@opponent.pbPartner.hp > 0 && :FIRE == checkAIbestMove(@opponent.pbPartner).pbType(@opponent.pbPartner))
          when :LIQUIDOOZE
            for j in aimem
          ...
          when :SCRAPPY,:MINDSEYE # Gen 9 Mod - Added Mind's Eye
          ...
          when :ILLUSION
            abilityscore+=40
          when :MOXIE, :HUBRIS, :BEASTBOOST, :SOULHEART, :GRIMNEIGH, :CHILLINGNEIGH, :ASONECHILLING, :ASONEGRIM
          ...
          when :GALEWINGS
            abilityscore+=50 if !pbAIfaster?(nil,nil,i,@opponent) && i.hp==i.totalhp && !(@attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock])
            abilityscore+=50 if @opponent.pbPartner.hp > 0 && !pbAIfaster?(nil,nil,i,@opponent.pbPartner) && i.hp==i.totalhp && !(@attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock])
          when :BULLETPROOF
          ...
          when :SANDSTREAM,:SNOWWARNING,:SANDSTREAM,:SNOWWARNING,:SANDSPIT,:SLEET
          ...
          when :WINDPOWER # Gen 9 Mod - Added Wind Power
            abilityscore+=10 if checkAIbestMove(@opponent).windMove? || (@opponent.pbPartner.hp>0 && checkAIbestMove(@opponent.pbPartner).windMove?)
          when :WINDRIDER # Gen 9 Mod - Added Wind Rider
            abilityscore+=60 if checkAIbestMove(@opponent).windMove? || (@opponent.pbPartner.hp>0 && checkAIbestMove(@opponent.pbPartner).windMove?)
      ...
      if transformed # pokemon has imposter ability. because we copy pokemon, we can use i to see ability opponent
        abilityscore += 50 if [:PUREPOWER, :HUGEPOWER, :ATHENIAN, :MOXIE, :HUBRIS, :CHILLINGNEIGH, :GRIMNEIGH, :SPEEDBOOST, :BEASTBOOST, :SOULHEART, :WONDERGUARD, :PROTEAN, :LIBERO].include?(i.ability)
      ...
      if @mondata.skill>=HIGHSKILL
        ...
        if (i.item == :FOCUSSASH || (@battle.FE == :CHESS && i.pokemon.piece==:PAWN) || i.ability == :STURDY || (@battle.FE == :CHESS && i.ability == :STALWART)) && i.hp == i.totalhp
          # Gen 9 Mod - Hail/Snow/Both
          # Resurgence - Crystal-typing Added to Sandstorm Immunity
          if   (((@battle.weather== :SANDSTORM && !(i.hasType?(:ROCK) || i.hasType?(:GROUND) || i.hasType?(:STEEL) || i.hasType?(:CRYSTAL)))  || (@battle.weather== :HAIL && !(i.hasType?(:ICE) && HAILSNOWMOD != "Snow"))) && !((i.ability == :OVERCOAT)))  || @attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock] ||
        ...
        if ((i.item == :PROTECTIVEPADS) || (i.item == :PUNCHINGGLOVE && move.punchMove?)) # Gen 9 Mod - Added Punching Glove
          itemscore+=25 if (i.ability == :EFFECTSPORE) || (i.ability == :STATIC) || (i.ability == :POISONPOINT) || (i.ability == :ROUGHSKIN) || (i.ability == :WANDERINGSPIRIT) || (i.ability == :PERISHBODY && @battle.FE != :HOLY) || (i.ability == :SLAG && @battle.FE != :UNDERWATER)  || (i.ability == :IRONBARBS) || (i.ability == :FLAMEBODY) || (i.ability == :CUTECHARM) || (i.ability == :MUMMY) || (i.ability == :AFTERMATH) || (i.ability == :GOOEY) || ((i.ability == :FLUFFY) && (!@opponent.hasType?(:FIRE) && !@opponent.pbPartner.hasType?(:FIRE))) || (@opponent.item == :ROCKYHELMET) || (@opponent.ability == :TOXICDEBRIS) || (@opponent.ability == :LINGERINGAROMA) # Gen 9 Mod - Added Toxic Debris & Lingering Aroma
        end
      ...
      if @mondata.skill>=BESTSKILL
        ...
        when :ELECTERRAIN
            ...
            # Gen 9 Mod - Added Hadron Engine
            fieldscore+=50 if (i.ability == :HADRONENGINE)
            # Resurgence
        ...
        when :DARKCRYSTALCAVERN
            ...
            # Resurgence
            fieldscore+=30 if (i.ability == :SHADOWCALL)
            fieldscore+=50 if (i.ability == :SHADOWDANCE)
            fieldscore+=20 if i.ability == :NOCTEM || nonmegaform.ability == :NOCTEM
            fieldscore+=30 if (i.ability == :PRISMGUARD)
            fieldscore+=30 if (i.ability == :CRYSTALLIZE)
        when :CHESS
            ...
            # Resurgence - Cerebral Fist
            fieldscore+=20 if (i.ability == :CEREBRALFIST)
        when :BIGTOP
            ...
            fieldscore+=20 if (i.ability == :AMPLIFIER)
        when :BURNING
        ...
        when :ICY
            ...
            fieldscore+=50 if (i.ability == :SLUSHRUSH) || (i.ability == :ICECLEATS) || (i.item == :EMPCREST && i.species == :EMPOLEON)
        when :ROCKY
        ...
        when :VOLCANICTOP
          ...
          fieldscore += 25 if i.ability == :STEAMENGINE
          fieldscore -= 30 if i.ability == :ICEFACE
          fieldscore += 30 if i.ability == :GALEWINGS && @battle.pbWeather == :STRONGWINDS
        when :FACTORY
            fieldscore+=25 if i.hasType?(:ELECTRIC)
        ...
        when :ASHENBEACH
            ...
            fieldscore+=15 if (i.ability == :FORTITUDE)
        ...
        when :MOUNTAIN
            ...
            fieldscore += 20 if i.ability == :SNOWWARNING || nonmegaform.ability == :SNOWWARNING || i.ability == :SLEET || nonmegaform.ability == :SLEET
        ...
        when :SNOWYMOUNTAIN
            ...
            fieldscore += 20 if i.ability == :SLEET || nonmegaform.ability == :SLEET
            fieldscore += 20 if i.ability == :DROUGHT || nonmegaform.ability == :DROUGHT
        ...
        when :HOLY
            ...
            # Resurgence - Psycho Call and Cerebral Fist
            fieldscore+=30 if (i.ability == :PSYCHOCALL)
            fieldscore+=20 if (i.ability == :CEREBRALFIST)
        when :MIRROR
          fieldscore+=25 if (i.ability == :SANDVEIL)
            fieldscore+=25 if (i.ability == :SNOWCLOAK)
        ...
        when :PSYTERRAIN
            ...
            fieldscore+=25 if i.ability == :POWERSPOT
            # Resurgence - Psycho Call & Cerebral Fist
            fieldscore+=30 if (i.ability == :PSYCHOCALL)
            fieldscore+=20 if (i.ability == :CEREBRALFIST)
        when :DIMENSIONAL
          fieldscore+=25 if i.hasType?(:DARK)
          #fieldscore+=30 if (i.ability == :ANGERSHELL) # Gen 9 Mod - Added Anger Shell / On base mod, it isn't affected by the field
          fieldscore+=30 if (i.ability == :SHADOWSHIELD)
          ...
          # Resurgence - Spirit Call, Shadow Call, Noctem & Shadow Dance
          fieldscore+=30 if (i.ability == :SPIRITCALL)
          fieldscore+=30 if (i.ability == :SHADOWCALL)
          fieldscore+=20 if i.ability == :NOCTEM || nonmegaform.ability == :NOCTEM
          fieldscore+=50 if (i.ability == :SHADOWDANCE)
        when :FROZENDIMENSION
          ...
          fieldscore+=25 if (i.ability == :ICEBODY)
            fieldscore+=25 if (i.ability == :SNOWCLOAK)
            fieldscore+=25 if (i.ability == :REFRIGERATE)
          #fieldscore+=35 if (i.ability == :ANGERSHELL) # Gen 9 Mod - Added Anger Shell / On base mod, it isn't affected by the field
        ...
        when :CONCERT1,:CONCERT2,:CONCERT3,:CONCERT4
          fieldscore+=25 if (i.ability == :TECHNICIAN)
          fieldscore+=25 if ([:HEAVYMETAL,:SOLIDROCK,:PUNKROCK,:ROCKHEAD,:SOUNDPROOF,:AMPLIFIER].include?(i.ability))
          fieldscore+=15 if ([:HEAVYMETAL,:SOLIDROCK,:PUNKROCK,:GALVANIZE,:PLUS,:AMPLIFIER].include?(i.ability)) && @battle.ProgressiveFieldCheck(PBFields::CONCERT,1,3)
      ...
      if @attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock] || @attacker.pbOwnSide.effects[:Spikes]>0
  ...
  def shouldSwitch?
    ...
    statusscore+=60 if @attacker.effects[:SaltCure]
    statusscore+=80 if @attacker.effects[:SaltCure] && (@attacker.hasType?(:WATER) || @attacker.hasType?(:STEEL))
    ...
    healscore+=30 if (@attacker.pbPartner.hp.to_f)/@attacker.pbPartner.totalhp<(3/4) && @attacker.ability == :HOSPITALITY
    if @attacker.effects[:Wish]>0
    ...
    for i in @attacker.moves
      ...
      for oppmon in [@opponent, @opponent.pbPartner]
        ...
        bothimmune = false if i.move==:PERISHSONG && !(oppmon.ability == :SOUNDPROOF && !(moldBreakerCheck(@attacker) || myceliumMightCheck(@attacker))) # Gen 9 Mod - Added myceliumMightCheck
    ...
    for oppmon in [@opponent,@opponent.pbPartner]
      ...
      #punishing skill-link/loaded dice multi-hit contact moves # Gen 9 Mod - Added Loaded Dice
      if oppmon.ability == :SKILLLINK || oppmon.hasWorkingItem(:LOADEDDICE)
        if getAIMemory(oppmon).any? {|moveloop| moveloop!=nil && moveloop.function==0xC0 && moveloop.contactMove?}
          for i in @mondata.party
            next if i.nil? || i.hp == 0 || @mondata.party.index(i) == @attacker.pokemonIndex
            if (i.item == :ROCKYHELMET) || (i.ability == :ROUGHSKIN) || (i.ability == :IRONBARBS) || (i.ability == :SLAG && @battle.FE != :UNDERWATER)
              specialscore+=70
              break
            end
          end
      ...
      #Thermal Exchange switch vs Fire attack moves
      # Gen 9 Mod - Added Thermal Exchange
      bestmove=checkAIbestMove()
      if bestmove.pbType(@opponent) == :FIRE && @attacker.ability != :THERMALEXCHANGE
        for i in @mondata.party
          next if i.nil? || i.hp == 0 || @mondata.party.index(i) == @attacker.pokemonIndex
    ...
    hazardantiscore += (15) if @attacker.pbOwnSide.effects[:StealthRock] || @attacker.pbOwnSide.effects[:FoundryStealthRock]
  ...
  def pbStatChangingSwitch(mon)
    ...
    if mon.pbOwnSide.effects[:StickyWeb] && !mon.isAirborne?
      ...
      mon.stages[PBStats::SPEED]-= drop unless mon.item == :WHITEHERB || mon.item == :CLEARAMULET || mon.ability == :WHITESMOKE || mon.ability == :CLEARBODY || mon.item == :HEAVYDUTYBOOTS # Gen 9 Mod - Added Clear Amulet
    ...
    if @battle.ProgressiveFieldCheck(PBFields::CONCERT)
      mon.stages[PBStats::DEFENSE]+=1 if [:HEAVYMETAL,:SOLIDROCK,:PUNKROCK,:SOUNDPROOF,:ROCKHEAD,:AMPLIFIER].include?(mon.ability)
  ...
  def checkAImovesAllOpponents(moveID)
    return checkAImoves(moveID) || checkAImoves(moveID, battler: @opponent.pbPartner)
  end

  def checkAImoves(moveID, memory = nil, battler: @opponent)
    memory = getAIMemory(battler) if memory.nil?
    # basic "does the other mon have x"
    for i in moveID
      for j in memory
        move = pbChangeMove(j, battler)
  ...
  def pbRoughDamage(move = @move, attacker = @attacker, opponent = @opponent)
    ...
    # Gen 9 Mod - Moves will deal twice as much damage if the opponent last used Glaive Rush and we can act before them
    # i.e. we can use our move before they use their next move and remove the Glaive Rush effect.
    if opponent.effects[:GlaiveRush] && pbAIfaster?(move,nil,attacker,opponent)
      basedamage*=2
    end
    ...
    if (move.pbIsMultiHit && attacker.crested == :DUDUNSPARCE) || (move.move == :MYSTICALFIRE && attacker.species == :DELPHOX && attacker.form == 2)
      move.defenseDamageCategory(attacker, opponent)
    end
    ...
    if @mondata.skill >= BESTSKILL && @battle.FE != :INDOOR
      ...
      case @battle.FE
        ...
        when :CHESS
          ...
          # Resurgence - Delta Scrafty's Crest boosting Recoil Moves for Damage Calcs
          if Rejuv && attacker.ability == :RECKLESS || attacker.ability == :GORILLATACTICS || attacker.crested == :SCRAFTY 
    ...
    if @mondata.skill >= MEDIUMSKILL
      ...
      case attacker.ability
        ...
        when :CEREBRALFIST then basedamage = (basedamage * 1.2).round if move.punchMove? && [:PSYTERRAIN, :CHESS, :HOLY].include?(@battle.FE)
        ...
        when :SPECTRALJAWS
          if PBStuff::BITEMOVE.include?(move.move)
            basedamage = (basedamage * 1.5).round
            @category = :special
          end
        ...
        when :SANDFORCE
          # Resurgence - Crystal-typing Added to Sandstorm Immunity
          if @battle.pbWeather == :SANDSTORM && (type == :ROCK || type == :GROUND || type == :STEEL || type == :CRYSTAL)
        ...
        when :FORTITUDE then atk = (atk * 1.5).round if !attacker.status.nil? && move.pbIsSpecial?(type)
        when :PLUS, :MINUS
          if move.pbIsSpecial?(type)
        ...
        when :ATHENIAN then atk = (atk * 2.0).round if move.pbIsSpecial?(type)
        ...
        when :WINTERJOY
          if @battle.pbWeather == :HAIL || [:ICY, :SNOWYMOUNTAIN, :FROZENDIMENSION].include?(@battle.FE)
            atk = (atk * 1.4)
          elsif @battle.pbWeather == :SUNNYDAY && attacker.item != :UTILITYUMBRELLA
            atk = (atk * 0.7)
          end
        when :SUPERCELL then atk = (atk * 1.5).round if (@battle.pbWeather == :RAINDANCE || @battle.pbWeather == :SHADOWSKY || @battle.FE == :FACTORY || @battle.FE == :SHORTCIRCUIT || @battle.FE == :ELECTRICT) && move.pbIsSpecial?(type)
        when :ABSOLUTION then atk = (atk * 1.5).round if (@battle.pbWeather == :SHADOWSKY || @battle.FE == :DARKCRYSTALCAVERN || @battle.FE == :DARKNESS1 || @battle.FE == :DARKNESS2 || @battle.FE == :DARKNESS3 || @battle.FE == :SHORTCIRCUIT || @battle.FE == :DIMENSIONAL) && move.pbIsSpecial?(type)
        ...
        when :AMPLIFIER then basedamage = [:BIGTOP,:CAVE].include?(@battle.FE) ? (basedamage * 1.5).round : (basedamage * 1.25).round if move.isSoundBased? # Resurgence - Amplifier
        ...
        when :WATERBUBBLE then basedamage = (basedamage * 2.0).round if type == :WATER
          # Resurgence - Shadow Synergy
        when :SHADOWSYNERGY then basedamage = [:DIMENSIONAL, :FROZENDIMENSION].include?(@battle.FE) ? (basedamage * 2.0).round : (basedamage * 1.5).round if move.type == :DARK
        ...
        # Resurgence - Protoabsolution
        when :PROTOABSOLUTION then basedamage = (basedamage * 1.3).round if (attacker.effects[:Protoabsolution][0] == PBStats::ATTACK && move.pbIsPhysical?(type)) || (attacker.effects[:Protoabsolution][0] == PBStats::SPATK && move.pbIsSpecial?(type))
        # Resurgence - Protocryophilic
        when :PROTOCRYOPHILIC then basedamage = (basedamage * 1.3).round if (attacker.effects[:Protocryophilic][0] == PBStats::ATTACK && move.pbIsPhysical?(type)) || (attacker.effects[:Protocryophilic][0] == PBStats::SPATK && move.pbIsSpecial?(type))
        ...
        when :ORICHALCUMPULSE
          if (@battle.pbWeather== :SUNNYDAY && move.pbIsPhysical?(type)) && @battle.FE != :FROZENDIMENSION
            basedamage=(basedamage*((5461/4096.to_f))).round
          end
        # Execution
        ...
        when :FOUNDRY then basedamage = [:VOLCANIC, :VOLCANICTOP, :DRAGONSDEN, :INFERNAL].include?(@battle.FE) ? (basedamage * 1.5).round : (basedamage * 1.3).round if move.type == :ROCK
        when :CRYSTALLIZE then basedamage = [:CRYSTALCAVERN, :DARKCRYSTALCAVERN].include?(@battle.FE) ? (basedamage * 1.5).round : (basedamage * 1.2).round if move.type == :ROCK
      ...
      if attitemworks # don't bother with this if it doesn't work
        ...
        case type
          when :NORMAL
            case attacker.item
              # Gen 9 Mod - Blank Plate added.
              when :SILKSCARF, :BLANKPLATE then basedamage = (basedamage * 1.2).round
          ...
          when :FAIRY
            case attacker.item
              # Gen 9 Mod - Added Fairy Feather
              when :FAIRYFEATHER,:PIXIEPLATE then basedamage = (basedamage * 1.2).round
              when :FAIRYGEM then basedamage = (basedamage * 1.3).round
            end
          when :CRYSTAL
            case attacker.item
              # Resurgence - Added Quartz Tear, Transparent Plate, and Crystal Gem
              when :QUARTZTEAR,:TRANSPARENTPLATE then basedamage = (basedamage * 1.2).round
              when :CRYSTALGEM then basedamage = (basedamage * 1.3).round
            end
        end
        # Muscle Band
        ...
        elsif (attacker.item == :WELLSPRINGMASK || attacker.item == :HEARTHFLAMEMASK || attacker.item == :CORNERSTONEMASK)
          if (attacker.pokemon.species == :OGERPON)
            basedamage = (basedamage*1.2).round
          end
        # Gen 9 Mod - Added Punching Glove
        elsif (attacker.item == :PUNCHINGGLOVE)
          basedamage = (basedamage*1.1).round if move.punchMove?
          ...
          # Resurgence - Making base Luxray and Delta Luxray's Crest Effects exclusive to each of their respective forms
            ...
            case attacker.form
              when 0 then basedamage = (basedamage * 1.2).round if move.type == :NORMAL && type == :ELECTRIC
              when 1 then basedamage = (basedamage * 1.5).round if !opponent.status.nil?
            end
      ...
      ############ MISC CHECKS ############
      # Charge
      # Resurgence - Superconduct charging next Attack
      # Gen 9 Mod - Charge lasts until the user uses an electric move.
      if (attacker.effects[:Charge] == true || attacker.effects[:ElectroCharge] == true) && type == :ELECTRIC
        basedamage = (basedamage*2.0).round
      end
      if attacker.effects[:CryoCharge] == true && type == :ICE
        basedamage = (basedamage*2.0).round
      end
      # Helping Hand
      ...
      # Fairy Aura/Aurabreak
        ...
        if @battle.battlers.any? { |battler| battler.ability == :FAIRYAURA }
          if @battle.pbWeather == :SHADOWSKY
            basedamage *= @battle.battlers.any? { |battler| battler.ability == :AURABREAK } ? 1.0 : 1.0
          else
            if @battle.FE == :DARKNESS1
      ...
      if attacker.ability == :SUPREMEOVERLORD # Gen 9 Mod - Added Supreme Overlord
        atk = (atk * (1.0 + (attacker.pbFaintedPokemonCount * 0.1))).round
      end
    end
    ...
    if @mondata.skill >= BESTSKILL
      if ([:BURNING, :VOLCANIC, :INFERNAL].include?(@battle.FE) || attacker.effects[:Blazed]) && attacker.ability == :BLAZE && type == :FIRE
        ...
        atk = (atk * 1.5).round
      # Resurgence
      elsif (@battle.FE == :PSYTERRAIN || @battle.FE == :HOLY) && (attacker.ability == :PSYCHOCALL && type == :PSYCHIC)
        atk = (atk * 1.5).round
      elsif (@battle.FE == :DARKCRYSTALCAVERN || @battle.FE == :DIMENSIONAL || @battle.FE == :SHORTCIRCUIT) && (attacker.ability == :SHADOWCALL && type == :DARK)
        atk = (atk * 1.5).round
      elsif (@battle.FE == :HAUNTED || @battle.FE == :DIMENSIONAL) && (attacker.ability == :SPIRITCALL && type == :GHOST)
        atk = (atk * 1.5).round
        ...
        if (attacker.ability == :OVERGROW && type == :GRASS) || (attacker.ability == :BLAZE && type == :FIRE && @battle.FE != :FROZENDIMENSION) ||
           ...
           (attacker.ability == :PSYCHOCALL && type == :PSYCHIC) || (attacker.ability == :SHADOWCALL && type == :DARK && @battle.FE != :HOLY) || (attacker.ability == :SPIRITCALL && type == :GHOST && @battle.FE != :HOLY) # Resurgence
          atk = (atk * 1.5).round
        end
      end
      ...
      if (attacker.ability == :OVERGROW && type == :GRASS) || (attacker.ability == :BLAZE && type == :FIRE) ||
         (attacker.ability == :TORRENT && type == :WATER) || (attacker.ability == :SWARM && type == :BUG) ||
         (attacker.ability == :PSYCHOCALL && type == :PSYCHIC) || (attacker.ability == :SHADOWCALL && type == :DARK) || (attacker.ability == :SPIRITCALL && type == :GHOST) # Resurgence
        atk = (atk * 1.5).round
      end
    end
    ...
    # Resurgence - Delta Noctowl increased Defense for Physical Moves for AI Calculations
    defense = (defense * 2).round if attacker.crested == :NOCTOWL && attacker.form == 1 && move.pbHitsPhysicalStat?(type)
    if @mondata.skill >= MEDIUMSKILL
      # Sandstorm weather
      ...
      if opponent.ability == :MARVELSCALE
        ...
        ddefense = (defense * 1.3).round if (opponent.effects[:Quarkdrive][0] == PBStats::DEFENSE && move.pbIsPhysical?(type)) || (opponent.effects[:Quarkdrive][0] == PBStats::SPDEF && move.pbIsSpecial?(type))
      ...
      # Resurgence - Protoabsolution
      elsif opponent.ability == :PROTOABSOLUTION
        defense = (defense * 1.3).round if (opponent.effects[:Protoabsolution][0] == PBStats::DEFENSE && move.pbIsPhysical?(type)) || (opponent.effects[:Protoabsolution][0] == PBStats::SPDEF && move.pbIsSpecial?(type))
      # Resurgence - Protocryophilic
      elsif opponent.ability == :PROTOCRYOPHILIC
        defense = (defense * 1.3).round if (opponent.effects[:Protocryophilic][0] == PBStats::DEFENSE && move.pbIsPhysical?(type)) || (opponent.effects[:Protocryophilic][0] == PBStats::SPDEF && move.pbIsSpecial?(type))
      # Gen 9 Mod - Beads of Ruin, Sword of Ruin
      ...
      defense=(defense*1.5).round if opponent.flowerGiftActive? && move.pbIsSpecial?(type) && @battle.FE != :GLITCH
      defense=(defense*1.5).round if opponent.pbPartner.flowerGiftActive? && move.pbIsSpecial?(type) && @battle.FE != :GLITCH
    end
    ...
    case attacker.crested
      ...
      # Resurgence - Making base Luxray Crest's Effects exclusive to its base form
      when :LUXRAY then typecrest = true if type == :DARK && attacker.form == 0
      ...
      # Resurgence - Delta Golem's Crest turning Normal-type moves into Psychic-type for damage calcs
      when :GOLEM then typecrest = true if type == :PSYCHIC
    ...
    if @mondata.skill>=MEDIUMSKILL
			# Ancient Presence
			if attacker.ability == :ANCIENTPRESENCE
				damage=(damage*1.5).round
			end
      # Water Bubble
      ...
      # Ethereal Shroud in Haunted Field
      if attacker.ability == :ETHEREALSHROUD && type == :GHOST && @battle.FE == :HAUNTED
        damage=(damage*=2).round
      end
      if (attacker.hasType?(type) && (!attacker.effects[:DesertsMark]) || attacker.ability == :PROTEAN || attacker.ability == :LIBERO || typecrest==true) && attacker.ability != :ANCIENTPRESENCE
        if attacker.ability == :ADAPTABILITY ||
          # Gen 9 Mod - Mega (Terastalized) Ogerpon gets a Tera Boost on it's STAB moves based on it's mask.
          (((attacker.ability == :EMBODYASPECTTEAL && type == :GRASS) ||
          (attacker.ability == :EMBODYASPECTWELLSPRING && type == :WATER) ||
          (attacker.ability == :EMBODYASPECTHEARTHFLAME && type == :FIRE) ||
          (attacker.ability == :EMBODYASPECTCORNERSTONE && type == :ROCK)) && attacker.species == :OGERPON)
          damage=(damage*2).round
        else
          damage=(damage*1.5).round
        end
      ...
      elsif (((attacker.ability == :EMBODYASPECTWELLSPRING) || (attacker.ability == :EMBODYASPECTHEARTHFLAME) || (attacker.ability == :EMBODYASPECTCORNERSTONE)) && type == :GRASS)
        damage=(damage*1.5).round
      # Gen 9 Mod - Moves of each type used by Mega (Terastalized) Terapagos will get a boost.
      elsif (attacker.species == :TERAPAGOS && attacker.form ==2)
    ...
    if @mondata.skill>=MEDIUMSKILL
      ...
      # Resurgence - Frostbite
      if attacker.status== :FROSTBITE && move.pbIsSpecial?(type)
        damage=(damage*0.5).round
      end
    end
    ...
    if @mondata.skill>=MEDIUMSKILL
      if opponent.ability == :WINDRIDER && move.windMove?
        damage*=0
      end
    end
    # Gen 9 Mod - Added Purifying Salt
    if @mondata.skill>=MEDIUMSKILL
      if opponent.ability == :PURIFYINGSALT && type == :GHOST
        damage=(damage*0.5).round
      end
    end
    # Shelter
    ...
    if @mondata.skill>=MEDIUMSKILL
      if (opponent.ability == :MULTISCALE || opponent.ability == :TERASHELL) && !moldBreakerCheck(attacker) || opponent.ability == :SHADOWSHIELD # Gen 9 Mod - Tera Shell same consideration as Multiscale
      ...
      # Resurgence - Delta Minun's Crest reducing damage taken for itself and Delta Plusle by 25% for damage calcs
      damage=(damage*0.75).round if opponent.pbPartner.ability == :FRIENDGUARD || (opponent.crested == :MINUN && opponent.pbPartner.species == :PLUSLE && opponent.pbPartner.isDelta?) || (opponent.species == :PLUSLE && opponent.isDelta? && opponent.pbPartner.crested == :MINUN)
      # Resurgence - Delta Plusle's Crest increasing damage dealt for itself and Delta Minun by 30% for damage calcs
      damage=(damage*1.3).round if (attacker.crested == :PLUSLE && attacker.pbPartner.species == :MINUN && attacker.pbPartner.isDelta?) || (attacker.species == :MINUN && attacker.isDelta? && attacker.pbPartner.crested == :PLUSLE)
      ...
      # Resurgence - Neuroforce & Executioner
      damage=(damage*1.25).round if (attacker.ability == :NEUROFORCE || attacker.ability == :EXECUTIONER) && typemod>4
      # Meganium Crest
    ...
    if @mondata.skill>=HIGHSKILL
      if attacker.ability == :PENDULUM
        if attacker.effects[:Metronome]>4
          damage=(damage*3.0).round
        else
          met=1.0+attacker.effects[:Metronome]*0.4
          damage=(damage*met).round
        end
      elsif (attitemworks && (attacker.item == :METRONOME || @battle.FE == :CONCERT4))
        if attacker.effects[:Metronome]>4
      ...
      if typemod>4 && oppitemworks && !ai_mon_attacking
        berrymod = (opponent.ability == :RIPEN || (opponent.crested == :CRUSTLE && !@battle.doublebattle)) ? 0.25 : 0.5
        case opponent.item
  ...
  def pbBetterBaseDamage(move=@move,attacker=@attacker,opponent=@opponent)
    ...
    case move.function
      ...
      when 0x6B # Dragon Rage
        ...
        return 40
      when 0x6C # Super Fang, Nature's Madness, # Gen 9 Mod - Ruination
      ...
      when 0x7E # Facade
        return basedamage*2 if attacker.status== :POISON || attacker.status== :BURN || attacker.status== :PARALYSIS || attacker.status== :FROSTBITE
      when 0x7F # Hex
        if !opponent.status.nil?
          return basedamage*2
        elsif @mondata.skill>=BESTSKILL
      ...
      when 0x8C # Crush Grip / Wring Out / # Gen 9 Mod - Hard Press
        return [attacker.happiness,250].min if attacker.crested == :LUVDISC
        return [(100*(opponent.hp.to_f)/opponent.totalhp).floor,1].max if move.move == :HARDPRESS
      ...
      when 0x9B # Heavy Slam / Heat Crash
        ...
        return 60 if n>=2
        return 40
      when 0xA0, 0x90B # Frost Breath, Flower Trick
        return basedamage*1.5
      when 0xBD, 0xBE, 0x91F # Double Kick, Twineedle, Tachyon Cutter
        return basedamage*2
      when 0xBF # Triple Kick, Triple Axel, Thunder Raid
        return basedamage*6
      when 0xC0 # Bullet Seed, Pin Missile, Arm Thrust, Bone Rush, Icicle Spear, Tail Slap, Spike Cannon, Comet Punch, Furey Swipes, Barrage, Double Slap, Fury Attack, Rock Blast, Water Shuriken
      ...
      # Gen 9 Mod - Gen 9 Moves Below
      when 0x904 # Collision Course / Electro Drift
        return basedamage*1.33 if pbTypeModNoMessages(@move.pbType(@attacker))>4
      when 0x910 # Last Respects
        faintedCounter=@battle.pbPartySingleOwner(attacker.index).find_all {|mon| mon && !mon.isEgg? && mon.hp<=0}
        return basedamage + (50*faintedCounter.length)
...
#####################################################
  ...
  def moldBreakerCheck(battler)
    return (battler.ability==:MOLDBREAKER || battler.ability==:TERAVOLT || battler.ability==:TURBOBLAZE) && @opponent.item != :ABILITYSHIELD # Gen 9 Mod - Added Ability Shield Check
  end

  def myceliumMightCheck(battler, move = @move)
  ...
  def unseenFistCheck(battler, move = @move)
    return battler.ability == :UNSEENFIST && !(battler.item == :PUNCHINGGLOVE && move.punchMove?) # Gen 9 Mod - Unsee Fist needs to check Punching Glove
  end
  ...
  def notOHKO?(attacker,opponent, immediate = false)
    return false if @battle.pbWeather == :HAIL && !(attacker.hasType?(:ICE) || [:ICEBODY,:SNOWCLOAK,:SLUSHRUSH,:LUNARIDOL,:MAGICGUARD,:OVERCOAT,:TEMPEST,:ICECLEATS,:SNOWWARNING,:SLEET].include?(attacker.ability) || HAILSNOWMOD == "Snow") && !immediate # Resurgence added Ice Cleats and Snow Warning
    # Resurgence - Crystal-typing Added to Sandstorm Immunity
    return false if @battle.pbWeather == :SANDSTORM && !(attacker.hasType?(:ROCK) || attacker.hasType?(:GROUND) || attacker.hasType?(:STEEL) || attacker.hasType?(:CRYSTAL) || [:SANDFORCE,:SANDRUSH,:SANDVEIL,:MAGICGUARD,:OVERCOAT,:TEMPEST,:SANDSTREAM].include?(attacker.ability)) && !immediate # Resurgence added Sand Stream
    return false if @battle.pbWeather == :SHADOWSKY && !(attacker.hasType?(:SHADOW) || attacker.hasType?(:DARK) || attacker.hasType?(:GHOST) || [:MAGICGUARD,:OVERCOAT,:TEMPEST,:SHADOWDANCE,:ABSOLUTION,:HELIOPHOBIA,:NOCTEM].include?(attacker.ability)) && !immediate
  ...
  def seedProtection?(battler = @attacker)
    return battler.effects[:KingsShield] || battler.effects[:BanefulBunker] || battler.effects[:SpikyShield] || battler.effects[:SilkTrap] || battler.effects[:BurningBulwark] || battler.effects[:FalloutShelter] # Gen 9 Mod - Added Silk Trap and Burning Bulwark
  end
  ...
  def accuracyWeatherAbilityActive?(battler)
    ...
    (battler.ability == :ILLUMINATE && (@battle.pbWeather== :SHADOWSKY)) # Resurgence - Illuminate increases evasion
  end
...
class PokeBattle_Move_FFF < PokeBattle_Move # Fake move used by AI to determine damage if no damaging AI memory move
  def initialize(battle, user, type)
    ...
    hash = {
      ...
      :effect => 0,
      :moreeffect => 0,
      :category => (user.attack > user.spatk ? :physical : :special),
      :accuracy => 100,
