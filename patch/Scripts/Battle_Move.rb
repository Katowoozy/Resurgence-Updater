class PokeBattle_Move
  ...
  attr_accessor   :partnerberry
  attr_accessor   :partnerberryname
...
################################################################################
  ...
  def pbType(attacker, type = @type)
    ...
    if !PBStuff::ZMOVES.include?(@move)
      ...
      if attacker.ability.is_a?(PokeAbility) && attacker.ability.ability!=nil
        if attacker.crested == :AVALUGG || attacker.ability.ability.is_a?(Array)
      ...
      case attacker.ability
        ...
        # Resurgence - Foundry, Cerebral Fist, and Sublimating Edge Move Type Changes
        when :FOUNDRY     then type = :FIRE     if type == :ROCK
        when :CRYSTALLIZE then type = :CRYSTAL  if type == :ROCK && @battle.FE != :GLITCH
        when :CEREBRALFIST then type = :PSYCHIC  if punchMove?
        when :SUBLIMATINGEDGE then type = attacker.type1 if type == :NORMAL
      end
    end
    case attacker.crested
      ...
      # Resurgence - Delta Golem's Crest turning Normal-type moves into Psychic-type
      when :GOLEM     then type = :PSYCHIC  if type == :NORMAL
      # Resurgence - Making base Luxray Crest's Effects exclusive to its base form
      when :LUXRAY    then type = :ELECTRIC if type == :NORMAL && attacker.form == 0
      when :SAWSBUCK
        case attacker.form
  ...
  def pbHitsSpecialStat?(type = @type)
    ...
    return !pbHitsPhysicalStat?(type)
  ...
  def pbHitsPhysicalStat?(type = @type)
    ...
    return true if @defenseToHit == :physical
  ...
  def pbDragonDartTargetting(attacker) # all of this just to get dragon darts targeting right, i hate it here
    ...
    if opp2.effects[:SkyDrop] || opp2.effects[:Commander] || opp2.effects[:SpiritAway] # Gen 9 Mod - Added Commander immunity when inside Dondozo's mouth.
      return [opp1]
    end
    # Resurgence - Spirit Away Invulnerability
    if opp1.effects[:SkyDrop] || opp1.effects[:Commander] || opp1.effects[:SpiritAway]  # Gen 9 Mod - Added Commander immunity when inside Dondozo's mouth.
      return [opp2]
    end
    if opp2.effects[:Protect] || opp2.effects[:SpikyShield] || opp2.effects[:BanefulBunker] ||
       ...
       opp2.effects[:SilkTrap] || opp2.effects[:BurningBulwark] || opp2.effects[:FalloutShelter]
      return [opp1]
    end
    if opp1.effects[:Protect] || opp1.effects[:SpikyShield] || opp1.effects[:BanefulBunker] ||
       ...
       # Gen 9 Mod - Added Silk Trap and Burning Bulwark
       # Resurgence - Added Fallout Shelter
       opp1.effects[:SilkTrap] || opp1.effects[:BurningBulwark] || opp1.effects[:FalloutShelter]
      return [opp2]
    end
    ...
    if (darttype == :FIRE || fieldsecondtype.include?(:FIRE)) && (((([:FLASHFIRE,:WELLBAKEDBODY].include?(opp1.ability) && @battle.FE != :FROZENDIMENSION && !opp1.moldbroken) || (opp1.ability == :MAGMAARMOR && [:VOLCANICTOP,:DRAGONSDEN,:INFERNAL].include?(@battle.FE))) && !opp1.moldbroken) || (Rejuv && @battle.FE == :GLITCH && opp1.species == :GENESECT && opp1.hasWorkingItem(:BURNDRIVE)) || opp1.crested == :DRUDDIGON)
      return [opp2]
    end
    if (darttype == :FIRE || fieldsecondtype.include?(:FIRE)) && (((([:FLASHFIRE,:WELLBAKEDBODY].include?(opp2.ability) && @battle.FE != :FROZENDIMENSION && !opp2.moldbroken) || (opp2.ability == :MAGMAARMOR && [:VOLCANICTOP,:DRAGONSDEN,:INFERNAL].include?(@battle.FE))) && !opp2.moldbroken) || (Rejuv && @battle.FE == :GLITCH && opp2.species == :GENESECT && opp2.hasWorkingItem(:BURNDRIVE)) || opp2.crested == :DRUDDIGON)
      return [opp1]
    end
    ...
    if (darttype == :GROUND || fieldsecondtype.include?(:GROUND)) && (opp2.isAirborne? || opp2.crested == :SKUNTANK || ([:EARTHEATER].include?(opp1.ability) && !opp1.moldbroken))
      return [opp1]
    end
    if (darttype == :GROUND || fieldsecondtype.include?(:GROUND)) && (opp1.isAirborne? || opp1.crested == :SKUNTANK || ([:EARTHEATER].include?(opp2.ability) && !opp2.moldbroken))
      return [opp2]
    end
    # Resurgence - Wind Force
    if (darttype == :FLYING || fieldsecondtype.include?(:FLYING)) && ((opp1.ability == :WINDFORCE || opp1.ability == :WINDBORNETYRANT) && !opp1.moldbroken)
      return [opp2]
    end
    if (darttype == :FLYING || fieldsecondtype.include?(:FLYING)) && ((opp2.ability == :WINDFORCE || opp1.ability == :WINDBORNETYRANT) && !opp2.moldbroken)
      return [opp1]
    end
...
=end
  ...
  def smartDamageCategory(attacker, opponent)
    ...
    calcatkmult *= (5461 / 4096.to_f)  if attacker.ability == :ORICHALCUMPULSE && (@battle.pbWeather== :SUNNYDAY) && @battle.FE != :FROZENDIMENSION
    ...
    # Resurgence - Supercell, Absolution, and Winter Joy
    calcspatkmult *= 1.5 if attacker.ability == :FORTITUDE && !attacker.status.nil?
    calcspatkmult *= 1.5 if attacker.ability == :SUPERCELL && (((@battle.pbWeather == :RAINDANCE || @battle.pbWeather == :SHADOWSKY) && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || @battle.FE == :FACTORY || @battle.FE == :SHORTCIRCUIT || @battle.FE == :ELECTERRAIN)
    calcspatkmult *= 1.5 if attacker.ability == :ABSOLUTION && ((@battle.pbWeather == :SHADOWSKY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || [:DARKCRYSTALCAVERN, :DARKNESS1, :DARKNESS2, :DARKNESS3, :SHORTCIRCUIT, :DIMENSIONAL].include?(@battle.FE))
    if attacker.ability == :WINTERJOY
      if @battle.pbWeather == :HAIL || [:ICY, :SNOWYMOUNTAIN, :FROZENDIMENSION].include?(@battle.FE)
        calcspatkmult *= 1.4
      elsif (@battle.pbWeather == :SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || @battle.FE == :DESERT
    ...
    calcatkmult *= 0.5 if attacker.status == :FROSTBITE
    ...
    calcspatkmult *= 2 if attacker.ability == :ATHENIAN
    # end spatk boosts
    ...
    # Resurgence - Delta Noctowl increased Defense for Physical Moves for Move Calculations
    calcdefmult *= 2.0 if opponent.crested == :NOCTOWL && opponent.form == 1
    ...
    calcspdefmult *= 1.5 if opponent.pbPartner.flowerGiftActive? && !(opponent.moldbroken)
  ...
  # Resurgence - Battle Bond Delphox's Mystical Fire boost and Delta Dudunsparce's Crest making Multi-hit moves target the opponent's lower defense stat
  def defenseDamageCategory(attacker, opponent)
    # Calculates how much Defense and SpDef opponent has
    calcdefensestage = opponent.stages[PBStats::DEFENSE] + 6
    # aaaand Defense
    calcdefmult = 1.0
    ...
    calcspdefmult *= 1.5 if @battle.FE == :DESERT && opponent.hasType?(:GROUND)
    calcspdefmult *= 1.5 if @battle.FE == :MISTY && opponent.hasType?(:FAIRY)
    if ((@battle.pbWeather== :SUNNYDAY && !opponent.hasWorkingItem(:UTILITYUMBRELLA)) || opponent.crested==:CHERRIM || opponent.pbPartner.crested==:CHERRIM || @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN) || @battle.FE == :BEWITCHED) && !(opponent.moldbroken)
      calcspdefmult *= 1.5 if opponent.ability == :FLOWERGIFT
      calcspdefmult *= 1.5 if opponent.pbPartner.ability == :FLOWERGIFT
    end
    calcspdefmult *= 2.0 if opponent.ability == :ICESCALES && !opponent.moldbroken
    calcspdefmult *= 1.5 if opponent.hasWorkingItem(:ASSAULTVEST)
    calcspdefmult *= 2.0 if opponent.hasWorkingItem(:DEEPSEASCALE) && opponent.pokemon.species == :CLAMPERL
    ...
    # Compares difference between opponent Def and SpDef to determine hitting which defense
    @defenseToHit = calcdefense < calcspdef ? :physical : :special
    return
  end
  ...
  def pbTypeModifier(type, attacker, opponent, zorovar = false)
    ...
    # Gen 9 Mod - Added Double Shock effect
    if otype1 == :ELECTRIC && opponent.effects[:DoubleShock]
      otype1 = otype2.nil? ? :QMARKS : otype2
    end
    if otype2 == :ELECTRIC && opponent.effects[:DoubleShock]
      otype2 = nil
    end
    ...
    if attacker.ability == :IRRELEPHANT
      mod1=2 if mod1==0
      mod2=2 if mod2==0
    end
    ...
    if @move == :ACHILLESHEEL && (opponent.effects[:Protect] || opponent.effects[:SpikyShield] || opponent.effects[:BanefulBunker] ||
       opponent.effects[:KingsShield] || opponent.effects[:Obstruct] || opponent.effects[:SilkTrap] || opponent.effects[:BurningBulwark] || 
       opponent.effects[:FalloutShelter])
    ...
    if attacker.ability == :OPPRESSIVECOLD
      mod1 = 4 if (otype1 == :FIRE) && atype == :ICE
      mod2 = 4 if (otype2 == :FIRE) && atype == :ICE
    end
  ...
  def pbTypeModifierNonBattler(type, attacker, opponent)
    ...
    # Gen 9 Mod - Added Mind's Eye
    if [:SCRAPPY, :MINDSEYE].include?(attacker.ability)
      mod1 = 2 if otype1 == :GHOST && [:NORMAL, :FIGHTING].include?(atype)
      mod2 = 2 if otype2 == :GHOST && [:NORMAL, :FIGHTING].include?(atype)
    end
  ...
  def pbStatusMoveAbsorption(type,attacker,opponent)
    ...
    # Gen 9 Mod - Added Earth Eater
    if (!(opponent.moldbroken) && (((opponent.ability == :DRYSKIN || opponent.ability == :WATERABSORB) &&  type == :WATER) || (opponent.ability == :VOLTABSORB && type == :ELECTRIC) || (opponent.ability == :EARTHEATER && type == :GROUND))) ||
    ...
    if opponent.ability == :WINDRIDER && self.windMove? && !(opponent.moldbroken)
      negator = getAbilityName(opponent.ability)
      if opponent.pbCanIncreaseStatStage?(PBStats::ATTACK)
        opponent.pbIncreaseStatBasic(PBStats::ATTACK,1)
        @battle.pbCommonAnimation("StatUp",opponent,nil)
    ...
    # Resurgence - Wind Force
    if opponent.ability == :WINDFORCE && type == :FLYING && !(opponent.moldbroken)
      negator = getAbilityName(opponent.ability)
      if opponent.pbCanIncreaseStatStage?(PBStats::SPEED)
        opponent.pbIncreaseStatBasic(PBStats::SPEED,1)
        @battle.pbCommonAnimation("StatUp",opponent,nil)
  ...
  def pbTypeModMessages(type, attacker, opponent)
    ...
    if opponent.ability == :WELLBAKEDBODY && type == :FIRE && !(opponent.moldbroken)
      ...
      if opponent.pbCanIncreaseStatStage?(PBStats::DEFENSE)
        opponent.pbIncreaseStatBasic(PBStats::DEFENSE,2)
        @battle.pbCommonAnimation("StatUp", opponent, nil)
        @battle.pbDisplay(_INTL("{1}'s {2} raised its Defense!", opponent.pbThis, negator))
      else
        @battle.pbDisplay(_INTL("{1}'s {2} made {3} ineffective!", opponent.pbThis, negator, self.name))
      end
      return 0
  ...
  def irregularTypeMods(attacker, opponent, typemod, type)
    ...
    if opponent.ability == :ETHEREALSHROUD
      typemod /= 2 if inverse ? PBTypes.oneTypeEff(type, :GHOST) > 2 : PBTypes.oneTypeEff(type, :GHOST) < 2
      typemod = 0 if PBTypes.oneTypeEff(type, :GHOST) == 0 && !inverse && !(attacker.ability == :SCRAPPY || attacker.ability == :MINDSEYE || opponent.effects[:Foresight]) && @battle.FE != :HOLY
    end
    if (opponent.type1 == :CRYSTAL || opponent.type2 == :CRYSTAL) && battle.FE == :DARKCRYSTALCAVERN
      typemod /= 2 if inverse ? PBTypes.oneTypeEff(type, :DARK) > 2 : PBTypes.oneTypeEff(type, :DARK) < 2
      typemod = 0 if PBTypes.oneTypeEff(type, :DARK) == 0 && !inverse
    end
  ...
  def fieldTypeChange(attacker,opponent,typemod,return_type=false)
    case @battle.FE
      when :RAINBOW # Rainbow Field
        ...
        elsif (pbType(attacker) == :CRYSTAL)
          moddedtype = @battle.getRandomType(:CRYSTAL)
        end
      ...
      when :CRYSTALCAVERN # Crystal Cavern
        if (pbType(attacker) == :ROCK) || (pbType(attacker) == :CRYSTAL) || (@move == :JUDGMENT || @move == :ROCKCLIMB || @move == :STRENGTH || @move == :MULTIATTACK || @move == :PRISMATICLASER)
  ...
  def pbAccuracyCheck(attacker, opponent, precheckedacc: nil)
    ...
    evastage = 0 if opponent.effects[:Foresight] || opponent.effects[:MiracleEye] || @function == 0xA9 || # Chip Away
                    ([:UNAWARE, :KEENEYE, :MINDSEYE].include?(attacker.ability) && !opponent.moldbroken) # Gen 9 Mod - Added Mind's Eye
    accstage -= evastage
    ...
    # Resurgence - Partner receiving Micle Berry Accuracy boost if partnered with Crested Delta Crustle
    if attacker.hasWorkingItem(:MICLEBERRY)
      if (attacker.ability == :GLUTTONY && attacker.hp <= (attacker.totalhp / 2.0).floor) ||
        ...
        accuracy *= 1.2
        if attacker.crested == :CRUSTLE && partner && @battle.doublebattle
          attacker.effects[:PartnerMicle] = true
        end
        attacker.pbDisposeItem(true)
      end
    end
    if partner && partner.effects[:PartnerMicle] == true
      accuracy *= 1.2
      partner.effects[:PartnerMicle] = false
    end
    ...
    if opponent.ability == :ILLUMINATE && @battle.pbWeather == :SHADOWSKY && !opponent.moldbroken # Resurgence - Illuminate increases evasion
      accuracy *= 0.8
    end
    ...
    accuracy = 100.0 if @battle.FE == :ASHENBEACH && [:OWNTEMPO, :INNERFOCUS, :PUREPOWER, :SANDVEIL, :STEADFAST, :FORTITUDE].include?(attacker.ability) && ![:UNNERVE, :ASONECHILLING, :ASONEGRIM].include?(opponent.ability)
...
################################################################################
  def pbCritRate?(attacker, opponent)
    ...
    c += 1 if attacker.ability == :SUPERLUCK || (attacker.ability == :SUBLIMATINGEDGE && sharpMove?)
  ...
  def pbCalcDamage(attacker, opponent, hitnum: 0)
    ...
    case attacker.ability
      ...
      when :SPECTRALJAWS
        if PBStuff::BITEMOVE.include?(@move)
          basemult *= 1.5
          @category = :special
        end
      ...
      # Resurgence - Cerebral Fist Punching Move Buff
      when :CEREBRALFIST  then basemult *= 1.2 if punchMove? && [:PSYTERRAIN, :CHESS, :HOLY].include?(@battle.FE)
      ...
      when :SANDFORCE     then basemult *= 1.3 if (@battle.pbWeather == :SANDSTORM || @battle.FE == :DESERT || @battle.FE == :ASHENBEACH) && (type == :ROCK || type == :GROUND || type == :STEEL || type == :CRYSTAL)
      ...
      when :FOUNDRY
        if @type == :ROCK && type == :FIRE
          case @battle.FE
            when :VOLCANIC, :VOLCANICTOP, :DRAGONSDEN, :INFERNAL then basemult *= 1.5
            else
              basemult *= 1.3
          end
        end
      when :CRYSTALLIZE
        if @type == :ROCK && (type == :CRYSTAL || (type == :NORMAL && @battle.FE == :GLITCH))
          case @battle.FE
            when :CRYSTALCAVERN, :DARKCRYSTALCAVERN then basemult *= 1.5
            else
              basemult *= 1.2
          end
        end
      ...
      when :SHADOWSYNERGY then basemult *= 1.5 if type == :DARK
        if type == :DARK
          case @battle.FE
            when :DIMENSIONAL, :FROZENDIMENSION then basemult *= 1.5
            else
              basemult *= 1.3
          end
        end
    ...
    case attacker.crested
      ...
      # Resurgence - Delta Scrafty's Crest giving a x1.2 increase to Recoil Moves
      when :SCRAFTY then basemult *= 1.2 if @recoil > 0 || [0x130, 0x10B, 0x506].include?(@function) 
      # Resurgence - Delta Dudunsparce's Crest making all Multi-hit Moves use its Attacking Stat
      when :DUDUNSPARCE then @category = :physical if pbIsMultiHit
      # Resurgence - Base Luxray and Delta Luxray's Crests boosting damage based on the effects with their respective forms
      when :LUXRAY 
        case attacker.form
          when 0 then basemult *= 1.2 if @type == :NORMAL && type == :ELECTRIC
          when 1 then basemult *= 1.5 if !opponent.status.nil?
        end
      when :SAWSBUCK
        case attacker.form
      ...
      when :CASTFORM
        if attacker.form==4
          # Resurgence - Added Crystal-typing to Sandstorm Immunity
          basemult *= 1.3 if (@battle.pbWeather == :SANDSTORM || @battle.FE == :DESERT || @battle.FE == :ASHENBEACH) && (type == :ROCK || type == :GROUND || type == :STEEL || type == :CRYSTAL)
        end
    end
    #type mods
    case type
      ...
      when :ELECTRIC
        ...
        basemult *= 2.0 if (attacker.effects[:Charge] == true || attacker.effects[:ElectroCharge] == true)
        attacker.effects[:Charge] = false
        attacker.effects[:ElectroCharge] = false
      when :ICE
        basemult *= 2.0 if attacker.effects[:CryoCharge] == true
        attacker.effects[:CryoCharge] = false
    end
    ...
    # Resurgence
      ...
      atkmult *= 1.5
    elsif (@battle.FE == :DARKCRYSTALCAVERN || @battle.FE == :DIMENSIONAL || @battle.FE == :SHORTCIRCUIT || @battle.FE == :SHORTCIRCUIT || @battle.FE == :DARKNESS1 || @battle.FE == :DARKNESS2) && (attacker.ability == :DARKNESS3 && type == :DARK)
      atkmult *= 1.5
      ...
      if (attacker.ability == :OVERGROW && type == :GRASS) || (attacker.ability == :BLAZE && type == :FIRE && @battle.FE != :FROZENDIMENSION) ||
         ...
         # Resurgence
         (attacker.ability == :PSYCHOCALL && type == :PSYCHIC) || (attacker.ability == :SHADOWCALL && type == :DARK && @battle.FE != :HOLY) || (attacker.ability == :SPIRITCALL && type == :GHOST && @battle.FE != :HOLY)
        atkmult *= 1.5
      end
    end
    # Resurgence - Delta Avalugg Crest calling all of it's Abilities using LAWDS' P3 Code
    ...
    case attacker.ability
      ...
      when :FORTITUDE then atkmult*=1.5 if !attacker.status.nil? && pbIsSpecial?(type)
      when :PLUS, :MINUS
      ...
      when :ATHENIAN then atkmult*=2.0 if pbIsSpecial?(type)
      ...
      when :SUPERCELL then atkmult*=1.5 if (((@battle.pbWeather== :RAINDANCE || @battle.pbWeather== :SHADOWSKY) && !(attitemworks && attacker.item == :UTILITYUMBRELLA)) || @battle.FE == :FACTORY || @battle.FE == :SHORTCIRCUIT || @battle.FE == :ELECTERRAIN) && pbIsSpecial?(type) && @battle.FE != :GLITCH
      when :ABSOLUTION then atkmult*=1.5 if ((@battle.pbWeather== :SHADOWSKY && !(attitemworks && attacker.item == :UTILITYUMBRELLA)) || [:DARKCRYSTALCAVERN, :DARKNESS1, :DARKNESS2, :DARKNESS3, :SHORTCIRCUIT, :DIMENSIONAL].include?(@battle.FE)) && pbIsSpecial?(type) && @battle.FE != :GLITCH
      when :WINTERJOY
        if @battle.pbWeather == :HAIL || [:ICY, :SNOWYMOUNTAIN, :FROZENDIMENSION].include?(@battle.FE)
          atkmult*=1.4 
        elsif (@battle.pbWeather == :SUNNYDAY && !attacker.hasWorkingItem(:UTILITYUMBRELLA)) || @battle.FE == :DESERT
      ...
      when :PROTOABSOLUTION then atkmult*=1.3 if (attacker.effects[:Protoabsolution][0] == PBStats::ATTACK && pbIsPhysical?(type)) || (attacker.effects[:Protoabsolution][0] == PBStats::SPATK && pbIsSpecial?(type))
      # Resurgence - Added Protocryophilic
      when :PROTOCRYOPHILIC then atkmult*=1.3 if (attacker.effects[:Protocryophilic][0] == PBStats::ATTACK && pbIsPhysical?(type)) || (attacker.effects[:Protocryophilic][0] == PBStats::SPATK && pbIsSpecial?(type))
    ...
    # Resurgence - Delta Noctowl increased Defense for Physical Moves for Move Calculations
    if opponent.crested == :NOCTOWL && opponent.form == 1
      defmult *= 2.0 if pbHitsPhysicalStat?(type)
    end
    ...
    case opponent.ability
      ...
      # Gen 9 Mod - Added Protoabsolution
      when :PROTOABSOLUTION then defmult *= 1.3 if (opponent.effects[:Protoabsolution][0] == PBStats::DEFENSE && pbIsPhysical?(type)) || (opponent.effects[:Protoabsolution][0] == PBStats::SPDEF && pbIsSpecial?(type))
        # Gen 9 Mod - Added Protocryophilic
      when :PROTOCRYOPHILIC then defmult *= 1.3 if (opponent.effects[:Protocryophilic][0] == PBStats::DEFENSE && pbIsPhysical?(type)) || (opponent.effects[:Protocryophilic][0] == PBStats::SPDEF && pbIsSpecial?(type))
      when :FLUFFY
    ...
    case type
      ...
      when :GHOST
        damage*=1.35 if @battle.pbWeather == :SHADOWSKY
        damage*=2 if attacker.ability == :ETHEREALSHROUD && @battle.FE == :HAUNTED
      when :DARK
        damage*=1.35 if @battle.pbWeather == :SHADOWSKY
      when :FAIRY
    ...
    if (attacker.hasType?(type) && (!attacker.effects[:DesertsMark])|| (attacker.ability == :STEELWORKER && type == :STEEL)  || (attacker.ability == :SOLARIDOL && type == :FIRE) || (attacker.ability == :LUNARIDOL && type == :ICE) || (attacker.ability == :ROCKYPAYLOAD && type == :ROCK) || typecrest==true) && attacker.ability != :ANCIENTPRESENCE
    ...
    damage *= 0.5 if attacker.status == :FROSTBITE && pbIsSpecial?(type) && attacker.ability != :FORTITUDE
    # Random Variance
    ...
    if !opponent.damagestate.critical && attacker.ability != :INFILTRATOR
      ...
      if @category != :status && opponent.pbOwnSide.screenActive?(betterCategory(type))
        ...
        finalmult *= (!opponent.pbPartner.isFainted? || attacker.midwayThroughMove) ? 0.53 : 0.4 if @battle.pbWeather == :SHADOWSKY # Resurgence - Shadow Sky increasing screens effectiveness
      end
    ...
    # Resurgence - Delta Minun's Crest reducing damage taken for itself and Delta Plusle by 25%
    finalmult *= 0.75 if (opponent.pbPartner.ability == :FRIENDGUARD && !opponent.moldbroken) || (opponent.crested == :MINUN && opponent.pbPartner.species == :PLUSLE && opponent.pbPartner.isDelta?) || (opponent.species == :PLUSLE && opponent.isDelta? && opponent.pbPartner.crested == :MINUN)
    # Resurgence - Delta Plusle's Crest increasing damage dealt for itself and Delta Minun by 30%
    finalmult *= 1.3 if (attacker.crested == :PLUSLE && attacker.pbPartner.species == :MINUN && attacker.pbPartner.isDelta?) || (attacker.species == :MINUN && attacker.isDelta? && attacker.pbPartner.crested == :PLUSLE)
    ...
    finalmult *= [1.0 + attacker.effects[:Metronome] * 0.4, 3.0].min if attacker.ability == :PENDULUM && attacker.movesUsed[-2] == attacker.movesUsed[-1]
    ...
    # Resurgence - Executioner
    finalmult *= 1.25 if (attacker.ability == :NEUROFORCE || attacker.ability == :EXECUTIONER) && opponent.damagestate.typemod > 4
    ...
    if hasberry && !([:UNNERVE, :ASONECHILLING, :ASONEGRIM].include?(attacker.ability) || [:UNNERVE, :ASONECHILLING, :ASONEGRIM].include?(attacker.pbPartner.ability))
      finalmult *= 0.5
      finalmult *= 0.5 if opponent.ability == :RIPEN || (opponent.crested == :CRUSTLE && !@battle.doublebattle)
      # Resurgence - Delta Crustle (Berry) Crest sharing berry effects with Partner
      # AsN - I know it looks ugly. I really don't know how else to code this. It's better than making a million effects.
      if opponent.crested == :CRUSTLE && opponent.pbPartner && @battle.doublebattle
        opponent.effects[:CrustleBerryPartner] = true
        case type
          when :FIGHTING 
            @partnerberry = type == :FIGHTING
            @partnerberryname = "Chople Berry"
          when :FLYING   
            @partnerberry = type == :FLYING
            @partnerberryname = "Coba Berry"
          when :POISON    
            @partnerberry = type == :POISON
            @partnerberryname = "Kebia Berry"
          when :GROUND   
            @partnerberry = type == :GROUND
            @partnerberryname = "Shuca Berry"
          when :ROCK       
            @partnerberry = type == :ROCK
            @partnerberryname = "Charti Berry"
          when :BUG        
            @partnerberry = type == :BUG
            @partnerberryname = "Tanga Berry"
          when :GHOST      
            @partnerberry = type == :GHOST
            @partnerberryname = "Kasib Berry"
          when :STEEL      
            @partnerberry = type == :STEEL
            @partnerberryname = "Babiri Berry"
          when :FIRE       
            @partnerberry = type == :FIRE
            @partnerberryname = "Occa Berry"
          when :WATER      
            @partnerberry = type == :WATER
            @partnerberryname = "Passho Berry"
          when :GRASS      
            @partnerberry = type == :GRASS
            @partnerberryname = "Rindo Berry"
          when :ELECTRIC   
            @partnerberry = type == :ELECTRIC
            @partnerberryname = "Wacan Berry"
          when :PSYCHIC    
            @partnerberry = type == :PSYCHIC
            @partnerberryname = "Papaya Berry"
          when :ICE        
            @partnerberry = type == :ICE
            @partnerberryname = "Yache Berry"
          when :DRAGON     
            @partnerberry = type == :DRAGON
            @partnerberryname = "Haban Berry"
          when :DARK       
            @partnerberry = type == :DARK
            @partnerberryname = "Colbur Berry"
          when :FAIRY      
            @partnerberry = type == :FAIRY
            @partnerberryname = "Roseli Berry"
          when :NORMAL
            @partnerberry = type == :NORMAL
            @partnerberryname = "Chilan Berry"
        end
      end
    ...
    # Resurgence - Delta Crustle (Berry) Crest sharing berry effects with Partner
    if @partnerberry && opponent.pbPartner.effects[:CrustleBerryPartner] == true && opponent.damagestate.typemod > 4
      finalmult *= 0.5
      finalmult *= 0.5 if opponent.ability == :RIPEN
      if !@battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("{3} shared its {1} with {2} to weaken the damage from the attack!", @partnerberryname, opponent.pbThis, opponent.pbPartner.pbThis))
      else
        @battle.pbDisplay(_INTL("{3} shared its {1} to weaken the damage to {2}!", @partnerberryname, opponent.pbThis, opponent.pbPartner.pbThis))
      end
      opponent.pbPartner.effects[:CrustleBerryPartner] = false
    end
...
################################################################################
  def pbEffectMessages(attacker,opponent,ignoretype=false)
    ...
    if !pbIsMultiHit && !attacker.effects[:ParentalBond] && !attacker.effects[:Lernean]
      if opponent.damagestate.typemod>4
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.effects[:Lernean]
      headnum = 4 + attacker.form
      currentheadnum = headnum + 1
    ...
    if opponent.damagestate.typemod!=0
      if !attacker.effects[:Lernean]
        pbShowAnimation(@move,attacker,opponent,hitnum,alltargets,showanimation)
      elsif attacker.effects[:Lernean] && hitnum == 0
        pbShowAnimation(@move,attacker,opponent,hitnum,alltargets,showanimation)
      end
      if self.function==0xC9 || self.function==0xCA || self.function==0xCB ||
        self.function==0xCC || self.function==0xCD || self.function==0xCE ||
        self.function==0x960 #Sprites for two turn moves
        @battle.scene.pbUnVanishSprite(attacker,false)
        if self.function==0xCE || self.function==0x960
  ...
  def priorityCheck(attacker)
    ...
    pri += 1 if attacker.pbOwnSide.effects[:JetStream] == 1 # Resurgence - Jet Stream
    pri += 2 if attacker.pbOwnSide.effects[:JetStream] == 1 && @battle.FE == :SKY # Resurgence - Jet Stream
