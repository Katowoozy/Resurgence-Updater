class PBEvolution
  ...
  Unhappiness    = 38
  DClodsire      = 39 # Resurgence - Delta Clodsire
  ...
  EVOPARAM=[0,     # Unknown (do not use)
     ...
     1,1,0,1       # EnemyDefeat, Palafin, Unhappiness, Delta Clodsire
  ]
end
...
def checkEvoConditions(pokemon,method,condition,evo)
  ...
  # Resurgence - Delta Clodsire Evolution
  case method
    when :Level, :Ninjask then return evo
    when :DClodsire then return evo if PWooper.include?($game_map.map_id)
...
def getEvolutionForm(mon,item=nil)
  ...
  when :MUNCHLAX
    if form == 1
      if SummerDSnorlax.include?($game_map.map_id)
        return 3
      elsif AutumnDSnorlax.include?($game_map.map_id)
        return 4
      elsif WinterDSnorlax.include?($game_map.map_id)
        return 5
      elsif SakuraDSnorlax.include?($game_map.map_id)
        return 6
      else
        return 2
      end
