...
class FightMenuButtons < BitmapSprite
  def pbFieldNotesBattle(move)
    ...
    when :UNDERWATER
      ...
      moveBoost = 0 if [:RAINDANCE, :SUNNYDAY, :HAIL, :SANDSTORM, :SHADOWSKY].include?(move.move)
    when :NEWWORLD
      moveBoost = 0 if [:ELECTRICTERRAIN, :GRASSYTERRAIN, :MISTYTERRAIN, :PSYCHICTERRAIN, :MIST].include?(move.move)
      moveBoost = 0 if [:RAINDANCE, :SUNNYDAY, :HAIL, :SANDSTORM, :SHADOWSKY].include?(move.move)
    when :ELECTERRAIN
  ...
  def refresh
    ...
    if !@battler.issossmon || (@battler.issossmon && @battler.index == 2)
      if @battler.pokemon.isPulse?
        imagepos.push(["Graphics/Pictures/Battle/battlePulseEvoBox.png",sbX+megaX,megaY,0,0,-1,-1])
      elsif @battler.pokemon.isRift?
        imagepos.push(["Graphics/Pictures/Battle/battleRiftEvoBox.png",sbX+megaX,megaY,0,0,-1,-1])
      elsif @battler.pokemon.isPerfection?
        imagepos.push(["Graphics/Pictures/Battle/battlePerfectionEvoBox.png",sbX+megaX,megaY,0,0,-1,-1])
      # Delta + Mega
      ...
      # Delta + Armored
        ...
        imagepos.push(["Graphics/Pictures/Battle/battleMegaEvoBox.png",sbX+megaX,megaY,0,0,-1,-1])
      elsif @battler.isArmored?(illusion)
      ...
      if @battler.species == :KYOGRE && @battler.form == 1 && !illusion
        imagepos.push(["Graphics/Pictures/Battle/BattleAlpha.png",sbX+megaX,megaY,0,0,-1,-1])
      end
      if @battler.species == :GROUDON && @battler.form == 1 && !illusion
        imagepos.push(["Graphics/Pictures/Battle/BattleOmega.png",sbX+megaX,megaY,0,0,-1,-1])
      end
      # Zeta / Omicron / Epsilon
      if @battler.species == :GIRATINA && @battler.form == 2 && !illusion
        imagepos.push(["Graphics/Pictures/Battle/BattleZeta.png",sbX+megaX,megaY,0,0,-1,-1])
      end
      if @battler.species == :ARCEUS && @battler.form == 19 && !illusion
        imagepos.push(["Graphics/Pictures/Battle/BattleOmicron.png",sbX+megaX,megaY,0,0,-1,-1])
      end
      if @battler.species == :REGIGIGAS && @battler.form == 1 && !illusion
        imagepos.push(["Graphics/Pictures/Battle/BattleEpsilon.png",sbX+megaX,megaY,0,0,-1,-1])      
      end
      # Owned
