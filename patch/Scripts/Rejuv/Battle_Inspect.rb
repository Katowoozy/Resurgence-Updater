PokeBattle_Battler.class_eval {
  def pbCalcAttack()
    ...
    atkmult = (atkmult * 1.5).round if self.flowerGiftActive?
    atkmult = (atkmult * 1.5).round if self.pbPartner.flowerGiftActive?
    ...
    atkmult = (atkmult * 1.3).round if self.ability == :PROTOABSOLUTION && self.effects[:Protoabsolution][0] == PBStats::ATTACK # Resurgence - Added Protoabsolution
    atkmult = (atkmult * 1.3).round if self.ability == :PROTOCRYOPHILIC && self.effects[:Protocryophilic][0] == PBStats::ATTACK # Resurgence - Added Protocryophilic
  ...
  def pbCalcSpAtk()
    ...
    atkmult = (atkmult * 1.5).round if self.ability == :FORTITUDE && !self.status.nil?
    ...
    atkmult = (atkmult * 2.0).round if self.ability == :ATHENIAN
    ...
    atkmult = (atkmult * 1.5).round if self.ability == :SUPERCELL && (@battle.pbWeather == :RAINDANCE || @battle.pbWeather == :SHADOWSKY  || [:FACTORY, :SHORTCIRCUIT, :ELECTRICTERRAIN].include?(@battle.FE))
    ...
    atkmult = (atkmult * 1.3).round if self.ability == :PROTOABSOLUTION && self.effects[:Protoabsolution][0] == PBStats::SPATK # Resurgence - Added Protoabsolution
    atkmult = (atkmult * 1.3).round if self.ability == :PROTOCRYOPHILIC && self.effects[:Protocryophilic][0] == PBStats::SPATK # Resurgence - Added Protocryophilic
  ...
  def pbCalcDefense()
    ...
    defmult = (defmult * 1.3).round if self.ability == :PROTOABSOLUTION && self.effects[:Protoabsolution][0] == PBStats::DEFENSE # Resurgence - Added Protoabsolution
    defmult = (defmult * 1.3).round if self.ability == :PROTOCRYOPHILIC && self.effects[:Protocryophilic][0] == PBStats::DEFENSE # Resurgence - Added Protocryophilic
   ...
   def pbCalcSpDef()
    ...
    defense = (defense * PBStats::StageMul[defstage]).floor
    defmult = 0x1000
    defense = (defense * 1.5).round if self.flowerGiftActive?
    defense = (defense * 1.5).round if self.pbPartner.flowerGiftActive?
    ...
    defmult = (defmult * 1.3).round if self.ability == :PROTOABSOLUTION && self.effects[:Protoabsolution][0] == PBStats::SPDEF # Resurgence - Added Protoabsolution
    defmult = (defmult * 1.3).round if self.ability == :PROTOCRYOPHILIC && self.effects[:Protocryophilic][0] == PBStats::SPDEF # Resurgence - Added Protocryophilic
...
def pbShowBattleStats(pkmn)
  ...
  report.push(_INTL("Amplifier: {1} turns",pkmn.effects[:Amplifier])) if pkmn.effects[:Amplifier]!=0
  ...
  report.push(_INTL("Protoabsolution: Active")) if pkmn.effects[:Protoabsolution][0]!=0 # Resurgence - Added Protoabsolution
  report.push(_INTL("Protocryophilic: Active")) if pkmn.effects[:Protocryophilic][0]!=0 # Resurgence - Added Protocryophilic
  ...
  report.push(_INTL("Superconduct: Electric")) if pkmn.ability == (:SUPERCONDUCT) && pkmn.effects[:ElectroCharge] == true # Resurgence - Superconduct Electric Attack Charged
  report.push(_INTL("Superconduct: Ice")) if pkmn.ability == (:SUPERCONDUCT) && pkmn.effects[:CryoCharge] == true # Resurgence - Superconduct Ice Attack Charged
  ...
  report.push(_INTL("Jet Stream: {1} turns",pkmn.pbOwnSide.effects[:JetStream])) if pkmn.pbOwnSide.effects[:JetStream]>0   
  ...
  report.push(_INTL("Stealth Rock active")) if pkmn.pbOwnSide.effects[:StealthRock] || pkmn.pbOwnSide.effects[:FoundryStealthRock]
  report.push(_INTL("Sticky Web active")) if pkmn.pbOwnSide.effects[:StickyWeb]

  report.push()
  # Resurgence - Delta Avalugg Crest calling all of its Abilities for Inspect using LAWDS' P3 Code
  if shownmon.ability==nil || (shownmon.ability.is_a?(PokeAbility) && shownmon.ability.ability==nil)
    report.push(_INTL("Ability: Ability Negated"))
  elsif shownmon.ability.is_a?(PokeAbility)
    if shownmon.ability.ability.is_a?(Array) && shownmon.crested == :AVALUGG
      abilstring = "Abilities:"
      report.push()
      report.push(_INTL(abilstring))
      for i in shownmon.ability.ability
        next if i==nil
        report.push(_INTL("    {1}",getAbilityName(i)))
      end
      report.push()
    else
      abilstring = "Ability: {1}"
      report.push(_INTL(abilstring, getAbilityName(shownmon.ability.ability)))
    end
  else
    report.push(_INTL("Ability: {1}",shownmon.ability.nil? ? "Ability Negated" : getAbilityName(shownmon.ability)))
  end
