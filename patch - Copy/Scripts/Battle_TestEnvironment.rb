      ...
      if @choices[i.index][0] == :switch
        ...
        for j in priority
          ...
          if pbChoseMoveFunctionCode?(j.index,0x88) && !j.effects[:Pursuit] && (@choices[j.index][3]==-1 || @choices[j.index][3]==i.index)
            if j.status!=:SLEEP && (!j.ability == (:TRUANT) || !j.effects[:Truant])
...
class PokeBattle_Scene
  ...
  def pbCanUseBattleItem(pkmnid, item, pkmnscreen)
    ...
    if battler && (battler.effects[:SkyDrop] || battler.effects[:SpiritAway])
      return false
    end
...
class PokeBattle_Pokemon
  def initialize(species, level, player = nil, withMoves = true, form = 0)
    @form = form
