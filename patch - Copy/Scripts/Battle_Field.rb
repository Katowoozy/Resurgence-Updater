...
class PokeBattle_Battle
  def noWeather
    if @weather != 0 && !pbCheckGlobalAbility(:TEMPEST)
      if @field.effect == :NEWWORLD
        ...
        # Gen 9 Mod - Hail/Snow/Both
        pbDisplay(_INTL("The " + HAILSNOWLOWMOD + " melted away!"))
        @weather = 0
        @weatherduration = 0
        persistentWeather
  ...
  def setField(fieldeffect, temp = 0, add_on: false, noEntryAbilities: false)
    ...
    # add_on = if true this field layers on top of the current field in the field stack, if the field has temp greater than 0, this will happen regardless
    # noEntryAbilities = if true prevents the reactivation of field entry abilities, mainly used for stage based fields where changing the stage isn't regarded as a true field change
  ...
  def fieldEffectAfterMove(basemove, user)
    ...
    if newfield == :INDOOR
      ...
      setField(newfield, add_on: dont_change_backup, noEntryAbilities: true)
    else
  ...
  def mistExplosion(basemove, user)
    if !pbCheckGlobalAbility(:DAMP)
      ...
      for i in @battlers
        ...
        next if i.effects[:SpiritAway] # Resurgence - Spirit Away Invulnerability
        next if i.effects[:Commander] # Gen 9 Mod - Added Commander immunity when inside Dondozo's mouth.
  ...
  def caveCollapse(basemove, user)
    if @field.counter == 0
      ...
      for i in @battlers
        ...
        next if i.effects[:SkyDrop]
        next if i.effects[:SpiritAway] # Resurgence - Spirit Away Invulnerability
        next if i.effects[:Commander] # Gen 9 Mod - Added Commander immunity to when inside Dondozo's mouth.

        quakedrop -= 1 if i.effects[:Endure] || i.ability == :STURDY
        quakedrop /= 2 if i.ability == :SHELLARMOR || i.ability == :BATTLEARMOR
        quakedrop /= 3 if i.ability == :PRISMARMOR || i.ability == :SOLIDROCK || i.ability == :PRISMGUARD
  ...
  def fieldAccuracyDrop
    ...
    for i in 0...4
      ...
      canthit = true if @battle.battlers[i].effects[:SkyDrop] || @battle.battlers[i].effects[:SpiritAway] # Resurgence - Spirit Away Invulnerability
  ...
  def growField(text, user)
    ...
    setField(newfield, add_on: false, noEntryAbilities: true)
  ...
  def reduceField
    ...
    setField(newfield, add_on: false, noEntryAbilities: true)
    pbDisplay(_INTL("The garden regressed in growth!")) if @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN)
  ...
  def fieldDefenseBoost(type, target)
    ...
    case @battle.FE
      ...
      when :DARKCRYSTALCAVERN
        ...
        defmult *= 1.33 if target.ability == :PRISMGUARD # Resurgence
      when :RAINBOW
        defmult *= 1.33 if target.ability == :PRISMARMOR
        defmult *= 1.33 if target.ability == :PRISMGUARD # Resurgence
      when :DRAGONSDEN
...
class PokeBattle_Battler
  def burningFieldPassiveDamage?
    ...
    return false if [:FLAREBOOST, :MAGMAARMOR, :FLAMEBODY, :FLASHFIRE, :BLAZEBOOST].include?(@ability)
