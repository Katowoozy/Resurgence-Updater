class PokeBattle_Battler
  ...
  def pbCanSleep?(showMessages, selfsleep = false, ignorestatus = false, moldbroken = self.moldbroken)
    ...
    for i in 0...4
      failure = :Uproar if @battle.battlers[i].effects[:Uproar] > 0 && @battle.lastMoveUsed != :YAWN
      failure = :Amplifier if @battle.battlers[i].effects[:Amplifier] > 0
    end
    return true if failure == :none
    if showMessages
      case failure
        when :Uproar then @battle.pbDisplay(_INTL("But the uproar kept {1} awake!", pbThis(true)))
        when :Amplifier then @battle.pbDisplay(_INTL("But the amplified sound kept {1} awake!", pbThis(true)))
  ...
  def pbCanSleepYawn?
    ...
    return true if failure == :none
    case failure
      when :Uproar then @battle.pbDisplay(_INTL("The uproar kept {1} awake!", pbThis(true)))
      when :Amplifier then @battle.pbDisplay(_INTL("The amplified sound kept {1} awake!", pbThis(true)))
  ...
  # Frostbite
  #===============================================================================
  def pbCanFrostbite?(showMessages, ownFrostOrb = false, moldbroken = self.moldbroken)
    return false if isFainted? && !(Rejuv && isbossmon && @shieldCount > 0)

    if self.status == :FROSTBITE
      @battle.pbDisplay(_INTL("{1} is already frostbitten.", pbThis)) if showMessages
      return false
    end
    return false if !pbCanStatus?(showMessages, false, moldbroken)

    failure = :none
    failure = :MAGMAARMOR if self.ability == :MAGMAARMOR && !moldbroken && @battle.FE != :FROZENDIMENSION
    failure = :TYPE if hasType?(:ICE)
    failure = :HotField if @battle.FE == :VOLCANIC || @battle.FE == :BURNING


    return true if failure == :none
    if showMessages
      case failure
        when :MAGMAARMOR then @battle.pbDisplay(_INTL("{1}'s {2} prevents burns!", pbThis, getAbilityName(self.ability)))
        when :TYPE then @battle.pbDisplay(_INTL("It doesn't affect {1}...", pbThis(true)))
        when :HotField then @battle.pbDisplay(_INTL("The hot temperatures prevented {1} from being frostbitten!", pbThis))
      end
    end
    return false
  end
  ...
  def pbContinueStatus(showAnim = true)
    ...
    if self.isbossmon
      if self.chargeAttack
        if [:SLEEP, :PARALYSIS].include?(self.status)
  ...
  def flowerGiftActive?
    ...
    return true if @battle.pbWeather == :SUNNYDAY && !self.hasWorkingItem(:UTILITYUMBRELLA)
