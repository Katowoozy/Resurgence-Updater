    ...
    when :CASTFORM
      ...
      when 0x102
        if @weather!=:HAIL
          ...
          pbDisplay(_INTL("{1}'s " + HAILSNOWNAMEMOD + " made it " + HAILSNOWLOWMOD + "!",@battlers[index].pbThis))
          @battlers[index].pbCheckForm
        end
      when 0x957
        if @weather!=:SHADOWSKY
          @weather=:SHADOWSKY
          @weatherduration=5
          @weatherduration=8 if @field.effect == :DARKCRYSTALCAVERN || @field.effect == :SHORTCIRCUIT || @field.effect == :DIMENSIONAL || @field.effect == :FROZENDIMENSION || @field.effect == :SKY || @field.effect == :DARKNESS1 || @field.effect == :DARKNESS2 || @field.effect == :DARKNESS3
          pbCommonAnimation("ShadowSky",nil,nil)
          pbDisplay(_INTL("{1}'s Shadow Prayer darkened the sky!",@battlers[index].pbThis))
          @battlers[index].pbCheckForm
        end        
      end
    when :THIEVUL
