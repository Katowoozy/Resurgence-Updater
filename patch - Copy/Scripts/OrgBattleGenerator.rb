...
def pbRandomPokemonFromRule(rule,trainer)
  ...
  begin
    ...
    if (item == :BLACKSLUDGE)
      if $cache.pkmn[species][:Type1] != :POISON && $cache.pkmn[species][:Type2] != :POISON
        item=:LEFTOVERS
      end
    end
...
def pbDecideWinnerScore(party0,party1,rating)
  ...
  for i in 0...party0.length
    for move in party0[i].moves
      next if move.move==0
      for j in 0...party1.length
        score+=pbDecideWinnerEffectiveness(move.move,
  ...
  if bttrainers.length==0
    ...
    bttrainers.sort!{|a,b|
       money1=(!trainertypes[a[0]] ||
               !trainertypes[a[0]].checkFlag?(:moneymult)) ? 30 : trainertypes[a[0]].checkFlag?(:moneymult)
       money2=(!trainertypes[b[0]] ||
               !trainertypes[b[0]].checkFlag?(:moneymult)) ? 30 : trainertypes[b[0]].checkFlag?(:moneymult)
